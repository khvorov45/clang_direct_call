...
#include "llvm_include_llvm_LTO_legacy_ThinLTOCodeGenerator.h"
...
#include "llvm_include_llvm_Bitcode_BitcodeWriterPass.h"
...
#include "llvm_include_llvm_LTO_SummaryBasedOptimizations.h"
#include "llvm_include_llvm_MC_SubtargetFeature.h"
#include "llvm_include_llvm_MC_TargetRegistry.h"
#include "llvm_include_llvm_Object_IRObjectFile.h"
#include "llvm_include_llvm_Passes_PassBuilder.h"
#include "llvm_include_llvm_Passes_StandardInstrumentations.h"
#include "llvm_include_llvm_Remarks_HotnessThresholdParser.h"
#include "llvm_include_llvm_Support_CachePruning.h"
#include "llvm_include_llvm_Support_Debug.h"
#include "llvm_include_llvm_Support_Error.h"
#include "llvm_include_llvm_Support_FileUtilities.h"
#include "llvm_include_llvm_Support_Path.h"
#include "llvm_include_llvm_Support_SHA1.h"
#include "llvm_include_llvm_Support_SmallVectorMemoryBuffer.h"
#include "llvm_include_llvm_Support_ThreadPool.h"
#include "llvm_include_llvm_Support_Threading.h"
...
#include "llvm_include_llvm_Transforms_Utils_FunctionImportUtils.h"
...
static void
saveTempBitcode(const Module& TheModule, StringRef TempDir, unsigned count, StringRef Suffix) {
    if (TempDir.empty())
        return;
    ...
    if (EC)
        report_fatal_error(Twine("Failed to open ") + SaveTempPath + " to save optimized bitcode\n");
...
getFirstDefinitionForLinker(const GlobalValueSummaryList& GVSummaryList) {
    ...
    auto StrongDefForLinker = llvm::find_if(
        ...
        [](const std::unique_ptr<GlobalValueSummary>& Summary) {
            ...
            return !GlobalValue::isAvailableExternallyLinkage(Linkage) && !GlobalValue::isWeakForLinker(Linkage);
        }
    );
...
generateModuleMap(std::vector<std::unique_ptr<lto::InputFile>>& Modules) {
    ...
    for (auto& M : Modules) {
        assert(ModuleMap.find(M->getName()) == ModuleMap.end() && "Expect unique Buffer Identifier");
...
static void
promoteModule(Module& TheModule, const ModuleSummaryIndex& Index, bool ClearDSOLocalOnDeclarations) {
...
namespace {
    class ThinLTODiagnosticInfo: public DiagnosticInfo {
      ...
      public:
        ThinLTODiagnosticInfo(const Twine& DiagMsg, DiagnosticSeverity Severity = DS_Error) :
...
static void
verifyLoadedModule(Module& TheModule) {
...
loadModuleFromInput(lto::InputFile* Input, LLVMContext& Context, bool Lazy, bool IsImporting) {
    ...
    if (!ModuleOrErr) {
        handleAllErrors(ModuleOrErr.takeError(), [&](ErrorInfoBase& EIB) {
            SMDiagnostic Err = SMDiagnostic(Mod.getModuleIdentifier(), SourceMgr::DK_Error, EIB.message());
...
static void
crossImportIntoModule(Module& TheModule, const ModuleSummaryIndex& Index, StringMap<lto::InputFile*>& ModuleMap, const FunctionImporter::ImportMapTy& ImportList, bool ClearDSOLocalOnDeclarations) {
    ...
    if (!Result) {
        handleAllErrors(Result.takeError(), [&](ErrorInfoBase& EIB) {
            SMDiagnostic Err = SMDiagnostic(TheModule.getModuleIdentifier(), SourceMgr::DK_Error, EIB.message());
            Err.print("ThinLTO", errs());
        });
...
static void
optimizeModule(Module& TheModule, TargetMachine& TM, unsigned OptLevel, bool Freestanding, bool DebugPassManager, ModuleSummaryIndex* Index) {
...
static void
addUsedSymbolToPreservedGUID(const lto::InputFile& File, DenseSet<GlobalValue::GUID>& PreservedGUID) {
...
static void
computeGUIDPreservedSymbols(const lto::InputFile& File, const StringSet<>& PreservedSymbols, const Triple& TheTriple, DenseSet<GlobalValue::GUID>& GUIDs) {
...
computeGUIDPreservedSymbols(const lto::InputFile& File, const StringSet<>& PreservedSymbols, const Triple& TheTriple) {
    DenseSet<GlobalValue::GUID> GUIDPreservedSymbols(PreservedSymbols.size());
    computeGUIDPreservedSymbols(File, PreservedSymbols, TheTriple, GUIDPreservedSymbols);
    return GUIDPreservedSymbols;
}

std::unique_ptr<MemoryBuffer>
codegenModule(Module& TheModule, TargetMachine& TM) {
...
class ModuleCacheEntry {
  ...
  public:
    ...
    ModuleCacheEntry(
        ...
        SmallString<40> Key;
        computeLTOCacheKey(Key, Conf, Index, ModuleID, ImportList, ExportList, ResolvedODR, DefinedGVSummaries);
    ...
    void
    write(const MemoryBuffer& OutputBuffer) {
        if (EntryPath.empty())
            return;
        ...
        if (auto Err = handleErrors(
                llvm::writeFileAtomically(TempFilename, EntryPath, OutputBuffer.getBuffer()),
                [](const llvm::AtomicFileWriteError& E) {
                    std::string              ErrorMsgBuffer;
                    llvm::raw_string_ostream S(ErrorMsgBuffer);
                    E.log(S);

                    if (E.Error == llvm::atomic_write_error::failed_to_create_uniq_file) {
...
ProcessThinLTOModule(Module& TheModule, ModuleSummaryIndex& Index, StringMap<lto::InputFile*>& ModuleMap, TargetMachine& TM, const FunctionImporter::ImportMapTy& ImportList, const FunctionImporter::ExportSetTy& ExportList, const DenseSet<GlobalValue::GUID>& GUIDPreservedSymbols, const GVSummaryMapTy& DefinedGlobals, const ThinLTOCodeGenerator::CachingOptions& CacheOptions, bool DisableCodeGen, StringRef SaveTempsDir, bool Freestanding, unsigned OptLevel, unsigned count, bool DebugPassManager) {
    ...
    bool ClearDSOLocalOnDeclarations =
        TM.getTargetTriple().isOSBinFormatELF() && TM.getRelocationModel() != Reloc::Static && TheModule.getPIELevel() == PIELevel::Default;
    ...
    if (!SingleModule) {
        crossImportIntoModule(TheModule, Index, ModuleMap, ImportList, ClearDSOLocalOnDeclarations);

        // Save temps: after cross-module import.
        saveTempBitcode(TheModule, SaveTempsDir, count, ".3.imported.bc");
    }

    optimizeModule(TheModule, TM, OptLevel, Freestanding, DebugPassManager, &Index);
...
resolvePrevailingInIndex(
    ...
    lto::Config Conf;
    thinLTOResolvePrevailingInIndex(Conf, Index, isPrevailing, recordNewLinkage, GUIDPreservedSymbols);
}

// Initialize the TargetMachine builder for a given Triple
static void
initTMBuilder(TargetMachineBuilder& TMBuilder, const Triple& TheTriple) {
    ...
    if (TMBuilder.MCpu.empty() && TheTriple.isOSDarwin()) {
        ...
        else if (TheTriple.getArch() == llvm::Triple::aarch64 || TheTriple.getArch() == llvm::Triple::aarch64_32)
...
void
ThinLTOCodeGenerator::addModule(StringRef Identifier, StringRef Data) {
    MemoryBufferRef Buffer(Data, Identifier);

    auto InputOrError = lto::InputFile::create(Buffer);
    if (!InputOrError)
        report_fatal_error(Twine("ThinLTO cannot create input file: ") + toString(InputOrError.takeError()));
    ...
    if (Modules.empty())
        ...
        if (!TMBuilder.TheTriple.isCompatibleWith(TheTriple))
            report_fatal_error(
                "ThinLTO modules with incompatible triples not "
                "supported"
            );
...
void
ThinLTOCodeGenerator::preserveSymbol(StringRef Name) {
    PreservedSymbols.insert(Name);
}

void
ThinLTOCodeGenerator::crossReferenceSymbol(StringRef Name) {
...
struct IsExported {
    ...
    bool
    operator()(StringRef ModuleIdentifier, ValueInfo VI) const {
        const auto& ExportList = ExportLists.find(ModuleIdentifier);
        return (ExportList != ExportLists.end() && ExportList->second.count(VI)) || GUIDPreservedSymbols.count(VI.getGUID());
    }
};
...
struct IsPrevailing {
    ...
    bool
    operator()(GlobalValue::GUID GUID, const GlobalValueSummary* S) const {
        const auto& Prevailing = PrevailingCopy.find(GUID);
        // Not in map means that there was only one copy, which must be prevailing.
        if (Prevailing == PrevailingCopy.end())
            return true;
 ...
 */
void
ThinLTOCodeGenerator::promote(Module& TheModule, ModuleSummaryIndex& Index, const lto::InputFile& File) {
    ...
    ComputeCrossModuleImport(Index, ModuleToDefinedGVSummaries, ImportLists, ExportLists);
    ...
    StringMap<std::map<GlobalValue::GUID, GlobalValue::LinkageTypes>> ResolvedODR;
    resolvePrevailingInIndex(Index, ResolvedODR, GUIDPreservedSymbols, PrevailingCopy);

    thinLTOFinalizeInModule(TheModule, ModuleToDefinedGVSummaries[ModuleIdentifier],
 ...
 */
void
ThinLTOCodeGenerator::crossModuleImport(Module& TheModule, ModuleSummaryIndex& Index, const lto::InputFile& File) {
 ...
 */
void
ThinLTOCodeGenerator::emitImports(Module& TheModule, StringRef OutputName, ModuleSummaryIndex& Index, const lto::InputFile& File) {
    auto ModuleCount = Index.modulePaths().size();
    auto ModuleIdentifier = TheModule.getModuleIdentifier();
    ...
    std::error_code EC;
    if ((EC = EmitImportsFiles(ModuleIdentifier, OutputName, ModuleToSummariesForIndex)))
 ...
 */
void
ThinLTOCodeGenerator::internalize(Module& TheModule, ModuleSummaryIndex& Index, const lto::InputFile& File) {
    ...
    thinLTOInternalizeModule(TheModule, ModuleToDefinedGVSummaries[ModuleIdentifier]);
}
 ...
 */
void
ThinLTOCodeGenerator::optimize(Module& TheModule) {
    initTMBuilder(TMBuilder, Triple(TheModule.getTargetTriple()));

    // Optimize now
    optimizeModule(TheModule, *TMBuilder.create(), OptLevel, Freestanding, DebugPassManager, nullptr);
}
...
std::string
ThinLTOCodeGenerator::writeGeneratedObject(int count, StringRef CacheEntryPath, const MemoryBuffer& OutputBuffer) {
    auto             ArchName = TMBuilder.TheTriple.getArchName();
    SmallString<128> OutputPath(SavedObjectsDirectoryPath);
    llvm::sys::path::append(OutputPath, Twine(count) + "." + ArchName + ".thinlto.o");
...
ThinLTOCodeGenerator::run() {
    ...
    for (const auto& M : Modules)
        computeGUIDPreservedSymbols(*M, PreservedSymbols, TMBuilder.TheTriple, GUIDPreservedSymbols);
    ...
    updateIndexWPDForExports(*Index, IsExported(ExportLists, GUIDPreservedSymbols), LocalWPDTargetsMap);
    thinLTOInternalizeAndPromoteInIndex(
        *Index,
        IsExported(ExportLists, GUIDPreservedSymbols),
        IsPrevailing(PrevailingCopy)
    ...
    // Parallel optimizer + codegen
    {
        ...
        for (auto IndexCount : ModulesOrdering) {
            ...
            Pool.async([&](int count) {
                ...
                ModuleCacheEntry CacheEntry(CacheOptions.Path, *Index, ModuleIdentifier, ImportLists[ModuleIdentifier], ExportList, ResolvedODR[ModuleIdentifier], DefinedGVSummaries, OptLevel, Freestanding, TMBuilder);
                ...
                if (!DiagFileOrErr) {
                    ...
                    report_fatal_error(
                        "ThinLTO: Can't get an output file for the "
                        "remarks"
                    );
                }
