...
unsigned
AsmPrinter::addInlineAsmDiagBuffer(StringRef AsmStr, const MDNode* LocMDNode) const {
...
void
AsmPrinter::emitInlineAsm(StringRef Str, const MCSubtargetInfo& STI, const MCTargetOptions& MCOptions, const MDNode* LocMDNode, InlineAsm::AsmDialect Dialect) const {
    ...
    if (!MCAI->useIntegratedAssembler() && !MCAI->parseInlineAsmUsingAsmParser() && !OutStreamer->isIntegratedAssemblerRequired()) {
        emitInlineAsmStart();
    ...
    std::unique_ptr<MCAsmParser> Parser(createMCAsmParser(SrcMgr, OutContext, *OutStreamer, *MAI, BufNum));
    ...
    if (TM.getTarget().MCAsmParserCtorFn == 0) {
        report_fatal_error("Inline asm not supported by this streamer because we don't have an asm parser for this target\n");
    }
    std::unique_ptr<MCTargetAsmParser> TAP(TM.getTarget().MCAsmParserCtorFn(STI, *Parser, *MII, MCOptions));
...
static void
EmitInlineAsmStr(const char* AsmStr, const MachineInstr* MI, MachineModuleInfo* MMI, const MCAsmInfo* MAI, AsmPrinter* AP, uint64_t LocCookie, raw_ostream& OS) {
    ...
    while (*LastEmitted) {
        switch (*LastEmitted) {
            default: {
                ...
                while (*LiteralEnd && *LiteralEnd != '{' && *LiteralEnd != '|' && *LiteralEnd != '}' && *LiteralEnd != '$' && *LiteralEnd != '\n')
                    ++LiteralEnd;
            ...
            case '$': {
                ...
                switch (*LastEmitted) {
                    ...
                    case '(':  // $( -> same as GCC's { character.
                        ++LastEmitted;  // Consume '(' character.
                        if (CurVariant != -1)
                            report_fatal_error("Nested variants found in inline asm string: '" + Twine(AsmStr) + "'");
                ...
                if (StringRef(IDStart, IDEnd - IDStart).getAsInteger(10, Val))
                    report_fatal_error("Bad $ operand number in inline asm string: '" + Twine(AsmStr) + "'");
                LastEmitted = IDEnd;

                if (Val >= NumOperands - 1)
                    report_fatal_error("Invalid $ operand number in inline asm string: '" + Twine(AsmStr) + "'");
                ...
                if (HasCurlyBraces) {
                    ...
                    if (*LastEmitted == ':') {
                        ++LastEmitted;  // Consume ':' character.
                        if (*LastEmitted == 0)
                            report_fatal_error("Bad ${:} expression in inline asm string: '" + Twine(AsmStr) + "'");
                ...
                if (CurVariant == -1 || CurVariant == AsmPrinterVariant) {
                    ...
                    if (OpNo >= MI->getNumOperands() || MI->getOperand(OpNo).isMetadata()) {
                        ...
                        if (MI->getOperand(OpNo).isBlockAddress()) {
                            ...
                            Error = AP->PrintAsmOperand(MI, OpNo, Modifier[0] ? Modifier : nullptr, OS);
                        }
                    }
                    if (Error) {
                        std::string        msg;
...
/// inline asm.
void
AsmPrinter::emitInlineAsm(const MachineInstr* MI) const {
    ...
    for (const MachineOperand& MO : llvm::reverse(MI->operands())) {
        if (MO.isMetadata() && (LocMD = MO.getMetadata()) && LocMD->getNumOperands() != 0) {
    ...
    emitInlineAsm(OS.str(), getSubtargetInfo(), TM.Options.MCOptions, LocMD, MI->getInlineAsmDialect());
...
void
AsmPrinter::PrintSpecial(const MachineInstr* MI, raw_ostream& OS, StringRef Code) const {
    if (Code == "private") {
...
void
AsmPrinter::PrintSymbolOperand(const MachineOperand& MO, raw_ostream& OS) {
...
bool
AsmPrinter::PrintAsmOperand(const MachineInstr* MI, unsigned OpNo, const char* ExtraCode, raw_ostream& O) {
...
bool
AsmPrinter::PrintAsmMemoryOperand(const MachineInstr* MI, unsigned OpNo, const char* ExtraCode, raw_ostream& O) {
    // Target doesn't support this yet!
    return true;
}

void
AsmPrinter::emitInlineAsmStart() const {}

void
AsmPrinter::emitInlineAsmEnd(const MCSubtargetInfo& StartInfo, const MCSubtargetInfo* EndInfo) const {}
