...
#include "clang_lib_Frontend_TestModuleFileExtension.h"
#include "clang_include_clang_Basic_Builtins.h"
#include "clang_include_clang_Basic_CharInfo.h"
#include "clang_include_clang_Basic_CodeGenOptions.h"
#include "clang_include_clang_Basic_CommentOptions.h"
#include "clang_include_clang_Basic_DebugInfoOptions.h"
...
#include "clang_include_clang_Basic_ObjCRuntime.h"
#include "clang_include_clang_Basic_Sanitizers.h"
...
#include "clang_include_clang_Basic_Visibility.h"
#include "clang_include_clang_Basic_XRayInstr.h"
...
#include "clang_include_clang_Frontend_CommandLineSourceLoc.h"
...
#include "clang_include_clang_Frontend_FrontendPluginRegistry.h"
...
#include "clang_include_clang_Lex_HeaderSearchOptions.h"
#include "clang_include_clang_Lex_PreprocessorOptions.h"
#include "clang_include_clang_Sema_CodeCompleteOptions.h"
#include "clang_include_clang_Serialization_ASTBitCodes.h"
...
#include "llvm_include_llvm_ADT_CachedHashString.h"
#include "llvm_include_llvm_ADT_DenseSet.h"
#include "llvm_include_llvm_ADT_FloatingPointMode.h"
...
#include "llvm_include_llvm_Option_OptSpecifier.h"
#include "llvm_include_llvm_Option_OptTable.h"
#include "llvm_include_llvm_Option_Option.h"
#include "llvm_include_llvm_ProfileData_InstrProfReader.h"
...
#include "llvm_include_llvm_Support_HashBuilder.h"
...
static Expected<std::optional<uint32_t>>
parseToleranceOption(StringRef Arg) {
    uint32_t Val;
    if (Arg.getAsInteger(10, Val))
        return llvm::createStringError(llvm::inconvertibleErrorCode(), "Not an integer: %s", Arg.data());
    return Val;
}
...
normalizeSimpleFlag(OptSpecifier Opt, unsigned TableIndex, const ArgList& Args, DiagnosticsEngine& Diags) {
    if (Args.hasArg(Opt))
        return true;
    return std::nullopt;
}
...
static std::optional<bool>
normalizeSimpleNegativeFlag(OptSpecifier Opt, unsigned, const ArgList& Args, DiagnosticsEngine&) {
    if (Args.hasArg(Opt))
        return false;
    return std::nullopt;
}
...
/// argument.
static void
denormalizeSimpleFlag(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator, Option::OptionClass, unsigned, /*T*/...) {
...
makeFlagToValueNormalizer(T Value) {
    return [Value](OptSpecifier Opt, unsigned, const ArgList& Args, DiagnosticsEngine&) -> std::optional<T> {
        if (Args.hasArg(Opt))
            return Value;
        return std::nullopt;
    };
...
static auto
makeBooleanOptionNormalizer(bool Value, bool OtherValue, OptSpecifier OtherOpt) {
    return [Value, OtherValue, OtherOpt](OptSpecifier Opt, unsigned, const ArgList& Args, DiagnosticsEngine&) -> std::optional<bool> {
...
static auto
makeBooleanOptionDenormalizer(bool Value) {
    return [Value](SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator, Option::OptionClass, unsigned, bool KeyPath) {
        if (KeyPath == Value)
            Args.push_back(Spelling);
    };
}
...
static void
denormalizeStringImpl(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator SA, Option::OptionClass OptClass, unsigned, const Twine& Value) {
    switch (OptClass) {
...
template<typename T>
static void
denormalizeString(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator SA, Option::OptionClass OptClass, unsigned TableIndex, T Value) {
...
normalizeSimpleEnum(OptSpecifier Opt, unsigned TableIndex, const ArgList& Args, DiagnosticsEngine& Diags) {
...
static void
denormalizeSimpleEnumImpl(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator SA, Option::OptionClass OptClass, unsigned TableIndex, unsigned Value) {
    assert(TableIndex < SimpleEnumValueTablesSize);
    const SimpleEnumValueTable& Table = SimpleEnumValueTables[TableIndex];
    if (auto MaybeEnumVal = findValueTableByValue(Table, Value)) {
        denormalizeString(Args, Spelling, SA, OptClass, TableIndex, MaybeEnumVal->Name);
    } else {
        llvm_unreachable(
...
template<typename T>
static void
denormalizeSimpleEnum(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator SA, Option::OptionClass OptClass, unsigned TableIndex, T Value) {
    return denormalizeSimpleEnumImpl(Args, Spelling, SA, OptClass, TableIndex, static_cast<unsigned>(Value));
}

static std::optional<std::string>
normalizeString(OptSpecifier Opt, int TableIndex, const ArgList& Args, DiagnosticsEngine& Diags) {
    auto* Arg = Args.getLastArg(Opt);
    if (!Arg)
        return std::nullopt;
...
static std::optional<IntTy>
normalizeStringIntegral(OptSpecifier Opt, int, const ArgList& Args, DiagnosticsEngine& Diags) {
    auto* Arg = Args.getLastArg(Opt);
    if (!Arg)
        return std::nullopt;
    IntTy Res;
...
normalizeStringVector(OptSpecifier Opt, int, const ArgList& Args, DiagnosticsEngine&) {
    return Args.getAllArgValues(Opt);
}

static void
denormalizeStringVector(SmallVectorImpl<const char*>& Args, const char* Spelling, CompilerInvocation::StringAllocator SA, Option::OptionClass OptClass, unsigned TableIndex, const std::vector<std::string>& Values) {
    switch (OptClass) {
        case Option::CommaJoinedClass: {
            ...
            denormalizeString(Args, Spelling, SA, Option::OptionClass::JoinedClass, TableIndex, CommaJoinedValue);
            break;
        }
        case Option::JoinedClass:
        case Option::SeparateClass:
...
static std::optional<std::string>
normalizeTriple(OptSpecifier Opt, int TableIndex, const ArgList& Args, DiagnosticsEngine& Diags) {
    auto* Arg = Args.getLastArg(Opt);
    if (!Arg)
        return std::nullopt;
...
static T
mergeForwardValue(T KeyPath, U Value) {
    return static_cast<T>(Value);
}

template<typename T, typename U>
static T
mergeMaskValue(T KeyPath, U Value) {
...
#define GENERATE_OPTION_WITH_MARSHALLING( \
    ...
    if ((FLAGS)&options::CC1Option) { \
        [&](const auto& Extracted) { \
            if (ALWAYS_EMIT || (Extracted != static_cast<decltype(KEYPATH)>((IMPLIED_CHECK) ? (IMPLIED_VALUE) : (DEFAULT_VALUE)))) \
                DENORMALIZER(ARGS, SPELLING, STRING_ALLOCATOR, Option::KIND##Class, TABLE_INDEX, Extracted); \
        }(EXTRACTOR(KEYPATH)); \
    }
...
static bool
FixupInvocation(CompilerInvocation& Invocation, DiagnosticsEngine& Diags, const ArgList& Args, InputKind IK) {
    ...
    if (Arg* A = Args.getLastArg(OPT_fdefault_calling_conv_EQ)) {
        ...
        bool emitError = (DefaultCC == LangOptions::DCC_FastCall || DefaultCC == LangOptions::DCC_StdCall) && Arch != llvm::Triple::x86;
        emitError |= (DefaultCC == LangOptions::DCC_VectorCall || DefaultCC == LangOptions::DCC_RegCall) && !T.isX86();
        if (emitError)
            Diags.Report(diag::err_drv_argument_not_allowed_with)
...
static unsigned
getOptimizationLevel(ArgList& Args, InputKind IK, DiagnosticsEngine& Diags) {
    unsigned DefaultOpt = llvm::CodeGenOpt::None;
    if ((IK.getLanguage() == Language::OpenCL || IK.getLanguage() == Language::OpenCLCXX) && !Args.hasArg(OPT_cl_opt_disable))
...
static unsigned
getOptimizationLevelSize(ArgList& Args) {
    if (Arg* A = Args.getLastArg(options::OPT_O_Group)) {
...
static void
GenerateArg(SmallVectorImpl<const char*>& Args, llvm::opt::OptSpecifier OptSpecifier, CompilerInvocation::StringAllocator SA) {
    Option Opt = getDriverOptTable().getOption(OptSpecifier);
    denormalizeSimpleFlag(Args, SA(Opt.getPrefix() + Opt.getName()), SA, Option::OptionClass::FlagClass, 0);
}

static void
GenerateArg(SmallVectorImpl<const char*>& Args, llvm::opt::OptSpecifier OptSpecifier, const Twine& Value, CompilerInvocation::StringAllocator SA) {
    Option Opt = getDriverOptTable().getOption(OptSpecifier);
    denormalizeString(Args, SA(Opt.getPrefix() + Opt.getName()), SA, Opt.getKind(), 0, Value);
}

// Parse command line arguments into CompilerInvocation.
using ParseFn =
    llvm::function_ref<bool(CompilerInvocation&, ArrayRef<const char*>, DiagnosticsEngine&, const char*)>;
...
static void
getAllNoBuiltinFuncValues(ArgList& Args, std::vector<std::string>& Funcs) {
...
static void
GenerateAnalyzerArgs(AnalyzerOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
#define ANALYSIS_CONSTRAINTS(NAME, CMDFLAG, DESC, CREATFN) \
    case NAME##Model: \
        ...
        break;
#include "clang_include_clang_StaticAnalyzer_Core_Analyses.def"
            default:
...
static bool
ParseAnalyzerArgs(AnalyzerOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
#include "clang_include_clang_StaticAnalyzer_Core_Analyses.def"
    ...
    for (const Arg* A :
        ...
        for (const StringRef& CheckerOrPackage : CheckersAndPackages)
            Opts.CheckersAndPackages.emplace_back(std::string(CheckerOrPackage), IsEnabled);
    }
    ...
    for (const auto* A : Args.filtered(OPT_analyzer_config)) {
        ...
        for (const auto& configVal : configVals) {
            ...
            if (val.contains('=')) {
                Diags.Report(SourceLocation(), diag::err_analyzer_config_multiple_values)
                    << configVal;
                break;
            }
...
static StringRef
getStringOption(AnalyzerOptions::ConfigTable& Config, StringRef OptionName, StringRef DefaultVal) {
    return Config.insert({OptionName, std::string(DefaultVal)}).first->second;
}

static void
initOption(AnalyzerOptions::ConfigTable& Config, DiagnosticsEngine* Diags, StringRef& OptionField, StringRef Name, StringRef DefaultVal) {
...
static void
initOption(AnalyzerOptions::ConfigTable& Config, DiagnosticsEngine* Diags, bool& OptionField, StringRef Name, bool DefaultVal) {
...
static void
initOption(AnalyzerOptions::ConfigTable& Config, DiagnosticsEngine* Diags, unsigned& OptionField, StringRef Name, unsigned DefaultVal) {
    OptionField = DefaultVal;
...
static void
parseAnalyzerConfigs(AnalyzerOptions& AnOpts, DiagnosticsEngine* Diags) {
...
#include "clang_include_clang_StaticAnalyzer_Core_AnalyzerOptions.def"
...
#define ANALYZER_OPTION_DEPENDS_ON_USER_MODE(TYPE, NAME, CMDFLAG, DESC, SHALLOW_VAL, DEEP_VAL) \
    initOption(AnOpts.Config, Diags, AnOpts.NAME, CMDFLAG, InShallowMode ? SHALLOW_VAL : DEEP_VAL);
#include "clang_include_clang_StaticAnalyzer_Core_AnalyzerOptions.def"
...
static void
GenerateOptimizationRemark(SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA, OptSpecifier OptEQ, StringRef Name, const CodeGenOptions::OptRemark& Remark) {
...
ParseOptimizationRemark(DiagnosticsEngine& Diags, ArgList& Args, OptSpecifier OptEQ, StringRef Name) {
    ...
    for (Arg* A : Args) {
        if (A->getOption().matches(OPT_R_Joined)) {
            ...
            if (Result.Kind == CodeGenOptions::RK_Disabled || Result.Kind == CodeGenOptions::RK_DisabledEverything) {
                Result.Pattern = "";
...
static bool
parseDiagnosticLevelMask(StringRef FlagName, const std::vector<std::string>& Levels, DiagnosticsEngine& Diags, DiagnosticLevelMask& M) {
    bool Success = true;
...
static void
parseSanitizerKinds(StringRef FlagName, const std::vector<std::string>& Sanitizers, DiagnosticsEngine& Diags, SanitizerSet& S) {
...
static void
parseXRayInstrumentationBundle(StringRef FlagName, StringRef Bundle, ArgList& Args, DiagnosticsEngine& D, XRayInstrSet& S) {
...
static void
setPGOUseInstrumentor(CodeGenOptions& Opts, const Twine& ProfileName, DiagnosticsEngine& Diags) {
    auto ReaderOrErr = llvm::IndexedInstrProfReader::create(ProfileName);
    if (auto E = ReaderOrErr.takeError()) {
        unsigned DiagID = Diags.getCustomDiagID(DiagnosticsEngine::Error, "Error in reading profile %0: %1");
...
#undef CODEGEN_OPTION_WITH_MARSHALLING
    ...
    for (const auto& Prefix : Opts.DebugPrefixMap)
        GenerateArg(Args, OPT_fdebug_prefix_map_EQ, Prefix.first + "=" + Prefix.second, SA);

    for (const auto& Prefix : Opts.CoveragePrefixMap)
        GenerateArg(Args, OPT_fcoverage_prefix_map_EQ, Prefix.first + "=" + Prefix.second, SA);
    ...
    if (Opts.DebugNameTable == static_cast<unsigned>(llvm::DICompileUnit::DebugNameTableKind::GNU))
        GenerateArg(Args, OPT_ggnu_pubnames, SA);
    else if (Opts.DebugNameTable == static_cast<unsigned>(llvm::DICompileUnit::DebugNameTableKind::Default))
    ...
    if (!Opts.MemoryProfileOutput.empty()) {
        if (Opts.MemoryProfileOutput == MemProfileBasename) {
            ...
            GenerateArg(Args, OPT_fmemory_profile_EQ, Opts.MemoryProfileOutput.substr(0, ArgLength), SA);
        }
    }

    if (memcmp(Opts.CoverageVersion, "408*", 4) != 0)
        GenerateArg(Args, OPT_coverage_version_EQ, StringRef(Opts.CoverageVersion, 4), SA);
    ...
    for (const auto& F : Opts.LinkBitcodeFiles) {
        bool Builtint = F.LinkFlags == llvm::Linker::Flags::LinkOnlyNeeded && F.PropagateAttrs && F.Internalize;
        GenerateArg(Args, Builtint ? OPT_mlink_builtin_bitcode : OPT_mlink_bitcode_file, F.Filename, SA);
    }
    ...
    if ((Opts.FPDenormalMode != Opts.FP32DenormalMode) || (Opts.FP32DenormalMode != llvm::DenormalMode::getIEEE()))
        GenerateArg(Args, OPT_fdenormal_fp_math_f32_EQ, Opts.FP32DenormalMode.str(), SA);
    ...
    GenerateOptimizationRemark(Args, SA, OPT_Rpass_EQ, "pass", Opts.OptimizationRemark);

    GenerateOptimizationRemark(Args, SA, OPT_Rpass_missed_EQ, "pass-missed", Opts.OptimizationRemarkMissed);

    GenerateOptimizationRemark(Args, SA, OPT_Rpass_analysis_EQ, "pass-analysis", Opts.OptimizationRemarkAnalysis);

    GenerateArg(Args, OPT_fdiagnostics_hotness_threshold_EQ, Opts.DiagnosticsHotnessThreshold ? Twine(*Opts.DiagnosticsHotnessThreshold) : "auto", SA);

    GenerateArg(Args, OPT_fdiagnostics_misexpect_tolerance_EQ, Twine(*Opts.DiagnosticsMisExpectTolerance), SA);
...
bool
CompilerInvocation::ParseCodeGenArgs(CodeGenOptions& Opts, ArgList& Args, InputKind IK, DiagnosticsEngine& Diags, const llvm::Triple& T, const std::string& OutputFile, const LangOptions& LangOptsRef) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
#undef CODEGEN_OPTION_WITH_MARSHALLING
    ...
    } else if (const Arg* A = Args.getLastArg(options::OPT_finline_functions, options::OPT_finline_hint_functions, options::OPT_fno_inline_functions, options::OPT_fno_inline)) {
    ...
    Opts.DirectAccessExternalData =
        Args.hasArg(OPT_fdirect_access_external_data) || (!Args.hasArg(OPT_fno_direct_access_external_data) && LangOpts->PICLevel == 0);
    ...
    if (const Arg* A =
        ...
        if (A->getOption().matches(OPT_fuse_ctor_homing) && Opts.getDebugInfo() == codegenoptions::LimitedDebugInfo)
            Opts.setDebugInfo(codegenoptions::DebugInfoConstructor);
        if (A->getOption().matches(OPT_fno_use_ctor_homing) && Opts.getDebugInfo() == codegenoptions::DebugInfoConstructor)
    ...
    if (Opts.OptimizationLevel > 0 && Opts.hasReducedDebugInfo() && llvm::is_contained(DebugEntryValueArchs, T.getArch()))
    ...
    Opts.NewStructPathTBAA = !Args.hasArg(OPT_no_struct_path_tbaa) && Args.hasArg(OPT_new_struct_path_tbaa);
    Opts.OptimizeSize = getOptimizationLevelSize(Args);
    Opts.SimplifyLibCalls = !LangOpts->NoBuiltin;
    if (Opts.SimplifyLibCalls)
        Opts.NoBuiltinFuncs = LangOpts->NoBuiltinFuncs;
    Opts.UnrollLoops =
        Args.hasFlag(OPT_funroll_loops, OPT_fno_unroll_loops, (Opts.OptimizationLevel > 1));
    ...
    for (const auto& A : Args) {
        ...
        if (A->getOption().getID() == options::OPT_o || A->getOption().getID() == options::OPT_INPUT || A->getOption().getID() == options::OPT_x || A->getOption().getID() == options::OPT_fembed_bitcode || A->getOption().matches(options::OPT_W_Group))
            continue;
        ArgStringList ASL;
        A->render(Args, ASL);
    ...
    if (XRayInstrBundles.empty())
        ...
        for (const auto& A : XRayInstrBundles)
            parseXRayInstrumentationBundle("-fxray-instrumentation-bundle=", A, Args, Diags, Opts.XRayInstrumentationBundle);
    ...
    if (!Args.getLastArg(OPT_femulated_tls) && !Args.getLastArg(OPT_fno_emulated_tls)) {
    ...
    if (Arg* A =
            Args.getLastArg(OPT_fpcc_struct_return, OPT_freg_struct_return, OPT_maix_struct_return, OPT_msvr4_struct_return)) {
        ...
        if (O.matches(OPT_fpcc_struct_return) || O.matches(OPT_maix_struct_return)) {
            Opts.setStructReturnConvention(CodeGenOptions::SRCK_OnStack);
        } else {
            assert(O.matches(OPT_freg_struct_return) || O.matches(OPT_msvr4_struct_return));
    ...
    NeedLocTracking |= Opts.OptimizationRemark.hasValidPattern() || Opts.OptimizationRemarkMissed.hasValidPattern() || Opts.OptimizationRemarkAnalysis.hasValidPattern();

    bool UsingSampleProfile = !Opts.SampleProfileFile.empty();
    bool UsingProfile = UsingSampleProfile || (Opts.getProfileUse() != CodeGenOptions::ProfileNone);
    ...
    parseSanitizerKinds("-fsanitize-recover=", Args.getAllArgValues(OPT_fsanitize_recover_EQ), Diags, Opts.SanitizeRecover);
    parseSanitizerKinds("-fsanitize-trap=", Args.getAllArgValues(OPT_fsanitize_trap_EQ), Diags, Opts.SanitizeTrap);
...
static void
GenerateDependencyOutputArgs(const DependencyOutputOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
static bool
ParseDependencyOutputArgs(DependencyOutputOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags, frontend::ActionKind Action, bool ShowLineMarkers) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
static bool
parseShowColorsArgs(const ArgList& Args, bool DefaultColor) {
...
static bool
checkVerifyPrefixes(const std::vector<std::string>& VerifyPrefixes, DiagnosticsEngine& Diags) {
    bool Success = true;
...
static void
GenerateFileSystemArgs(const FileSystemOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
static bool
ParseFileSystemArgs(FileSystemOptions& Opts, const ArgList& Args, DiagnosticsEngine& Diags) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
static void
GenerateMigratorArgs(const MigratorOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
static bool
ParseMigratorArgs(MigratorOptions& Opts, const ArgList& Args, DiagnosticsEngine& Diags) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
void
CompilerInvocation::GenerateDiagnosticArgs(
...
#undef DIAG_OPTION_WITH_MARSHALLING
    ...
    if (!Opts.DiagnosticSerializationFile.empty())
        GenerateArg(Args, OPT_diagnostic_serialized_file, Opts.DiagnosticSerializationFile, SA);

    if (Opts.ShowColors)
        GenerateArg(Args, OPT_fcolor_diagnostics, SA);

    if (Opts.VerifyDiagnostics && llvm::is_contained(Opts.VerifyPrefixes, "expected"))
    ...
    for (const auto& Remark : Opts.Remarks) {
        ...
        // CodeGenOptions.
        StringRef IgnoredRemarks[] = {"pass", "no-pass", "pass-analysis", "no-pass-analysis", "pass-missed", "no-pass-missed"};
...
bool
clang::ParseDiagnosticArgs(DiagnosticOptions& Opts, ArgList& Args, DiagnosticsEngine* Diags, bool DefaultDiagColor) {
    std::optional<DiagnosticsEngine> IgnoringDiags;
    if (!Diags) {
        IgnoringDiags.emplace(new DiagnosticIDs(), new DiagnosticOptions(), new IgnoringDiagConsumer());
        Diags = &*IgnoringDiags;
    }
...
static bool
parseTestModuleFileExtensionArg(StringRef Arg, std::string& BlockName, unsigned& MajorVersion, unsigned& MinorVersion, bool& Hashed, std::string& UserInfo) {
...
static Optional<frontend::ActionKind>
...
static void
GenerateFrontendArgs(const FrontendOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA, bool IsHeader) {
...
#undef FRONTEND_OPTION_WITH_MARSHALLING
    ...
    if (!ProgramActionOpt) {
        ...
        assert(Opts.ProgramAction == frontend::PluginAction && "Frontend action without option.");
    ...
    if (!Opts.CodeCompletionAt.FileName.empty())
        GenerateArg(Args, OPT_code_completion_at, Opts.CodeCompletionAt.ToString(), SA);
...

    {
        ...
        switch (Opts.DashX.getLanguage()) {
            ...
            case Language::Unknown:
                assert(Opts.DashX.getFormat() == InputKind::Precompiled && "Generating -x argument for unknown language (not precompiled).");
                Lang = "ast";
                break;
        ...
        GenerateArg(Args, OPT_x, Lang + HeaderUnit + Header + ModuleMap + Preprocessed, SA);
    }
...
static bool
ParseFrontendArgs(FrontendOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags, bool& IsHeaderFile) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
#undef FRONTEND_OPTION_WITH_MARSHALLING
        ...
        if (ProgramAction == frontend::GenerateInterfaceStubs) {
            ...
            if (ArgStr == "experimental-yaml-elf-v1" || ArgStr == "experimental-ifs-v1" || ArgStr == "experimental-ifs-v2" || ArgStr == "experimental-tapi-elf-v1") {
                std::string ErrorMessage =
                    "Invalid interface stub format: " + ArgStr.str() + " is deprecated.";
                Diags.Report(diag::err_drv_invalid_value)
    ...
    if (const Arg* A = Args.getLastArg(OPT_x)) {
        ...
        IsHeaderFile = IsHeader && !Preprocessed && !ModuleMap && HUK == InputKind::HeaderUnit_None;
        ...
        if (DashX.isUnknown() && Preprocessed && !IsHeaderFile && !ModuleMap && HUK == InputKind::HeaderUnit_None)
            DashX = llvm::StringSwitch<InputKind>(XValue)
        ...
        if (DashX.isUnknown() && !Preprocessed && !IsHeaderFile && !ModuleMap && HUK == InputKind::HeaderUnit_None)
            DashX = llvm::StringSwitch<InputKind>(XValue)
                        ...
                        .Cases("ast", "pcm", "precompiled-header", InputKind(Language::Unknown, InputKind::Precompiled))
    ...
    for (unsigned i = 0, e = Inputs.size(); i != e; ++i) {
        ...
        if (Opts.ProgramAction == frontend::GenerateModule && IK.getFormat() == InputKind::Source) {
...
std::string
CompilerInvocation::GetResourcesPath(const char* Argv0, void* MainAddr) {
...
static void
GenerateHeaderSearchArgs(HeaderSearchOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
#undef HEADER_SEARCH_OPTION_WITH_MARSHALLING
    ...
    auto Matches = [](const HeaderSearchOptions::Entry&         Entry,
        ...
        return llvm::is_contained(Groups, Entry.Group) && (!IsFramework || (Entry.IsFramework == *IsFramework)) && (!IgnoreSysRoot || (Entry.IgnoreSysRoot == *IgnoreSysRoot));
    };
    ...
    for (; It < End && Matches(*It, {frontend::IndexHeaderMap, frontend::Angled}, std::nullopt, true);
         ++It) {
    ...
    // include paths.
    for (; It < End && Matches(*It, {frontend::After, frontend::Angled}, false, true);
         ++It) {
        OptSpecifier Opt =
    ...
    for (; It < End && Matches(*It, {frontend::System}, false, std::nullopt);
         ++It)
        GenerateArg(Args, It->IgnoreSysRoot ? OPT_isystem : OPT_iwithsysroot, It->Path, SA);
    ...
    // command line was such that this has no semantic impact on include paths.
    for (; It < End && Matches(*It, {frontend::System, frontend::ExternCSystem}, false, true);
         ++It) {
...
static bool
ParseHeaderSearchArgs(HeaderSearchOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags, const std::string& WorkingDir) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
#undef HEADER_SEARCH_OPTION_WITH_MARSHALLING
    ...
    for (const auto* A : Args.filtered(OPT_I, OPT_F, OPT_index_header_map)) {
        ...
        if (IsSysrootSpecified && !IsFramework && A->getValue()[0] == '=') {
            ...
            llvm::sys::path::append(Buffer, Opts.Sysroot, llvm::StringRef(A->getValue()).substr(1));
    ...
    for (const auto* A : Args.filtered(OPT_isystem, OPT_iwithsysroot))
        Opts.AddPath(A->getValue(), frontend::System, false, !A->getOption().matches(OPT_iwithsysroot));
    ...
    for (const auto* A :
        ...
        Opts.AddSystemHeaderPrefix(
            A->getValue(),
            A->getOption().matches(OPT_system_header_prefix)
        );
...
static bool
IsInputCompatibleWithStandard(InputKind IK, const LangStandard& S) {
    switch (IK.getLanguage()) {
        ...
        case Language::OpenCL:
            return S.getLanguage() == Language::OpenCL || S.getLanguage() == Language::OpenCLCXX;
        ...
        case Language::CUDA:
            ...
            return S.getLanguage() == Language::CUDA || S.getLanguage() == Language::CXX;
...
void
CompilerInvocation::GenerateLangArgs(const LangOptions& Opts, SmallVectorImpl<const char*>& Args, StringAllocator SA, const llvm::Triple& T, InputKind IK) {
    if (IK.getFormat() == InputKind::Precompiled || IK.getLanguage() == Language::LLVM_IR) {
...
#undef LANG_OPTION_WITH_MARSHALLING
    ...
    if (Opts.GNUCVersion != 0) {
        ...
        GenerateArg(Args, OPT_fgnuc_version_EQ, Twine(Major) + "." + Twine(Minor) + "." + Twine(Patch), SA);
    }
    ...
    if (Opts.MSCompatibilityVersion != 0) {
        ...
        GenerateArg(Args, OPT_fms_compatibility_version, Twine(Major) + "." + Twine(Minor) + "." + Twine(Subminor), SA);
    }
    ...
    if (Opts.ConvergentFunctions && !(Opts.OpenCL || (Opts.CUDA && Opts.CUDAIsDevice) || Opts.SYCLIsDevice))
    ...
    if (Opts.OpenMPTargetDebug != 0)
        GenerateArg(Args, OPT_fopenmp_target_debug_EQ, Twine(Opts.OpenMPTargetDebug), SA);

    if (Opts.OpenMPCUDANumSMs != 0)
        GenerateArg(Args, OPT_fopenmp_cuda_number_of_sm_EQ, Twine(Opts.OpenMPCUDANumSMs), SA);

    if (Opts.OpenMPCUDABlocksPerSM != 0)
        GenerateArg(Args, OPT_fopenmp_cuda_blocks_per_sm_EQ, Twine(Opts.OpenMPCUDABlocksPerSM), SA);

    if (Opts.OpenMPCUDAReductionBufNum != 1024)
        GenerateArg(Args, OPT_fopenmp_cuda_teams_reduction_recs_num_EQ, Twine(Opts.OpenMPCUDAReductionBufNum), SA);

    if (!Opts.OMPTargetTriples.empty()) {
        std::string              Targets;
        llvm::raw_string_ostream OS(Targets);
        llvm::interleave(
            Opts.OMPTargetTriples,
            OS,
    ...
    if (Opts.getSignReturnAddressScope() == LangOptions::SignReturnAddressScopeKind::All)
        GenerateArg(Args, OPT_msign_return_address_EQ, "all", SA);
    else if (Opts.getSignReturnAddressScope() == LangOptions::SignReturnAddressScopeKind::NonLeaf)
        GenerateArg(Args, OPT_msign_return_address_EQ, "non-leaf", SA);

    if (Opts.getSignReturnAddressKey() == LangOptions::SignReturnAddressKeyKind::BKey)
...
bool
CompilerInvocation::ParseLangArgs(LangOptions& Opts, ArgList& Args, InputKind IK, const llvm::Triple& T, std::vector<std::string>& Includes, DiagnosticsEngine& Diags) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
    ...
    if (IK.getFormat() == InputKind::Precompiled || IK.getLanguage() == Language::LLVM_IR) {
        ...
        parseSanitizerKinds("-fsanitize=", Args.getAllArgValues(OPT_fsanitize_EQ), Diags, Opts.Sanitize);
...
#include "clang_include_clang_Basic_LangStandards.def"
    ...
    if (const Arg* A = Args.getLastArg(OPT_cl_std_EQ)) {
        LangStandard::Kind OpenCLLangStd = llvm::StringSwitch<LangStandard::Kind>(A->getValue())
...
#undef LANG_OPTION_WITH_MARSHALLING
    ...
    if ((Args.hasArg(OPT_fsycl_is_device) || Args.hasArg(OPT_fsycl_is_host)) && !Args.hasArg(OPT_sycl_std_EQ)) {
    ...
    if (const Arg* A = Args.getLastArg(OPT_fms_compatibility_version)) {
        ...
        if (VT.tryParse(A->getValue()))
            Diags.Report(diag::err_drv_invalid_value) << A->getAsString(Args)
                                                      << A->getValue();
        Opts.MSCompatibilityVersion = VT.getMajor() * 10000000 + VT.getMinor().value_or(0) * 100000 + VT.getSubminor().value_or(0);
    }
    ...
    Opts.Blocks = Args.hasArg(OPT_fblocks) || (Opts.OpenCL && Opts.OpenCLVersion == 200);

    Opts.ConvergentFunctions = Opts.OpenCL || (Opts.CUDA && Opts.CUDAIsDevice) || Opts.SYCLIsDevice || Args.hasArg(OPT_fconvergent_functions);
    ...
    if (Opts.OpenMP || Opts.OpenMPSimd) {
        if (int Version = getLastArgIntValue(
                ...
                (IsSimdSpecified || IsTargetSpecified) ? 50 : Opts.OpenMP,
                Diags
            ))
            Opts.OpenMP = Version;
    ...
    if ((Opts.OpenMPIsDevice && (T.isNVPTX() || T.isAMDGCN())) || Opts.OpenCLCPlusPlus) {
        Opts.Exceptions = 0;
        Opts.CXXExceptions = 0;
    }
    if (Opts.OpenMPIsDevice && T.isNVPTX()) {
        Opts.OpenMPCUDANumSMs =
            getLastArgIntValue(Args, options::OPT_fopenmp_cuda_number_of_sm_EQ, Opts.OpenMPCUDANumSMs, Diags);
        Opts.OpenMPCUDABlocksPerSM =
            getLastArgIntValue(Args, options::OPT_fopenmp_cuda_blocks_per_sm_EQ, Opts.OpenMPCUDABlocksPerSM, Diags);
        Opts.OpenMPCUDAReductionBufNum = getLastArgIntValue(
            Args,
            options::OPT_fopenmp_cuda_teams_reduction_recs_num_EQ,
            Opts.OpenMPCUDAReductionBufNum,
            Diags
        );
    }
    ...
    if (Opts.OpenMPIsDevice && (Args.hasArg(OPT_fopenmp_target_debug) || Args.hasArg(OPT_fopenmp_target_debug_EQ))) {
        Opts.OpenMPTargetDebug = getLastArgIntValue(
            Args,
            OPT_fopenmp_target_debug_EQ,
            Opts.OpenMPTargetDebug,
            Diags
        );
    ...
    Opts.OpenMPCUDAMode = Opts.OpenMPIsDevice && (T.isNVPTX() || T.isAMDGCN()) && Args.hasArg(options::OPT_fopenmp_cuda_mode);
    ...
    Opts.NoSanitizeFiles.insert(Opts.NoSanitizeFiles.end(), systemIgnorelists.begin(), systemIgnorelists.end());
    ...
    Opts.RelativeCXXABIVTables =
        Args.hasFlag(options::OPT_fexperimental_relative_cxx_abi_vtables, options::OPT_fno_experimental_relative_cxx_abi_vtables, TargetCXXABI::usesRelativeVTables(T));
    ...
    Opts.UseTargetPathSeparator =
        !Args.getLastArg(OPT_fno_file_reproducible) && (Args.getLastArg(OPT_ffile_compilation_dir_EQ) || Args.getLastArg(OPT_fmacro_prefix_map_EQ) || Args.getLastArg(OPT_ffile_reproducible));
    ...
    if (Opts.HLSL) {
        bool SupportedTarget = T.getArch() == llvm::Triple::dxil && T.getOS() == llvm::Triple::ShaderModel;
        if (!SupportedTarget)
...
static void
GeneratePreprocessorArgs(PreprocessorOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA, const LangOptions& LangOpts, const FrontendOptions& FrontendOpts, const CodeGenOptions& CodeGenOpts) {
...
#undef PREPROCESSOR_OPTION_WITH_MARSHALLING
    ...
    if (Opts.PrecompiledPreambleBytes != std::make_pair(0u, false))
        GenerateArg(Args, OPT_preamble_bytes_EQ, Twine(Opts.PrecompiledPreambleBytes.first) + "," + (Opts.PrecompiledPreambleBytes.second ? "1" : "0"), SA);
    ...
    for (const auto& M : Opts.Macros) {
        ...
        if (M.first == "__CET__=1" && !M.second && !CodeGenOpts.CFProtectionReturn && CodeGenOpts.CFProtectionBranch)
            continue;
        if (M.first == "__CET__=2" && !M.second && CodeGenOpts.CFProtectionReturn && !CodeGenOpts.CFProtectionBranch)
            continue;
        if (M.first == "__CET__=3" && !M.second && CodeGenOpts.CFProtectionReturn && CodeGenOpts.CFProtectionBranch)
            continue;
    ...
    for (const auto& I : Opts.Includes) {
        ...
        // generated elsewhere.
        if (LangOpts.OpenCL && LangOpts.IncludeDefaultHeader && ((LangOpts.DeclareOpenCLBuiltins && I == "opencl-c-base.h") || I == "opencl-c.h"))
            continue;
...
static bool
ParsePreprocessorArgs(PreprocessorOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags, frontend::ActionKind Action, const FrontendOptions& FrontendOpts) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
static bool
ParsePreprocessorOutputArgs(PreprocessorOutputOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags, frontend::ActionKind Action) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
static void
GenerateTargetArgs(const TargetOptions& Opts, SmallVectorImpl<const char*>& Args, CompilerInvocation::StringAllocator SA) {
...
#undef TARGET_OPTION_WITH_MARSHALLING
    ...
    if (!Opts.SDKVersion.empty())
        GenerateArg(Args, OPT_target_sdk_version_EQ, Opts.SDKVersion.getAsString(), SA);
    if (!Opts.DarwinTargetVariantSDKVersion.empty())
        GenerateArg(Args, OPT_darwin_target_variant_sdk_version_EQ, Opts.DarwinTargetVariantSDKVersion.getAsString(), SA);
}

static bool
ParseTargetArgs(TargetOptions& Opts, ArgList& Args, DiagnosticsEngine& Diags) {
    unsigned NumErrorsBefore = Diags.getNumErrors();
...
#include "clang_include_clang_Basic_LangOptions.def"
    ...
    HBuilder.add(TargetOpts->Triple, TargetOpts->CPU, TargetOpts->TuneCPU, TargetOpts->ABI);
    ...
    HBuilder.add(hsOpts.Sysroot, hsOpts.ModuleFormat, hsOpts.UseDebugInfo, hsOpts.UseBuiltinIncludes, hsOpts.UseStandardSystemIncludes, hsOpts.UseStandardCXXIncludes, hsOpts.UseLibcxx, hsOpts.ModulesValidateDiagnosticOptions);
...
#include "clang_include_clang_Basic_DiagnosticOptions.def"
#undef DIAGOPT
#undef ENUM_DIAGOPT
    }
...
void
CompilerInvocation::generateCC1CommandLine(
    SmallVectorImpl<const char*>& Args,
    StringAllocator               SA
) const {
    llvm::Triple T(TargetOpts->Triple);
    ...
    GenerateCodeGenArgs(CodeGenOpts, Args, SA, T, FrontendOpts.OutputFile, &*LangOpts);
    GeneratePreprocessorArgs(*PreprocessorOpts, Args, SA, *LangOpts, FrontendOpts, CodeGenOpts);
    GeneratePreprocessorOutputArgs(PreprocessorOutputOpts, Args, SA, FrontendOpts.ProgramAction);
...
void
CompilerInvocation::resetNonModularOptions() {
...
clang::createVFSFromCompilerInvocation(const CompilerInvocation& CI, DiagnosticsEngine& Diags) {
    return createVFSFromCompilerInvocation(CI, Diags, llvm::vfs::getRealFileSystem());
}
...
clang::createVFSFromCompilerInvocation(
    ...
    return createVFSFromOverlayFiles(CI.getHeaderSearchOpts().VFSOverlayFiles, Diags, std::move(BaseFS));
}
