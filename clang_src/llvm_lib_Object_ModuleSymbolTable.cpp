...
void
ModuleSymbolTable::addModule(Module* M) {
    if (FirstMod)
...
static void
initializeRecordStreamer(const Module& M, function_ref<void(RecordStreamer&)> Init) {
...
void
ModuleSymbolTable::printSymbolName(raw_ostream& OS, Symbol S) const {
    if (S.is<AsmSymbol*>()) {
...
uint32_t
ModuleSymbolTable::getSymbolFlags(Symbol S) const {
    if (S.is<AsmSymbol*>())
    ...
    if (GV->hasLinkOnceLinkage() || GV->hasWeakLinkage() || GV->hasExternalWeakLinkage())
        Res |= BasicSymbolRef::SF_Weak;
