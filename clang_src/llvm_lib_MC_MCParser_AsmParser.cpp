...
#include "llvm_include_llvm_ADT_APFloat.h"
...
#include "llvm_include_llvm_DebugInfo_CodeView_SymbolRecord.h"
#include "llvm_include_llvm_MC_MCAsmInfo.h"
#include "llvm_include_llvm_MC_MCCodeView.h"
#include "llvm_include_llvm_MC_MCContext.h"
#include "llvm_include_llvm_MC_MCDirectives.h"
...
#include "llvm_include_llvm_MC_MCParser_AsmCond.h"
#include "llvm_include_llvm_MC_MCParser_AsmLexer.h"
#include "llvm_include_llvm_MC_MCParser_MCAsmLexer.h"
#include "llvm_include_llvm_MC_MCParser_MCAsmParser.h"
#include "llvm_include_llvm_MC_MCParser_MCAsmParserExtension.h"
#include "llvm_include_llvm_MC_MCParser_MCAsmParserUtils.h"
...
#include "llvm_include_llvm_Support_MD5.h"
...
class AsmParser: public MCAsmParser {
  ...
  protected:
    ...
    bool parseAndMatchAndEmitTargetInstruction(ParseStatementInfo& Info, StringRef IDVal, AsmToken ID, SMLoc IDLoc);
  ...
  public:
    AsmParser(SourceMgr& SM, MCContext& Ctx, MCStreamer& Out, const MCAsmInfo& MAI, unsigned CB);
    ...
    void
    addDirectiveHandler(StringRef Directive, ExtensionDirectiveHandler Handler) override {
        ExtensionDirectiveMap[Directive] = Handler;
    }

    void
    addAliasForDirective(StringRef Directive, StringRef Alias) override {
    ...
    void
    setAssemblerDialect(unsigned i) override {
        AssemblerDialect = i;
    }
    ...
    void
    setParsingMSInlineAsm(bool V) override {
    ...
    bool
    isParsingMSInlineAsm() override {
        return ParsingMSInlineAsm;
    }

    bool
    discardLTOSymbol(StringRef Name) const override {
        return LTODiscardSymbols.contains(Name);
    }

    bool parseMSInlineAsm(std::string& AsmString, unsigned& NumOutputs, unsigned& NumInputs, SmallVectorImpl<std::pair<void*, bool>>& OpDecls, SmallVectorImpl<std::string>& Constraints, SmallVectorImpl<std::string>& Clobbers, const MCInstrInfo* MII, const MCInstPrinter* IP, MCAsmParserSemaCallback& SI) override;

    bool parseExpression(const MCExpr*& Res);
    bool parseExpression(const MCExpr*& Res, SMLoc& EndLoc) override;
    bool parsePrimaryExpr(const MCExpr*& Res, SMLoc& EndLoc, AsmTypeInfo* TypeInfo) override;
    bool parseParenExpression(const MCExpr*& Res, SMLoc& EndLoc) override;
    bool parseParenExprOfDepth(unsigned ParenDepth, const MCExpr*& Res, SMLoc& EndLoc) override;
  ...
  private:
    ...
    void checkForBadMacro(SMLoc DirectiveLoc, StringRef Name, StringRef Body, ArrayRef<MCAsmMacroParameter> Parameters);
    bool expandMacro(raw_svector_ostream& OS, StringRef Body, ArrayRef<MCAsmMacroParameter> Parameters, ArrayRef<MCAsmMacroArgument> A, bool EnableAtPseudoVariable, SMLoc L);
    ...
    void
    printMessage(SMLoc Loc, SourceMgr::DiagKind Kind, const Twine& Msg, SMRange Range = std::nullopt) const {
    ...
    bool processIncbinFile(const std::string& Filename, int64_t Skip = 0, const MCExpr* Count = nullptr, SMLoc Loc = SMLoc());
    ...
    unsigned getBinOpPrecedence(AsmToken::TokenKind K, MCBinaryExpr::Opcode& Kind);
    ...
    const MCExpr* applyModifierToExpr(const MCExpr* E, MCSymbolRefExpr::VariantKind Variant);

    // Macro-like directives
    MCAsmMacro* parseMacroLikeBody(SMLoc DirectiveLoc);
    void        instantiateMacroLikeBody(MCAsmMacro* M, SMLoc DirectiveLoc, raw_svector_ostream& OS);
    ...
    // "_emit" or "__emit"
    bool parseDirectiveMSEmit(SMLoc DirectiveLoc, ParseStatementInfo& Info, size_t Len);
...
class HLASMAsmParser final: public AsmParser {
  private:
    ...
    bool parseAsMachineInstruction(ParseStatementInfo& Info, MCAsmParserSemaCallback* SI);

  public:
    HLASMAsmParser(SourceMgr& SM, MCContext& Ctx, MCStreamer& Out, const MCAsmInfo& MAI, unsigned CB = 0) :
...
AsmParser::AsmParser(SourceMgr& SM, MCContext& Ctx, MCStreamer& Out, const MCAsmInfo& MAI, unsigned CB = 0) :
    Lexer(MAI),
    Ctx(Ctx),
    Out(Out),
    MAI(MAI),
...
AsmParser::~AsmParser() {
    assert((HadError || ActiveMacros.empty()) && "Unexpected active macro instantiation!");
...
AsmParser::printMacroInstantiations() {
    ...
    for (std::vector<MacroInstantiation*>::const_reverse_iterator
         ...
         it != ie;
         ++it)
        printMessage((*it)->InstantiationLoc, SourceMgr::DK_Note, "while in macro instantiation");
}

void
AsmParser::Note(SMLoc L, const Twine& Msg, SMRange Range) {
    printPendingErrors();
...
bool
AsmParser::Warning(SMLoc L, const Twine& Msg, SMRange Range) {
...
bool
AsmParser::printError(SMLoc L, const Twine& Msg, SMRange Range) {
    HadError = true;
...
bool
AsmParser::enterIncludeFile(const std::string& Filename) {
...
bool
AsmParser::processIncbinFile(const std::string& Filename, int64_t Skip, const MCExpr* Count, SMLoc Loc) {
    std::string IncludedFile;
    unsigned    NewBuf =
        SrcMgr.AddIncludeFile(Filename, Lexer.getLoc(), IncludedFile);
    if (!NewBuf)
...
void
AsmParser::jumpToLoc(SMLoc Loc, unsigned InBuffer) {
    CurBuffer = InBuffer ? InBuffer : SrcMgr.FindBufferContainingLoc(Loc);
    Lexer.setBuffer(SrcMgr.getMemoryBuffer(CurBuffer)->getBuffer(), Loc.getPointer());
}
...
bool
AsmParser::enabledGenDwarfForAssembly() {
...
bool
AsmParser::Run(bool NoInitialTextSection, bool NoFinalize) {
...
AsmParser::checkForValidSection() {
    if (!ParsingMSInlineAsm && !getStreamer().getCurrentSectionOnly()) {
        Out.initSections(false, getTargetParser().getSTI());
        return Error(getTok().getLoc(), "expected section directive before assembly directive");
    }
    return false;
}
...
AsmParser::parseStringToComma() {
    ...
    while (Lexer.isNot(AsmToken::EndOfStatement) && Lexer.isNot(AsmToken::Comma) && Lexer.isNot(AsmToken::Eof))
        Lexer.Lex();
...
///
bool
AsmParser::parseParenExpr(const MCExpr*& Res, SMLoc& EndLoc) {
...
///
bool
AsmParser::parseBracketExpr(const MCExpr*& Res, SMLoc& EndLoc) {
    if (parseExpression(Res))
        return true;
...
bool
AsmParser::parsePrimaryExpr(const MCExpr*& Res, SMLoc& EndLoc, AsmTypeInfo* TypeInfo) {
        ...
        case AsmToken::Identifier: {
            ...
            if (parseIdentifier(Identifier)) {
                ...
                if (getTok().is(AsmToken::Dollar) || getTok().is(AsmToken::Star)) {
                    ...
                    if ((getTok().is(AsmToken::Dollar) && MAI.getDollarIsPC()) || (getTok().is(AsmToken::Star) && MAI.getStarIsPC()))
            ...
            if (!Split.second.empty()) {
                ...
                if (Variant != MCSymbolRefExpr::VK_Invalid) {
                    ...
                    return Error(SMLoc::getFromPointer(Split.second.begin()), "invalid variant '" + Split.second + "'");
                }
            }
...
bool
AsmParser::parseExpression(const MCExpr*& Res) {
    SMLoc EndLoc;
    return parseExpression(Res, EndLoc);
}

const MCExpr*
AsmParser::applyModifierToExpr(const MCExpr* E, MCSymbolRefExpr::VariantKind Variant) {
    ...
    switch (E->getKind()) {
        ...
        case MCExpr::SymbolRef: {
            ...
            if (SRE->getKind() != MCSymbolRefExpr::VK_None) {
                TokError("invalid variant on expression '" + getTok().getIdentifier() + "' (already modified)");
                return E;
            }
...
static bool
isAngleBracketString(SMLoc& StrLoc, SMLoc& EndLoc) {
    assert((StrLoc.getPointer() != nullptr) && "Argument to the function cannot be a NULL value");
...
///
bool
AsmParser::parseExpression(const MCExpr*& Res, SMLoc& EndLoc) {
    // Parse the expression.
    Res = nullptr;
    if (getTargetParser().parsePrimaryExpr(Res, EndLoc) || parseBinOpRHS(1, Res, EndLoc))
        return true;
    ...
    if (Lexer.getKind() == AsmToken::At) {
        ...
        if (!ModifiedRes) {
            return TokError("invalid modifier '" + getTok().getIdentifier() + "' (no symbols present)");
        }
...
bool
AsmParser::parseParenExpression(const MCExpr*& Res, SMLoc& EndLoc) {
    Res = nullptr;
    return parseParenExpr(Res, EndLoc) || parseBinOpRHS(1, Res, EndLoc);
}

bool
AsmParser::parseParenExprOfDepth(unsigned ParenDepth, const MCExpr*& Res, SMLoc& EndLoc) {
...
bool
AsmParser::parseAbsoluteExpression(int64_t& Res) {
    const MCExpr* Expr;
...
static unsigned
getDarwinBinOpPrecedence(AsmToken::TokenKind K, MCBinaryExpr::Opcode& Kind, bool ShouldUseLogicalShr) {
    switch (K) {
        default:
...
static unsigned
getGNUBinOpPrecedence(const MCAsmInfo& MAI, AsmToken::TokenKind K, MCBinaryExpr::Opcode& Kind, bool ShouldUseLogicalShr) {
    switch (K) {
        default:
            return 0;  // not a binop.
...
unsigned
AsmParser::getBinOpPrecedence(AsmToken::TokenKind K, MCBinaryExpr::Opcode& Kind) {
...
bool
AsmParser::parseBinOpRHS(unsigned Precedence, const MCExpr*& Res, SMLoc& EndLoc) {
    SMLoc StartLoc = Lexer.getLoc();
    while (true) {
...
bool
AsmParser::parseStatement(ParseStatementInfo& Info, MCAsmParserSemaCallback* SI) {
    ...
    if (Lexer.is(AsmToken::EndOfStatement)) {
        ...
        if (getTok().getString().empty() || getTok().getString().front() == '\r' || getTok().getString().front() == '\n')
            Out.addBlankLine();
        Lex();
        return false;
    }
    ...
    if (Lexer.is(AsmToken::HashDirective))
        return parseCppHashLineFilenameComment(IDLoc, !isInsideMacroInstantiation());
    ...
    if (Lexer.is(AsmToken::Integer)) {
        ...
        Lex();
        IDVal = "}";
    } else if (Lexer.is(AsmToken::Star) && getTargetParser().starIsStartOfStatement()) {
        ...
        case AsmToken::Colon: {
            ...
            if (LocalLabelVal == -1) {
                if (ParsingMSInlineAsm && SI) {
                    ...
                    assert(!RewrittenLabel.empty() && "We should have an internal name here.");
                    Info.AsmRewrites->emplace_back(AOK_Label, IDLoc, IDVal.size(), RewrittenLabel);
            ...
            if (enabledGenDwarfForAssembly())
                MCGenDwarfLabelEntry::Make(Sym, &getStreamer(), getSourceManager(), IDLoc);
    ...
    if (IDVal.startswith(".") && IDVal != ".") {
        ...
        switch (DirKind) {
            ...
            case DK_CODE16:
            case DK_CODE16GCC:
                return TokError(Twine(IDVal) + " not currently supported for this target");
            case DK_REPT:
    ...
    if (ParsingMSInlineAsm && (IDVal == "_emit" || IDVal == "__emit" || IDVal == "_EMIT" || IDVal == "__EMIT"))
...
bool
AsmParser::parseAndMatchAndEmitTargetInstruction(ParseStatementInfo& Info, StringRef IDVal, AsmToken ID, SMLoc IDLoc) {
    // Canonicalize the opcode to lower case.
    std::string          OpcodeStr = IDVal.lower();
    ParseInstructionInfo IInfo(Info.AsmRewrites);
    bool                 ParseHadError = getTargetParser().ParseInstruction(IInfo, OpcodeStr, ID, Info.ParsedOperands);
    ...
    if (!ParseHadError && enabledGenDwarfForAssembly() && getContext().getGenDwarfSectionSyms().count(getStreamer().getCurrentSectionOnly())) {
        unsigned Line;
        if (ActiveMacros.empty())
            Line = SrcMgr.FindLineNumber(IDLoc, CurBuffer);
        else
            Line = SrcMgr.FindLineNumber(ActiveMacros.front()->InstantiationLoc, ActiveMacros.front()->ExitBuffer);
...
AsmParser::parseCurlyBlockScope(SmallVectorImpl<AsmRewrite>& AsmStrRewrites) {
    ...
    AsmStrRewrites.emplace_back(AOK_Skip, StartLoc, Lexer.getLoc().getPointer() - StartLoc.getPointer());
    return true;
}
...
///   ::= # number "filename"
bool
AsmParser::parseCppHashLineFilenameComment(SMLoc L, bool SaveLocInfo) {
    Lex();  // Eat the hash token.
    // Lexer only ever emits HashDirective if it fully formed if it's
    // done the checking already so this is an internal error.
    assert(getTok().is(AsmToken::Integer) && "Lexing Cpp line comment: Expected Integer");
    int64_t LineNumber = getTok().getIntVal();
    Lex();
    assert(getTok().is(AsmToken::String) && "Lexing Cpp line comment: Expected String");
...
void
AsmParser::DiagHandler(const SMDiagnostic& Diag, void* Context) {
    ...
    if (!Parser->SavedDiagHandler && DiagCurBuffer && DiagCurBuffer != DiagSrcMgr.getMainFileID()) {
    ...
    SMDiagnostic NewDiag(*Diag.getSourceMgr(), Diag.getLoc(), Filename, LineNo, Diag.getColumnNo(), Diag.getKind(), Diag.getMessage(), Diag.getLineContents(), Diag.getRanges());
...
bool
AsmParser::expandMacro(raw_svector_ostream& OS, StringRef Body, ArrayRef<MCAsmMacroParameter> Parameters, ArrayRef<MCAsmMacroArgument> A, bool EnableAtPseudoVariable, SMLoc L) {
            ...
            if (Argument == "@") {
                ...
                if (Index == NParameters) {
                    ...
                    for (const AsmToken& Token : A[Index])
                        ...
                        // representation.
                        if (AltMacroMode && Token.getString().front() == '%' && Token.is(AsmToken::Integer))
                            // Emit an integer value to the buffer.
                            OS << Token.getIntVal();
                        // Only Token that was validated as a string and begins with '<'
                        // is considered altMacroString!!!
                        else if (AltMacroMode && Token.getString().front() == '<' && Token.is(AsmToken::String)) {
...
static bool
isOperator(AsmToken::TokenKind kind) {
    switch (kind) {
        default:
            return false;
        case AsmToken::Plus:
...
bool
AsmParser::parseMacroArgument(MCAsmMacroArgument& MA, bool Vararg) {
    if (Vararg) {
...
bool
AsmParser::parseMacroArguments(const MCAsmMacro* M, MCAsmMacroArguments& A) {
    ...
    for (unsigned Parameter = 0; !NParameters || Parameter < NParameters;
        ...
        if (AltMacroMode && Lexer.is(AsmToken::Percent)) {
            ...
            if (parseExpression(AbsoluteExp, EndLoc))
                return false;
            if (!AbsoluteExp->evaluateAsAbsolute(Value, getStreamer().getAssemblerPtr()))
                return Error(StrLoc, "expected absolute expression");
            const char* StrChar = StrLoc.getPointer();
            const char* EndChar = EndLoc.getPointer();
            AsmToken    newToken(AsmToken::Integer, StringRef(StrChar, EndChar - StrChar), Value);
            FA.Value.push_back(newToken);
        } else if (AltMacroMode && Lexer.is(AsmToken::Less) && isAngleBracketString(StrLoc, EndLoc)) {
            const char* StrChar = StrLoc.getPointer();
            const char* EndChar = EndLoc.getPointer();
        ...
        if (!FA.Name.empty()) {
            ...
            if (FAI >= NParameters) {
                ...
                return Error(IDLoc, "parameter named '" + FA.Name + "' does not exist for macro '" + M->Name + "'");
            }
            PI = FAI;
        }
...
bool
AsmParser::handleMacroEntry(const MCAsmMacro* M, SMLoc NameLoc) {
...
bool
AsmParser::parseAssignment(StringRef Name, AssignmentKind Kind) {
    MCSymbol*     Sym;
    const MCExpr* Value;
    SMLoc         ExprLoc = getTok().getLoc();
    bool          AllowRedef =
        Kind == AssignmentKind::Set || Kind == AssignmentKind::Equal;
    if (MCParserUtils::parseAssignmentExpression(Name, AllowRedef, *this, Sym, Value))
        return true;
...
///   ::= string
bool
AsmParser::parseIdentifier(StringRef& Res) {
...
bool
AsmParser::parseDirectiveSet(StringRef IDVal, AssignmentKind Kind) {
    StringRef Name;
    if (check(parseIdentifier(Name), "expected identifier") || parseComma() || parseAssignment(Name, Kind))
        return true;
    return false;
}

bool
AsmParser::parseEscapedString(std::string& Data) {
...
bool
AsmParser::parseAngleBracketString(std::string& Data) {
...
bool
AsmParser::parseDirectiveAscii(StringRef IDVal, bool ZeroTerminated) {
    auto parseOp = [&]() -> bool {
        std::string Data;
        if (checkForValidSection())
            return true;
...
bool
AsmParser::parseDirectiveReloc(SMLoc DirectiveLoc) {
    const MCExpr* Offset;
    ...
    if (parseExpression(Offset))
        return true;
    if (parseComma() || check(getTok().isNot(AsmToken::Identifier), "expected relocation name"))
        return true;
    ...
    if (std::optional<std::pair<bool, std::string>> Err =
            getStreamer().emitRelocDirective(*Offset, Name, Expr, DirectiveLoc, STI))
...
bool
AsmParser::parseDirectiveValue(StringRef IDVal, unsigned Size) {
    auto parseOp = [&]() -> bool {
        const MCExpr* Value;
...
static bool
parseHexOcta(AsmParser& Asm, uint64_t& hi, uint64_t& lo) {
    if (Asm.getTok().isNot(AsmToken::Integer) && Asm.getTok().isNot(AsmToken::BigNum))
...
bool
AsmParser::parseDirectiveOctaValue(StringRef IDVal) {
    auto parseOp = [&]() -> bool {
        if (checkForValidSection())
            return true;
        uint64_t hi, lo;
...
bool
AsmParser::parseRealValue(const fltSemantics& Semantics, APInt& Res) {
    ...
    if (Lexer.isNot(AsmToken::Integer) && Lexer.isNot(AsmToken::Real) && Lexer.isNot(AsmToken::Identifier))
    ...
    if (getLexer().is(AsmToken::Identifier)) {
        if (!IDVal.compare_insensitive("infinity") || !IDVal.compare_insensitive("inf"))
...
bool
AsmParser::parseDirectiveRealValue(StringRef IDVal, const fltSemantics& Semantics) {
    auto parseOp = [&]() -> bool {
        APInt AsInt;
        if (checkForValidSection() || parseRealValue(Semantics, AsInt))
            return true;
        getStreamer().emitIntValue(AsInt.getLimitedValue(), AsInt.getBitWidth() / 8);
        return false;
    };
...
bool
AsmParser::parseDirectiveAlign(bool IsPow2, unsigned ValueSize) {
    ...
    auto parseAlign = [&]() -> bool {
        ...
        if (parseOptionalToken(AsmToken::Comma)) {
            ...
            if (parseOptionalToken(AsmToken::Comma))
                if (parseTokenLoc(MaxBytesLoc) || parseAbsoluteExpression(MaxBytesToFill))
                    return true;
        }
        return parseEOL();
    };
    ...
    if ((!HasFillExpr || Lexer.getMAI().getTextAlignFillValue() == FillExpr) && ValueSize == 1 && useCodeAlign) {
        ...
        getStreamer().emitValueToAlignment(Align(Alignment), FillExpr, ValueSize, MaxBytesToFill);
    }
...
bool
AsmParser::parseDirectiveFile(SMLoc DirectiveLoc) {
    ...
    if (getLexer().is(AsmToken::String)) {
        if (check(FileNumber == -1, "explicit path specified, but no file number") || parseEscapedString(FilenameData))
            return true;
    ...
    while (!parseOptionalToken(AsmToken::EndOfStatement)) {
        StringRef Keyword;
        if (check(getTok().isNot(AsmToken::Identifier), "unexpected token in '.file' directive") || parseIdentifier(Keyword))
            return true;
        if (Keyword == "md5") {
            HasMD5 = true;
            if (check(FileNumber == -1, "MD5 checksum specified, but no file number") || parseHexOcta(*this, MD5Hi, MD5Lo))
                return true;
        } else if (Keyword == "source") {
            HasSource = true;
            if (check(FileNumber == -1, "source specified, but no file number") || check(getTok().isNot(AsmToken::String), "unexpected token in '.file' directive") || parseEscapedString(SourceString))
                return true;
        } else {
...
bool
AsmParser::parseDirectiveLoc() {
    int64_t FileNumber = 0, LineNumber = 0;
    SMLoc   Loc = getTok().getLoc();
    if (parseIntToken(FileNumber, "unexpected token in '.loc' directive") || check(FileNumber < 1 && Ctx.getDwarfVersion() < 5, Loc, "file number less than one in '.loc' directive") || check(!getContext().isValidDwarfFileNumber(FileNumber), Loc, "unassigned file number in '.loc' directive"))
        return true;
    ...
    getStreamer().emitDwarfLocDirective(FileNumber, LineNumber, ColumnPos, Flags, Isa, Discriminator, StringRef());
...
bool
AsmParser::parseDirectiveCVFile() {
    ...
    if (parseIntToken(FileNumber, "expected file number in '.cv_file' directive") || check(FileNumber < 1, FileNumberLoc, "file number less than one") || check(getTok().isNot(AsmToken::String), "unexpected token in '.cv_file' directive") || parseEscapedString(Filename))
        return true;
    if (!parseOptionalToken(AsmToken::EndOfStatement)) {
        if (check(getTok().isNot(AsmToken::String), "unexpected token in '.cv_file' directive") || parseEscapedString(Checksum) || parseIntToken(ChecksumKind, "expected checksum kind in '.cv_file' directive") || parseEOL())
            return true;
    }
    ...
    ArrayRef<uint8_t> ChecksumAsBytes(reinterpret_cast<const uint8_t*>(CKMem), Checksum.size());

    if (!getStreamer().emitCVFileDirective(FileNumber, Filename, ChecksumAsBytes, static_cast<uint8_t>(ChecksumKind)))
...
bool
AsmParser::parseCVFunctionId(int64_t& FunctionId, StringRef DirectiveName) {
    SMLoc Loc;
    return parseTokenLoc(Loc) || parseIntToken(FunctionId, "expected function id in '" + DirectiveName + "' directive") || check(FunctionId < 0 || FunctionId >= UINT_MAX, Loc, "expected function id within range [0, UINT_MAX)");
}

bool
AsmParser::parseCVFileId(int64_t& FileNumber, StringRef DirectiveName) {
    SMLoc Loc;
    return parseTokenLoc(Loc) || parseIntToken(FileNumber, "expected integer in '" + DirectiveName + "' directive") || check(FileNumber < 1, Loc, "file number less than one in '" + DirectiveName + "' directive") || check(!getCVContext().isValidFileNumber(FileNumber), Loc, "unassigned file number in '" + DirectiveName + "' directive");
}
...
bool
AsmParser::parseDirectiveCVFuncId() {
...
bool
AsmParser::parseDirectiveCVInlineSiteId() {
    SMLoc   FunctionIdLoc = getTok().getLoc();
    int64_t FunctionId;
    int64_t IAFunc;
    int64_t IAFile;
    ...
    // "within"
    if (check((getLexer().isNot(AsmToken::Identifier) || getTok().getIdentifier() != "within"), "expected 'within' identifier in '.cv_inline_site_id' directive"))
        return true;
    Lex();
    ...
    // "inlined_at"
    if (check((getLexer().isNot(AsmToken::Identifier) || getTok().getIdentifier() != "inlined_at"),
    ...
    // IAFile IALine
    if (parseCVFileId(IAFile, ".cv_inline_site_id") || parseIntToken(IALine, "expected line number after 'inlined_at'"))
        return true;
    ...
    if (!getStreamer().emitCVInlineSiteIdDirective(FunctionId, IAFunc, IAFile, IALine, IACol, FunctionIdLoc))
        return Error(FunctionIdLoc, "function id already allocated");
...
/// optional items are .loc sub-directives.
bool
AsmParser::parseDirectiveCVLoc() {
    SMLoc   DirectiveLoc = getTok().getLoc();
    int64_t FunctionId, FileNumber;
    if (parseCVFunctionId(FunctionId, ".cv_loc") || parseCVFileId(FileNumber, ".cv_loc"))
        return true;
    ...
    getStreamer().emitCVLocDirective(FunctionId, FileNumber, LineNumber, ColumnPos, PrologueEnd, IsStmt, StringRef(), DirectiveLoc);
    return false;
}
...
bool
AsmParser::parseDirectiveCVLinetable() {
    int64_t   FunctionId;
    StringRef FnStartName, FnEndName;
    SMLoc     Loc = getTok().getLoc();
    if (parseCVFunctionId(FunctionId, ".cv_linetable") || parseComma() || parseTokenLoc(Loc) || check(parseIdentifier(FnStartName), Loc, "expected identifier in directive") || parseComma() || parseTokenLoc(Loc) || check(parseIdentifier(FnEndName), Loc, "expected identifier in directive"))
        return true;
...
bool
AsmParser::parseDirectiveCVInlineLinetable() {
    ...
    MCSymbol* FnStartSym = getContext().getOrCreateSymbol(FnStartName);
    MCSymbol* FnEndSym = getContext().getOrCreateSymbol(FnEndName);
    getStreamer().emitCVInlineLinetableDirective(PrimaryFunctionId, SourceFileId, SourceLineNum, FnStartSym, FnEndSym);
    return false;
}

void
AsmParser::initializeCVDefRangeTypeMap() {
...
bool
AsmParser::parseDirectiveCVDefRange() {
    SMLoc                                                    Loc;
    ...
    switch (CVDRType) {
        ...
        case CVDR_DEFRANGE_FRAMEPOINTER_REL: {
            int64_t DROffset;
            if (parseToken(AsmToken::Comma, "expected comma before offset in .cv_def_range directive") || parseAbsoluteExpression(DROffset))
        ...
        case CVDR_DEFRANGE_SUBFIELD_REGISTER: {
            ...
            if (parseToken(AsmToken::Comma,
                ...
                || parseAbsoluteExpression(DRRegister))
                return Error(Loc, "expected register number");
            if (parseToken(AsmToken::Comma, "expected comma before offset in .cv_def_range directive") || parseAbsoluteExpression(DROffsetInParent))
                return Error(Loc, "expected offset value");
...
bool
AsmParser::parseDirectiveCVString() {
    std::string Data;
...
/// ::= .cv_stringtable
bool
AsmParser::parseDirectiveCVStringTable() {
...
/// ::= .cv_filechecksums
bool
AsmParser::parseDirectiveCVFileChecksums() {
...
/// ::= .cv_fpo_data procsym
bool
AsmParser::parseDirectiveCVFPOData() {
...
bool
AsmParser::parseDirectiveCFIStartProc() {
    StringRef Simple;
    if (!parseOptionalToken(AsmToken::EndOfStatement)) {
        if (check(parseIdentifier(Simple) || Simple != "simple", "unexpected token") || parseEOL())
            return true;
    }
...
/// ::= .cfi_endproc
bool
AsmParser::parseDirectiveCFIEndProc() {
    if (parseEOL())
        return true;
...
bool
AsmParser::parseRegisterOrRegisterNumber(int64_t& Register, SMLoc DirectiveLoc) {
    MCRegister RegNo;
...
bool
AsmParser::parseDirectiveCFIDefCfa(SMLoc DirectiveLoc) {
    int64_t Register = 0, Offset = 0;
    if (parseRegisterOrRegisterNumber(Register, DirectiveLoc) || parseComma() || parseAbsoluteExpression(Offset) || parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFIRegister(SMLoc DirectiveLoc) {
    int64_t Register1 = 0, Register2 = 0;
    if (parseRegisterOrRegisterNumber(Register1, DirectiveLoc) || parseComma() || parseRegisterOrRegisterNumber(Register2, DirectiveLoc) || parseEOL())
        return true;
...
/// ::= .cfi_window_save
bool
AsmParser::parseDirectiveCFIWindowSave() {
    if (parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFIAdjustCfaOffset() {
...
bool
AsmParser::parseDirectiveCFIDefCfaRegister(SMLoc DirectiveLoc) {
    int64_t Register = 0;
...
bool
AsmParser::parseDirectiveCFILLVMDefAspaceCfa(SMLoc DirectiveLoc) {
    int64_t Register = 0, Offset = 0, AddressSpace = 0;
    if (parseRegisterOrRegisterNumber(Register, DirectiveLoc) || parseComma() || parseAbsoluteExpression(Offset) || parseComma() || parseAbsoluteExpression(AddressSpace) || parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFIOffset(SMLoc DirectiveLoc) {
    int64_t Register = 0;
    int64_t Offset = 0;
...
bool
AsmParser::parseDirectiveCFIRelOffset(SMLoc DirectiveLoc) {
    int64_t Register = 0, Offset = 0;
...
static bool
isValidEncoding(int64_t Encoding) {
    if (Encoding & ~0xff)
        return false;
    ...
    if (Format != dwarf::DW_EH_PE_absptr && Format != dwarf::DW_EH_PE_udata2 && Format != dwarf::DW_EH_PE_udata4 && Format != dwarf::DW_EH_PE_udata8 && Format != dwarf::DW_EH_PE_sdata2 && Format != dwarf::DW_EH_PE_sdata4 && Format != dwarf::DW_EH_PE_sdata8 && Format != dwarf::DW_EH_PE_signed)
        return false;

    const unsigned Application = Encoding & 0x70;
    if (Application != dwarf::DW_EH_PE_absptr && Application != dwarf::DW_EH_PE_pcrel)
        return false;
...
bool
AsmParser::parseDirectiveCFIPersonalityOrLsda(bool IsPersonality) {
    int64_t Encoding = 0;
    ...
    StringRef Name;
    if (check(!isValidEncoding(Encoding), "unsupported encoding.") || parseComma() || check(parseIdentifier(Name), "expected identifier in directive") || parseEOL())
        return true;
...
/// ::= .cfi_remember_state
bool
AsmParser::parseDirectiveCFIRestoreState() {
    if (parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFISameValue(SMLoc DirectiveLoc) {
    int64_t Register = 0;
...
bool
AsmParser::parseDirectiveCFIRestore(SMLoc DirectiveLoc) {
    int64_t Register = 0;
    if (parseRegisterOrRegisterNumber(Register, DirectiveLoc) || parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFIEscape() {
    std::string Values;
    int64_t     CurrValue;
...
bool
AsmParser::parseDirectiveCFIReturnColumn(SMLoc DirectiveLoc) {
    int64_t Register = 0;
    if (parseRegisterOrRegisterNumber(Register, DirectiveLoc) || parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveCFIUndefined(SMLoc DirectiveLoc) {
    int64_t Register = 0;
...
/// ::= .noaltmacro
bool
AsmParser::parseDirectiveAltmacro(StringRef Directive) {
    if (parseEOL())
        return true;
...
/// ::= .macros_off
bool
AsmParser::parseDirectiveMacrosOnOff(StringRef Directive) {
    if (parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveMacro(SMLoc DirectiveLoc) {
    StringRef Name;
    if (parseIdentifier(Name))
    ...
    while (getLexer().isNot(AsmToken::EndOfStatement)) {
        if (!Parameters.empty() && Parameters.back().Vararg)
            return Error(Lexer.getLoc(), "vararg parameter '" + Parameters.back().Name + "' should be the last parameter");
    ...
    while (true) {
        ...
        if (getLexer().is(AsmToken::Identifier)) {
            if (getTok().getIdentifier() == ".endm" || getTok().getIdentifier() == ".endmacro") {
                if (MacroDepth == 0) {  // Outermost macro.
                    EndToken = getTok();
                    Lexer.Lex();
                    if (getLexer().isNot(AsmToken::EndOfStatement))
                        return TokError("unexpected token in '" + EndToken.getIdentifier() + "' directive");
                    break;
                } else {
...
void
AsmParser::checkForBadMacro(SMLoc DirectiveLoc, StringRef Name, StringRef Body, ArrayRef<MCAsmMacroParameter> Parameters) {
...
/// ::= .exitm
bool
AsmParser::parseDirectiveExitMacro(StringRef Directive) {
    if (parseEOL())
        return true;
...
/// ::= .endmacro
bool
AsmParser::parseDirectiveEndMacro(StringRef Directive) {
    if (getLexer().isNot(AsmToken::EndOfStatement))
...
/// ::= .purgem name
bool
AsmParser::parseDirectivePurgeMacro(SMLoc DirectiveLoc) {
    StringRef Name;
    SMLoc     Loc;
    if (parseTokenLoc(Loc) || check(parseIdentifier(Name), Loc, "expected identifier in '.purgem' directive") || parseEOL())
        return true;
    ...
    DEBUG_WITH_TYPE("asm-macros", dbgs() << "Un-defining macro: " << Name << "\n");
    return false;
}
...
bool
AsmParser::parseDirectiveBundleAlignMode() {
    // Expect a single argument: an expression that evaluates to a constant
    // in the inclusive range 0-30.
    SMLoc   ExprLoc = getLexer().getLoc();
    int64_t AlignSizePow2;
    if (checkForValidSection() || parseAbsoluteExpression(AlignSizePow2) || parseEOL() || check(AlignSizePow2 < 0 || AlignSizePow2 > 30, ExprLoc, "invalid bundle alignment size (expected between 0 and 30)"))
        return true;
...
AsmParser::parseDirectiveBundleLock() {
    ...
    if (!parseOptionalToken(AsmToken::EndOfStatement)) {
        if (check(parseIdentifier(Option), Loc, kInvalidOptionError) || check(Option != "align_to_end", Loc, kInvalidOptionError) || parseEOL())
            return true;
        AlignToEnd = true;
    }
...
/// ::= {.bundle_lock}
bool
AsmParser::parseDirectiveBundleUnlock() {
...
bool
AsmParser::parseDirectiveSpace(StringRef IDVal) {
    SMLoc         NumBytesLoc = Lexer.getLoc();
    const MCExpr* NumBytes;
    if (checkForValidSection() || parseExpression(NumBytes))
        return true;
...
bool
AsmParser::parseDirectiveDCB(StringRef IDVal, unsigned Size) {
    SMLoc   NumValuesLoc = Lexer.getLoc();
    int64_t NumValues;
...
bool
AsmParser::parseDirectiveRealDCB(StringRef IDVal, const fltSemantics& Semantics) {
    SMLoc   NumValuesLoc = Lexer.getLoc();
    int64_t NumValues;
    if (checkForValidSection() || parseAbsoluteExpression(NumValues))
        return true;
...
bool
AsmParser::parseDirectiveDS(StringRef IDVal, unsigned Size) {
    SMLoc   NumValuesLoc = Lexer.getLoc();
    int64_t NumValues;
    if (checkForValidSection() || parseAbsoluteExpression(NumValues) || parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveLEB128(bool Signed) {
    if (checkForValidSection())
        return true;
...
bool
AsmParser::parseDirectiveSymbolAttribute(MCSymbolAttr Attr) {
    auto parseOp = [&]() -> bool {
        StringRef Name;
        SMLoc     Loc = getTok().getLoc();
        if (parseIdentifier(Name))
...
bool
AsmParser::parseDirectiveComm(bool IsLocal) {
    if (checkForValidSection())
        return true;
    ...
    if (getLexer().is(AsmToken::Comma)) {
        ...
        if ((!IsLocal && Lexer.getMAI().getCOMMDirectiveAlignmentIsInBytes()) || (IsLocal && LCOMM == LCOMM::ByteAlignment)) {
    ...
    if (IsLocal) {
        getStreamer().emitLocalCommonSymbol(Sym, Size, Align(1ULL << Pow2Alignment));
        return false;
    }
...
///  ::= .include "filename"
bool
AsmParser::parseDirectiveInclude() {
    // Allow the strings to have escaped octal character sequence.
    std::string Filename;
    SMLoc       IncludeLoc = getTok().getLoc();

    if (check(getTok().isNot(AsmToken::String), "expected string in '.include' directive") || parseEscapedString(Filename) || check(getTok().isNot(AsmToken::EndOfStatement), "unexpected token in '.include' directive") ||
        // Attempt to switch the lexer to the included file before consuming the
        // end of statement to avoid losing it when we switch.
        check(enterIncludeFile(Filename), IncludeLoc, "Could not find include file '" + Filename + "'"))
        return true;
...
bool
AsmParser::parseDirectiveIncbin() {
    // Allow the strings to have escaped octal character sequence.
    std::string Filename;
    SMLoc       IncbinLoc = getTok().getLoc();
    if (check(getTok().isNot(AsmToken::String), "expected string in '.incbin' directive") || parseEscapedString(Filename))
        return true;
...
bool
AsmParser::parseDirectiveIf(SMLoc DirectiveLoc, DirectiveKind DirKind) {
...
/// ::= .ifb string
bool
AsmParser::parseDirectiveIfb(SMLoc DirectiveLoc, bool ExpectBlank) {
    TheCondStack.push_back(TheCondState);
    TheCondState.TheCond = AsmCond::IfCond;
...
bool
AsmParser::parseDirectiveIfc(SMLoc DirectiveLoc, bool ExpectEqual) {
    TheCondStack.push_back(TheCondState);
    TheCondState.TheCond = AsmCond::IfCond;
...
bool
AsmParser::parseDirectiveIfeqs(SMLoc DirectiveLoc, bool ExpectEqual) {
...
/// ::= .ifdef symbol
bool
AsmParser::parseDirectiveIfdef(SMLoc DirectiveLoc, bool expect_defined) {
    StringRef Name;
    TheCondStack.push_back(TheCondState);
    TheCondState.TheCond = AsmCond::IfCond;
    ...
    if (TheCondState.Ignore) {
        ...
        if (check(parseIdentifier(Name), "expected identifier after '.ifdef'") || parseEOL())
            return true;
...
/// ::= .elseif expression
bool
AsmParser::parseDirectiveElseIf(SMLoc DirectiveLoc) {
    if (TheCondState.TheCond != AsmCond::IfCond && TheCondState.TheCond != AsmCond::ElseIfCond)
...
/// ::= .else
bool
AsmParser::parseDirectiveElse(SMLoc DirectiveLoc) {
    if (parseEOL())
        return true;
...
/// ::= .end
bool
AsmParser::parseDirectiveEnd(SMLoc DirectiveLoc) {
    if (parseEOL())
        return true;
...
///   ::= .warning [string]
bool
AsmParser::parseDirectiveWarning(SMLoc L) {
    if (!TheCondStack.empty()) {
        if (TheCondStack.back().Ignore) {
            eatToEndOfStatement();
            return false;
...
/// ::= .endif
bool
AsmParser::parseDirectiveEndIf(SMLoc DirectiveLoc) {
    if (parseEOL())
        return true;
...
void
AsmParser::initializeDirectiveKindMap() {
...
AsmParser::parseMacroLikeBody(SMLoc DirectiveLoc) {
    ...
    while (true) {
        ...
        if (Lexer.is(AsmToken::Identifier) && (getTok().getIdentifier() == ".rep" || getTok().getIdentifier() == ".rept" || getTok().getIdentifier() == ".irp" || getTok().getIdentifier() == ".irpc")) {
            ++NestLevel;
        }
        ...
        if (Lexer.is(AsmToken::Identifier) && getTok().getIdentifier() == ".endr") {
            if (NestLevel == 0) {
                ...
                Lex();
                if (Lexer.isNot(AsmToken::EndOfStatement)) {
                    printError(getTok().getLoc(), "unexpected token in '.endr' directive");
                    return nullptr;
                }
                break;
            }
...
void
AsmParser::instantiateMacroLikeBody(MCAsmMacro* M, SMLoc DirectiveLoc, raw_svector_ostream& OS) {
    OS << ".endr\n";
...
///   ::= .rep | .rept count
bool
AsmParser::parseDirectiveRept(SMLoc DirectiveLoc, StringRef Dir) {
...
/// ::= .irp symbol,values
bool
AsmParser::parseDirectiveIrp(SMLoc DirectiveLoc) {
    MCAsmMacroParameter Parameter;
    MCAsmMacroArguments A;
    if (check(parseIdentifier(Parameter.Name), "expected identifier in '.irp' directive") || parseComma() || parseMacroArguments(nullptr, A) || parseEOL())
        return true;
...
/// ::= .irpc symbol,values
bool
AsmParser::parseDirectiveIrpc(SMLoc DirectiveLoc) {
    MCAsmMacroParameter Parameter;
    MCAsmMacroArguments A;

    if (check(parseIdentifier(Parameter.Name), "expected identifier in '.irpc' directive") || parseComma() || parseMacroArguments(nullptr, A))
        return true;
...
bool
AsmParser::parseDirectiveEndr(SMLoc DirectiveLoc) {
    if (ActiveMacros.empty())
...
bool
AsmParser::parseDirectiveMSEmit(SMLoc IDLoc, ParseStatementInfo& Info, size_t Len) {
    const MCExpr* Value;
    SMLoc         ExprLoc = getLexer().getLoc();
    if (parseExpression(Value))
        return true;
...
bool
AsmParser::parseDirectiveMSAlign(SMLoc IDLoc, ParseStatementInfo& Info) {
    const MCExpr* Value;
    SMLoc         ExprLoc = getLexer().getLoc();
    if (parseExpression(Value))
        return true;
...
bool
AsmParser::parseDirectivePrint(SMLoc DirectiveLoc) {
...
bool
AsmParser::parseDirectiveAddrsig() {
    if (parseEOL())
        return true;
...
bool
AsmParser::parseDirectiveAddrsigSym() {
    StringRef Name;
...
bool
AsmParser::parseDirectivePseudoProbe() {
    int64_t Guid;
    int64_t Index;
    int64_t Type;
    int64_t Attr;
    ...
    while (getLexer().is(AsmToken::At)) {
        ...
        if (getLexer().is(AsmToken::Integer)) {
            if (parseIntToken(CallerGuid, "unexpected token in '.pseudoprobe' directive"))
                return true;
        }
        ...
        if (getLexer().is(AsmToken::Integer)) {
            if (parseIntToken(CallerProbeId, "unexpected token in '.pseudoprobe' directive"))
                return true;
        }
...
/// symbols.
bool
AsmParser::parseDirectiveLTODiscard() {
    auto ParseOp = [&]() -> bool {
        StringRef Name;
        SMLoc     Loc = getTok().getLoc();
        if (parseIdentifier(Name))
...
static int
rewritesSort(const AsmRewrite* AsmRewriteA, const AsmRewrite* AsmRewriteB) {
    ...
    if (AsmRewritePrecedence[AsmRewriteA->Kind] > AsmRewritePrecedence[AsmRewriteB->Kind])
        return -1;
...
AsmParser::parseMSInlineAsm(
    ...
    while (getLexer().isNot(AsmToken::Eof)) {
        ...
        for (unsigned i = 1, e = Info.ParsedOperands.size(); i != e; ++i) {
            ...
            // Register operand.
            if (Operand.isReg() && !Operand.needAddressOf() && !getTargetParser().OmitRegisterFromClobberLists(Operand.getReg())) {
            ...
            if (isOutput) {
                ...
                AsmStrRewrites.emplace_back(AOK_Output, Start, SymName.size(), 0, Restricted);
            } else {
                InputDecls.push_back(OpDecl);
                InputDeclsAddressOf.push_back(Operand.needAddressOf());
                InputConstraints.push_back(Constraint.str());
                if (Desc.OpInfo[i - 1].isBranchTarget())
                    AsmStrRewrites.emplace_back(AOK_CallInput, Start, SymName.size(), 0, Restricted);
                else
                    AsmStrRewrites.emplace_back(AOK_Input, Start, SymName.size(), 0, Restricted);
            }
        }

        // Consider implicit defs to be clobbers.  Think of cpuid and push.
        ArrayRef<MCPhysReg> ImpDefs(Desc.getImplicitDefs(), Desc.getNumImplicitDefs());
    ...
    ClobberRegs.erase(std::unique(ClobberRegs.begin(), ClobberRegs.end()), ClobberRegs.end());
    ...
    for (auto it = AsmStrRewrites.begin(); it != AsmStrRewrites.end(); ++it) {
        ...
        switch (Kind) {
                ...
                if (AR.IntelExp.hasOffset()) {
                    ...
                    auto      rewrite_it = std::find_if(
                        ...
                        [&](const AsmRewrite& FusingAR) {
                            return FusingAR.Loc == OffsetLoc && FusingAR.Len == OffsetLen && (FusingAR.Kind == AOK_Input || FusingAR.Kind == AOK_CallInput);
                        }
                    );
            ...
            case AOK_Align: {
                ...
                AdditionalSkip = (Val < 4) ? 2 : Val < 7 ? 3
                                                         : 4;
                break;
            }
            case AOK_EVEN:
...
bool
HLASMAsmParser::parseAsHLASMLabel(ParseStatementInfo& Info, MCAsmParserSemaCallback* SI) {
    ...
    // i.e asm("<token>\n")
    if (getTok().is(AsmToken::EndOfStatement))
        return Error(LabelLoc, "Cannot have just a label for an HLASM inline asm statement");
    ...
    // info to make a dwarf label entry for this label if needed.
    if (enabledGenDwarfForAssembly())
        MCGenDwarfLabelEntry::Make(Sym, &getStreamer(), getSourceManager(), LabelLoc);
...
bool
HLASMAsmParser::parseAsMachineInstruction(ParseStatementInfo& Info, MCAsmParserSemaCallback* SI) {
...
bool
HLASMAsmParser::parseStatement(ParseStatementInfo& Info, MCAsmParserSemaCallback* SI) {
    assert(!hasPendingError() && "parseStatement started with pending error");
...
namespace MCParserUtils {
    ...
    /// or subexpressions.
    static bool
    isSymbolUsedInExpression(const MCSymbol* Sym, const MCExpr* Value) {
        switch (Value->getKind()) {
            case MCExpr::Binary: {
                const MCBinaryExpr* BE = static_cast<const MCBinaryExpr*>(Value);
                return isSymbolUsedInExpression(Sym, BE->getLHS()) || isSymbolUsedInExpression(Sym, BE->getRHS());
            }
            case MCExpr::Target:
            case MCExpr::Constant:
                return false;
            ...
            case MCExpr::Unary:
                return isSymbolUsedInExpression(
                    Sym,
                    static_cast<const MCUnaryExpr*>(Value)->getSubExpr()
                );
        }
    ...
    bool
    parseAssignmentExpression(StringRef Name, bool allow_redef, MCAsmParser& Parser, MCSymbol*& Sym, const MCExpr*& Value) {
        ...
        if (Sym) {
            ...
            if (isSymbolUsedInExpression(Sym, Value))
                ...
                return Parser.Error(EqualLoc, "invalid reassignment of non-absolute variable '" + Name + "'");
        } else if (Name == ".") {
...
MCAsmParser*
llvm::createMCAsmParser(SourceMgr& SM, MCContext& C, MCStreamer& Out, const MCAsmInfo& MAI, unsigned CB) {
