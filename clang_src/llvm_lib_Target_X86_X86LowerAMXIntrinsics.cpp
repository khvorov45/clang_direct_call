...
#include "llvm_include_llvm_CodeGen_ValueTypes.h"
...
#include "llvm_include_llvm_IR_IntrinsicsX86.h"
...
isV256I32Ty(Type* Ty) {
    if (auto* FVT = dyn_cast<FixedVectorType>(Ty))
        return FVT->getNumElements() == 256 && FVT->getElementType()->isIntegerTy(32);
    return false;
}
#endif

static cl::opt<bool>
    X86ScalarizeAMX("enable-x86-scalar-amx", cl::init(false), cl::Hidden, cl::desc("X86: enable AMX scalarizition."));
...
class X86LowerAMXIntrinsics {
  ...
  private:
    DomTreeUpdater& DTU;
    LoopInfo*       LI;
    BasicBlock*     createLoop(BasicBlock* Preheader, BasicBlock* Exit, Value* Bound, Value* Step, StringRef Name, IRBuilderBase& B, Loop* L);
    template<bool IsTileLoad>
    Value* createTileLoadStoreLoops(BasicBlock* Start, BasicBlock* End, IRBuilderBase& B, Value* Row, Value* Col, Value* Ptr, Value* Stride, Value* Tile);
    template<Intrinsic::ID IntrID>
    std::enable_if_t<IntrID == Intrinsic::x86_tdpbssd_internal || IntrID == Intrinsic::x86_tdpbsud_internal || IntrID == Intrinsic::x86_tdpbusd_internal || IntrID == Intrinsic::x86_tdpbuud_internal || IntrID == Intrinsic::x86_tdpbf16ps_internal, Value*>
    createTileDPLoops(BasicBlock* Start, BasicBlock* End, IRBuilderBase& B, Value* Row, Value* Col, Value* K, Value* Acc, Value* LHS, Value* RHS);
    template<bool IsTileLoad>
    bool lowerTileLoadStore(Instruction* TileLoadStore);
    template<Intrinsic::ID IntrID>
    std::enable_if_t<IntrID == Intrinsic::x86_tdpbssd_internal || IntrID == Intrinsic::x86_tdpbsud_internal || IntrID == Intrinsic::x86_tdpbusd_internal || IntrID == Intrinsic::x86_tdpbuud_internal || IntrID == Intrinsic::x86_tdpbf16ps_internal, bool>
...
BasicBlock*
X86LowerAMXIntrinsics::createLoop(BasicBlock* Preheader, BasicBlock* Exit, Value* Bound, Value* Step, StringRef Name, IRBuilderBase& B, Loop* L) {
...
X86LowerAMXIntrinsics::createTileLoadStoreLoops(
    ...
    BasicBlock* RowBody = createLoop(Start, End, Row, B.getInt16(1), IntrinName + ".scalarize.rows", B, RowLoop);
    BasicBlock* RowLatch = RowBody->getSingleSuccessor();

    BasicBlock* ColBody = createLoop(RowBody, RowLatch, Col, B.getInt16(1), IntrinName + ".scalarize.cols", B, ColLoop);
...
template<Intrinsic::ID IntrID>
std::enable_if_t<IntrID == Intrinsic::x86_tdpbssd_internal || IntrID == Intrinsic::x86_tdpbsud_internal || IntrID == Intrinsic::x86_tdpbusd_internal || IntrID == Intrinsic::x86_tdpbuud_internal || IntrID == Intrinsic::x86_tdpbf16ps_internal, Value*>
X86LowerAMXIntrinsics::createTileDPLoops(BasicBlock* Start, BasicBlock* End, IRBuilderBase& B, Value* Row, Value* Col, Value* K, Value* Acc, Value* LHS, Value* RHS) {
    ...
    BasicBlock* InnerBody =
        createLoop(ColBody, ColLoopLatch, K, B.getInt16(1), IntrinName + ".scalarize.inner", B, InnerLoop);
    ...
    if (IntrID != Intrinsic::x86_tdpbf16ps_internal) {
        ...
        Value*           AV2F32 = B.CreateBitCast(
            B.CreateShuffleVector(SubVecA, ZeroV2I16, ShuffleArray),
            V2F32Ty
        );
        Value* BV2F32 = B.CreateBitCast(
            B.CreateShuffleVector(SubVecB, ZeroV2I16, ShuffleArray),
            V2F32Ty
        );
...
X86LowerAMXIntrinsics::lowerTileDP(Instruction* TileDP) {
    Value *M, *N, *K, *C, *A, *B;
    match(TileDP, m_Intrinsic<IntrID>(m_Value(M), m_Value(N), m_Value(K), m_Value(C), m_Value(A), m_Value(B)));
    ...
    Value*      ResVec = createTileDPLoops<IntrID>(Start, End, Builder, M, NDWord, KDWord, C, A, B);
...
template<bool IsTileLoad>
bool
X86LowerAMXIntrinsics::lowerTileLoadStore(Instruction* TileLoadStore) {
    Value *M, *N, *Ptr, *Stride, *Tile;
    if (IsTileLoad)
        match(TileLoadStore, m_Intrinsic<Intrinsic::x86_tileloadd64_internal>(m_Value(M), m_Value(N), m_Value(Ptr), m_Value(Stride)));
    else
        match(TileLoadStore, m_Intrinsic<Intrinsic::x86_tilestored64_internal>(m_Value(M), m_Value(N), m_Value(Ptr), m_Value(Stride), m_Value(Tile)));
...
bool
X86LowerAMXIntrinsics::lowerTileZero(Instruction* TileZero) {
...
class X86LowerAMXIntrinsicsLegacyPass: public FunctionPass {
  public:
    ...
    runOnFunction(Function& F) override {
        ...
        if (!F.hasFnAttribute(Attribute::OptimizeNone) && TM->getOptLevel() != CodeGenOpt::None)
            return false;
...
static void*
initializeX86LowerAMXIntrinsicsLegacyPassPassOnce(PassRegistry& Registry) {
    initializeTargetPassConfigPass(Registry);
    PassInfo* PI = new PassInfo(PassName, "lower-amx-intrinsics", &X86LowerAMXIntrinsicsLegacyPass::ID, PassInfo::NormalCtor_t(callDefaultCtor<X86LowerAMXIntrinsicsLegacyPass>), false, false);
    Registry.registerPass(*PI, true);
    return PI;
}

static llvm::once_flag InitializeX86LowerAMXIntrinsicsLegacyPassPassFlag;

void
llvm::initializeX86LowerAMXIntrinsicsLegacyPassPass(PassRegistry& Registry) {
    llvm::call_once(InitializeX86LowerAMXIntrinsicsLegacyPassPassFlag, initializeX86LowerAMXIntrinsicsLegacyPassPassOnce, std::ref(Registry));
}
