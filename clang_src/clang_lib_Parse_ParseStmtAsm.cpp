...
#include "clang_include_clang_Parse_Parser.h"
#include "clang_include_clang_Parse_RAIIObjectsForParser.h"
...
class ClangAsmParserCallback: public llvm::MCAsmParserSemaCallback {
  ...
  public:
    ClangAsmParserCallback(Parser& P, SourceLocation Loc, StringRef AsmString, ArrayRef<Token> Toks, ArrayRef<unsigned> Offsets) :
        TheParser(P),
        AsmLoc(Loc),
        AsmString(AsmString),
        AsmToks(Toks),
    ...
    void LookupInlineAsmIdentifier(StringRef& LineBuf, llvm::InlineAsmIdentifierInfo& Info, bool IsUnevaluatedContext) override;

    StringRef LookupInlineAsmLabel(StringRef Identifier, llvm::SourceMgr& LSM, llvm::SMLoc Location, bool Create) override;

    bool
    LookupInlineAsmField(StringRef Base, StringRef Member, unsigned& Offset) override {
        return TheParser.getActions().LookupInlineAsmField(Base, Member, Offset, AsmLoc);
    }

    static void
    DiagHandlerCallback(const llvm::SMDiagnostic& D, void* Context) {
        ((ClangAsmParserCallback*)Context)->handleDiagnostic(D);
    }

  private:
    /// Collect the appropriate tokens for the given string.
    void findTokensForString(StringRef Str, SmallVectorImpl<Token>& TempToks, const Token*& FirstOrigToken) const;
...
ClangAsmParserCallback::LookupInlineAsmIdentifier(
    ...
    ExprResult Result = TheParser.ParseMSAsmIdentifier(LineToks, NumConsumedToks, IsUnevaluatedContext);
    ...
    if (NumConsumedToks == 0 || NumConsumedToks == LineToks.size()) {
        ...
        assert(FirstOrigToken[NumConsumedToks].getLocation() == LineToks[NumConsumedToks].getLocation());
        ...
        unsigned TotalOffset =
            (AsmTokOffsets[LastIndex] + AsmToks[LastIndex].getLength() - AsmTokOffsets[FirstIndex]);
...
StringRef
ClangAsmParserCallback::LookupInlineAsmLabel(StringRef Identifier, llvm::SourceMgr& LSM, llvm::SMLoc Location, bool Create) {
...
ClangAsmParserCallback::findTokensForString(
    StringRef               Str,
    SmallVectorImpl<Token>& TempToks,
    const Token*&           FirstOrigToken
) const {
    ...
    assert(!std::less<const char*>()(Str.begin(), AsmString.begin()) && !std::less<const char*>()(AsmString.end(), Str.end()));
...
SourceLocation
ClangAsmParserCallback::translateLocation(const llvm::SourceMgr& LSM, llvm::SMLoc SMLoc) {
...
void
ClangAsmParserCallback::handleDiagnostic(const llvm::SMDiagnostic& D) {
...
ExprResult
Parser::ParseMSAsmIdentifier(llvm::SmallVectorImpl<Token>& LineToks, unsigned& NumLineToksConsumed, bool IsUnevaluatedContext) {
    ...
    if (Tok.is(tok::kw_this)) {
        ...
        Invalid =
            ParseUnqualifiedId(SS, /*ObjectType=*/nullptr,
                               ...
                               Id);
        // Perform the lookup.
        Result = Actions.LookupInlineAsmIdentifier(SS, TemplateKWLoc, Id, IsUnevaluatedContext);
    }
    ...
    while (Result.isUsable() && Tok.is(tok::period)) {
        ...
        Result = Actions.LookupInlineAsmVarDeclField(Result.get(), Id->getName(), Tok.getLocation());
    }
...
static bool
buildMSAsmString(Preprocessor& PP, SourceLocation AsmLoc, ArrayRef<Token> AsmToks, SmallVectorImpl<unsigned>& TokOffsets, SmallString<512>& Asm) {
...
bool
Parser::isGCCAsmStatement(const Token& TokAfterAsm) const {
    return TokAfterAsm.is(tok::l_paren) || isGNUAsmQualifier(TokAfterAsm);
}

bool
Parser::isGNUAsmQualifier(const Token& TokAfterAsm) const {
...
Parser::ParseMicrosoftAsmStatement(SourceLocation AsmLoc) {
    ...
    do {
        ...
        if (!InAsmComment && Tok.is(tok::l_brace)) {
            ...
            std::pair<FileID, unsigned> ExpLoc =
                SrcMgr.getDecomposedExpansionLoc(TokLoc);
            if (ExpLoc.first != FID || SrcMgr.getLineNumber(ExpLoc.first, ExpLoc.second) != LineNo) {
        ...
        if (!InAsmComment && BraceNesting && Tok.is(tok::r_brace) && BraceCount == (savedBraceCount + BraceNesting)) {
    ...
    if (!TheTriple.isX86()) {
        ...
        std::string Error;
        TheTarget = LLVMTargetRegistryTheTarget;
        if (!TheTarget)
    ...
    std::unique_ptr<llvm::MCAsmInfo>       MAI(TheTarget->MCAsmInfoCtorFn(*MRI, llvm::Triple(TT), MCOptions));
    std::unique_ptr<llvm::MCInstrInfo>     MII(TheTarget->MCInstrInfoCtorFn());
    std::unique_ptr<llvm::MCSubtargetInfo> STI(TheTarget->MCSubtargetInfoCtorFn(llvm::Triple(TT), TO.CPU, FeaturesStr));
    ...
    ClangAsmParserCallback Callback(*this, AsmLoc, AsmString, AsmToks, TokOffsets);
    TargetParser->setSemaCallback(&Callback);
    TempSrcMgr.setDiagHandler(ClangAsmParserCallback::DiagHandlerCallback, &Callback);
    ...
    if (Parser->parseMSInlineAsm(AsmStringIR, NumOutputs, NumInputs, OpExprs, Constraints, Clobbers, MII.get(), IP.get(), Callback))
        return StmtError();
    ...
    return Actions.ActOnMSAsmStmt(AsmLoc, LBraceLocs[0], AsmToks, AsmStringIR, NumOutputs, NumInputs, ConstraintRefs, ClobberRefs, Exprs, EndLoc);
}
...
Parser::ParseAsmStatement(bool& msAsm) {
    ...
    if (AteExtraColon || Tok.is(tok::colon)) {
        ...
        while (true) {
            ...
            LabelDecl* LD = Actions.LookupOrCreateLabel(Tok.getIdentifierInfo(), Tok.getLocation());
    ...
    T.consumeClose();
    return Actions.ActOnGCCAsmStmt(AsmLoc, false, GAQ.isVolatile(), NumOutputs, NumInputs, Names.data(), Constraints, Exprs, AsmString.get(), Clobbers, NumLabels, T.getCloseLocation());
}
...
bool
Parser::ParseAsmOperandsOpt(SmallVectorImpl<IdentifierInfo*>& Names, SmallVectorImpl<Expr*>& Constraints, SmallVectorImpl<Expr*>& Exprs) {
