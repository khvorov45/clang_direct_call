...
#include "llvm_lib_MC_MCDisassembler_Disassembler.h"
#include "llvm_include_llvm-c_Disassembler.h"
...
#include "llvm_include_llvm_MC_MCDisassembler_MCDisassembler.h"
#include "llvm_include_llvm_MC_MCDisassembler_MCRelocationInfo.h"
#include "llvm_include_llvm_MC_MCDisassembler_MCSymbolizer.h"
...
#include "llvm_include_llvm_MC_MCInstrItineraries.h"
#include "llvm_include_llvm_MC_MCRegisterInfo.h"
#include "llvm_include_llvm_MC_MCSchedule.h"
...
LLVMDisasmContextRef
LLVMCreateDisasmCPU(const char* TT, const char* CPU, void* DisInfo, int TagType, LLVMOpInfoCallback GetOpInfo, LLVMSymbolLookupCallback SymbolLookUp) {
    return LLVMCreateDisasmCPUFeatures(TT, CPU, "", DisInfo, TagType, GetOpInfo, SymbolLookUp);
}

LLVMDisasmContextRef
LLVMCreateDisasm(const char* TT, void* DisInfo, int TagType, LLVMOpInfoCallback GetOpInfo, LLVMSymbolLookupCallback SymbolLookUp) {
    return LLVMCreateDisasmCPUFeatures(TT, "", "", DisInfo, TagType, GetOpInfo, SymbolLookUp);
}
...
//
void
LLVMDisasmDispose(LLVMDisasmContextRef DCR) {
...
static void
emitComments(LLVMDisasmContext* DC, formatted_raw_ostream& FormattedOS) {
...
static int
getItineraryLatency(LLVMDisasmContext* DC, const MCInst& Inst) {
...
/// if no information is available.
static int
getLatency(LLVMDisasmContext* DC, const MCInst& Inst) {
...
static void
emitLatency(LLVMDisasmContext* DC, const MCInst& Inst) {
...
//
size_t
LLVMDisasmInstruction(LLVMDisasmContextRef DCR, uint8_t* Bytes, uint64_t BytesSize, uint64_t PC, char* OutString, size_t OutStringSize) {
    LLVMDisasmContext* DC = static_cast<LLVMDisasmContext*>(DCR);
    ...
    switch (S) {
        ...
        case MCDisassembler::Success: {
            ...
            IP->printInst(&Inst, PC, AnnotationsStr, *DC->getSubtargetInfo(), FormattedOS);
...
//
int
LLVMSetDisasmOptions(LLVMDisasmContextRef DCR, uint64_t Options) {
