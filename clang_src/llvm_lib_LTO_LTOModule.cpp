...
#include "llvm_include_llvm_Object_MachO.h"
...
LTOModule::LTOModule(std::unique_ptr<Module> M, MemoryBufferRef MBRef, llvm::TargetMachine* TM) :
    Mod(std::move(M)),
    MBRef(MBRef),
    _target(TM) {
...
/// bitcode.
bool
LTOModule::isBitcodeFile(const void* Mem, size_t Length) {
...
bool
LTOModule::isBitcodeFile(StringRef Path) {
...
bool
LTOModule::isBitcodeForTarget(MemoryBuffer* Buffer, StringRef TriplePrefix) {
...
std::string
LTOModule::getProducerString(MemoryBuffer* Buffer) {
    Expected<MemoryBufferRef> BCOrErr =
        IRObjectFile::findBitcodeInMemBuffer(Buffer->getMemBufferRef());
    if (errorToBool(BCOrErr.takeError()))
        return "";
...
LTOModule::createFromFile(LLVMContext& Context, StringRef path, const TargetOptions& options) {
    ErrorOr<std::unique_ptr<MemoryBuffer>> BufferOrErr =
        MemoryBuffer::getFile(path);
...
ErrorOr<std::unique_ptr<LTOModule>>
LTOModule::createFromOpenFile(LLVMContext& Context, int fd, StringRef path, size_t size, const TargetOptions& options) {
    return createFromOpenFileSlice(Context, fd, path, size, 0, options);
}

ErrorOr<std::unique_ptr<LTOModule>>
LTOModule::createFromOpenFileSlice(LLVMContext& Context, int fd, StringRef path, size_t map_size, off_t offset, const TargetOptions& options) {
    ErrorOr<std::unique_ptr<MemoryBuffer>> BufferOrErr =
        MemoryBuffer::getOpenFileSlice(sys::fs::convertFDToNativeFile(fd), path, map_size, offset);
    if (std::error_code EC = BufferOrErr.getError()) {
        Context.emitError(EC.message());
        return EC;
    }
...
ErrorOr<std::unique_ptr<LTOModule>>
LTOModule::createFromBuffer(LLVMContext& Context, const void* mem, size_t length, const TargetOptions& options, StringRef path) {
...
ErrorOr<std::unique_ptr<LTOModule>>
LTOModule::createInLocalContext(std::unique_ptr<LLVMContext> Context, const void* mem, size_t length, const TargetOptions& options, StringRef path) {
    StringRef       Data((const char*)mem, length);
    MemoryBufferRef Buffer(Data, path);
...
parseBitcodeFileImpl(MemoryBufferRef Buffer, LLVMContext& Context, bool ShouldBeLazy) {
    // Find the buffer.
    ...
    if (!ShouldBeLazy) {
        // Parse the full file.
        return expectedToErrorOrAndEmitErrors(Context, parseBitcodeFile(*MBOrErr, Context));
    }
...
ErrorOr<std::unique_ptr<LTOModule>>
LTOModule::makeLTOModule(MemoryBufferRef Buffer, const TargetOptions& options, LLVMContext& Context, bool ShouldBeLazy) {
    ...
    std::string   errMsg;
    const LLVMTarget* march = LLVMTargetRegistryTheTarget;
    if (!march)
    ...
    TargetMachine* target = 0;
    if (march->TargetMachineCtorFn) {
        target = march->TargetMachineCtorFn(*march, Triple, CPU, FeatureStr, options, std::nullopt, std::nullopt, CodeGenOpt::Default, false);
    }
...
void
LTOModule::addObjCClass(const GlobalVariable* clgv) {
    ...
    if (objcClassNameFromExpression(c->getOperand(2), className)) {
        ...
        info.attributes = LTO_SYMBOL_PERMISSIONS_DATA | LTO_SYMBOL_DEFINITION_REGULAR | LTO_SYMBOL_SCOPE_DEFAULT;
        info.isFunction = false;
        info.symbol = clgv;
...
void
LTOModule::addObjCCategory(const GlobalVariable* clgv) {
    const ConstantStruct* c = dyn_cast<ConstantStruct>(clgv->getInitializer());
    if (!c)
        return;
...
void
LTOModule::addObjCClassRef(const GlobalVariable* clgv) {
    std::string targetclassName;
...
void
LTOModule::addDefinedDataSymbol(ModuleSymbolTable::Symbol Sym) {
...
void
LTOModule::addDefinedDataSymbol(StringRef Name, const GlobalValue* v) {
...
void
LTOModule::addDefinedFunctionSymbol(ModuleSymbolTable::Symbol Sym) {
    SmallString<64> Buffer;
    {
        raw_svector_ostream OS(Buffer);
        SymTab.printSymbolName(OS, Sym);
...
void
LTOModule::addDefinedFunctionSymbol(StringRef Name, const Function* F) {
    // add to list of defined symbols
    addDefinedSymbol(Name, F, true);
}

void
LTOModule::addDefinedSymbol(StringRef Name, const GlobalValue* def, bool isFunction) {
...
/// defined list.
void
LTOModule::addAsmGlobalSymbol(StringRef name, lto_symbol_attributes scope) {
...
/// undefined list.
void
LTOModule::addAsmGlobalSymbolUndef(StringRef name) {
...
void
LTOModule::addPotentialUndefinedSymbol(ModuleSymbolTable::Symbol Sym, bool isFunc) {
    SmallString<64> name;
    {
...
lto::InputFile*
LTOModule::createInputFile(const void* buffer, size_t buffer_size, const char* path, std::string& outErr) {
    ...
    outErr = std::string(path) + ": Could not read LTO input file: " + toString(ObjOrErr.takeError());
    return nullptr;
}

size_t
LTOModule::getDependentLibraryCount(lto::InputFile* input) {
    return input->getDependentLibraries().size();
}

const char*
LTOModule::getDependentLibrary(lto::InputFile* input, size_t index, size_t* size) {
