...
#include "llvm_lib_Target_X86_X86AsmPrinter.h"
...
#include "llvm_include_llvm_CodeGen_MachineConstantPool.h"
#include "llvm_include_llvm_CodeGen_MachineModuleInfoImpls.h"
#include "llvm_include_llvm_CodeGen_TargetLoweringObjectFileImpl.h"
...
#include "llvm_include_llvm_MC_MCInstBuilder.h"
#include "llvm_include_llvm_MC_MCSectionCOFF.h"
#include "llvm_include_llvm_MC_MCSectionELF.h"
#include "llvm_include_llvm_MC_MCSectionMachO.h"
...
#include "llvm_include_llvm_Support_MachineValueType.h"
#include "llvm_include_llvm_Target_TargetMachine.h"

using namespace llvm;

X86AsmPrinter::X86AsmPrinter(TargetMachine& TM, std::unique_ptr<MCStreamer> Streamer) :
...
///
bool
X86AsmPrinter::runOnMachineFunction(MachineFunction& MF) {
...

    {
        ...
        if (TM.getTarget().MCCodeEmitterCtorFn) {
            emitter = TM.getTarget().MCCodeEmitterCtorFn(*Subtarget->getInstrInfo(), MF.getContext());
        }
        CodeEmitter.reset(emitter);
    }
    ...
    if (Subtarget->isTargetCOFF()) {
        ...
        OutStreamer->emitCOFFSymbolType(COFF::IMAGE_SYM_DTYPE_FUNCTION << COFF::SCT_COMPLEX_TYPE_SHIFT);
...
void
X86AsmPrinter::emitFunctionBodyEnd() {
    if (EmitFPOData) {
        auto* XTS =
            static_cast<X86TargetStreamer*>(OutStreamer->getTargetStreamer());
        XTS->emitFPOEndProc();
...
uint32_t
X86AsmPrinter::MaskKCFIType(uint32_t Value) {
...
void
X86AsmPrinter::EmitKCFITypePadding(const MachineFunction& MF, bool HasType) {
...
/// format.
void
X86AsmPrinter::emitKCFITypeId(const MachineFunction& MF) {
    ...
    if (MAI->hasDotTypeDotSizeDirective()) {
        ...
        const MCExpr* SizeExp = MCBinaryExpr::createSub(
            ...
            MCSymbolRefExpr::create(FnSym, OutContext),
            OutContext
        );
...
void
X86AsmPrinter::PrintSymbolOperand(const MachineOperand& MO, raw_ostream& O) {
    switch (MO.getType()) {
        ...
        case MachineOperand::MO_GlobalAddress: {
            ...
            MCSymbol* GVSym;
            if (MO.getTargetFlags() == X86II::MO_DARWIN_NONLAZY || MO.getTargetFlags() == X86II::MO_DARWIN_NONLAZY_PIC_BASE)
            ...
            if (MO.getTargetFlags() == X86II::MO_DARWIN_NONLAZY || MO.getTargetFlags() == X86II::MO_DARWIN_NONLAZY_PIC_BASE) {
                ...
                if (!StubSym.getPointer())
                    StubSym = MachineModuleInfoImpl::StubValueTy(getSymbol(GV), !GV->hasInternalLinkage());
            }
...
void
X86AsmPrinter::PrintOperand(const MachineInstr* MI, unsigned OpNo, raw_ostream& O) {
...
/// a register.
void
X86AsmPrinter::PrintModifiedOperand(const MachineInstr* MI, unsigned OpNo, raw_ostream& O, const char* Modifier) {
    const MachineOperand& MO = MI->getOperand(OpNo);
    ...
    if (strncmp(Modifier, "subreg", strlen("subreg")) == 0) {
        unsigned Size = (strcmp(Modifier + 6, "64") == 0) ? 64 : (strcmp(Modifier + 6, "32") == 0) ? 32
...
void
X86AsmPrinter::PrintPCRelImm(const MachineInstr* MI, unsigned OpNo, raw_ostream& O) {
    const MachineOperand& MO = MI->getOperand(OpNo);
    switch (MO.getType()) {
...
void
X86AsmPrinter::PrintLeaMemReference(const MachineInstr* MI, unsigned OpNo, raw_ostream& O, const char* Modifier) {
    ...
    if (HasBaseReg && Modifier && !strcmp(Modifier, "no-rip") && BaseReg.getReg() == X86::RIP)
        HasBaseReg = false;
...
static bool
isSimpleReturn(const MachineInstr& MI) {
    // We exclude all tail calls here which set both isReturn and isCall.
    return MI.getDesc().isReturn() && !MI.getDesc().isCall();
}

static bool
isIndirectBranchOrTailCall(const MachineInstr& MI) {
    unsigned Opc = MI.getOpcode();
    return MI.getDesc().isIndirectBranch() /*Make below code in a good shape*/ || Opc == X86::TAILJMPr || Opc == X86::TAILJMPm || Opc == X86::TAILJMPr64 || Opc == X86::TAILJMPm64 || Opc == X86::TCRETURNri || Opc == X86::TCRETURNmi || Opc == X86::TCRETURNri64 || Opc == X86::TCRETURNmi64 || Opc == X86::TAILJMPr64_REX || Opc == X86::TAILJMPm64_REX;
}

void
X86AsmPrinter::emitBasicBlockEnd(const MachineBasicBlock& MBB) {
    if (Subtarget->hardenSlsRet() || Subtarget->hardenSlsIJmp()) {
        auto I = MBB.getLastNonDebugInstr();
        if (I != MBB.end()) {
            if ((Subtarget->hardenSlsRet() && isSimpleReturn(*I)) || (Subtarget->hardenSlsIJmp() && isIndirectBranchOrTailCall(*I))) {
                MCInst TmpInst;
...
void
X86AsmPrinter::PrintMemReference(const MachineInstr* MI, unsigned OpNo, raw_ostream& O, const char* Modifier) {
...
void
X86AsmPrinter::PrintIntelMemReference(const MachineInstr* MI, unsigned OpNo, raw_ostream& O, const char* Modifier) {
    const MachineOperand& BaseReg = MI->getOperand(OpNo + X86::AddrBaseReg);
    unsigned              ScaleVal = MI->getOperand(OpNo + X86::AddrScaleAmt).getImm();
    const MachineOperand& IndexReg = MI->getOperand(OpNo + X86::AddrIndexReg);
    const MachineOperand& DispSpec = MI->getOperand(OpNo + X86::AddrDisp);
    ...
    if (Modifier && (DispSpec.isGlobal() || DispSpec.isSymbol()) && !strcmp(Modifier, "disp-only")) {
        HasBaseReg = false;
    }
...
static bool
printAsmMRegister(const X86AsmPrinter& P, const MachineOperand& MO, char Mode, raw_ostream& O) {
    Register Reg = MO.getReg();
    bool     EmitPercent = MO.getParent()->getInlineAsmDialect() == InlineAsm::AD_ATT;

    if (!X86::GR8RegClass.contains(Reg) && !X86::GR16RegClass.contains(Reg) && !X86::GR32RegClass.contains(Reg) && !X86::GR64RegClass.contains(Reg))
        return true;
...
static bool
printAsmVRegister(const MachineOperand& MO, char Mode, raw_ostream& O) {
    Register Reg = MO.getReg();
    bool     EmitPercent = MO.getParent()->getInlineAsmDialect() == InlineAsm::AD_ATT;
...
///
bool
X86AsmPrinter::PrintAsmOperand(const MachineInstr* MI, unsigned OpNo, const char* ExtraCode, raw_ostream& O) {
...
bool
X86AsmPrinter::PrintAsmMemoryOperand(const MachineInstr* MI, unsigned OpNo, const char* ExtraCode, raw_ostream& O) {
    if (ExtraCode && ExtraCode[0]) {
        if (ExtraCode[1] != 0)
            return true;  // Unknown modifier.
...
void
X86AsmPrinter::emitStartOfAsmFile(Module& M) {
    ...
    if (TT.isOSBinFormatCOFF()) {
        ...
        OutStreamer->emitAssignment(
            S,
            MCConstantExpr::create(Feat00Value, MMI->getContext())
        );
    }
...
static void
emitNonLazySymbolPointer(MCStreamer& OutStreamer, MCSymbol* StubLabel, MachineModuleInfoImpl::StubValueTy& MCSym) {
    // L_foo$stub:
...
static void
emitNonLazyStubs(MachineModuleInfo* MMI, MCStreamer& OutStreamer) {
...
void
X86AsmPrinter::emitEndOfAsmFile(Module& M) {
    const Triple& TT = TM.getTargetTriple();
...
AsmPrinter*
X86AsmPrinterCtorFn(TargetMachine& TM, std::unique_ptr<MCStreamer>&& Streamer) {
    return new X86AsmPrinter(TM, std::move(Streamer));
}

extern "C" LLVM_EXTERNAL_VISIBILITY void
LLVMInitializeX86AsmPrinter() {
    LLVMTargetRegistryTheTarget->AsmPrinterCtorFn = X86AsmPrinterCtorFn;
}
