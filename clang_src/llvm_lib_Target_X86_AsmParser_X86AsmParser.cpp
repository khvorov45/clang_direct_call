...
#include "llvm_include_llvm_MC_MCParser_MCParsedAsmOperand.h"
...
static bool
checkScale(unsigned Scale, StringRef& ErrMsg) {
...
class X86AsmParser: public MCTargetAsmParser {
  ...
  private:
    ...
    X86TargetStreamer&
    getTargetStreamer() {
        assert(getParser().getStreamer().getTargetStreamer() && "do not have a target streamer");
        MCTargetStreamer& TS = *getParser().getStreamer().getTargetStreamer();
        return static_cast<X86TargetStreamer&>(TS);
    }

    unsigned
    MatchInstruction(const OperandVector& Operands, MCInst& Inst, uint64_t& ErrorInfo, FeatureBitset& MissingFeatures, bool matchingInlineAsm, unsigned VariantID = 0) {
        // In Code16GCC mode, match as 32-bit.
        if (Code16GCC)
            SwitchMode(X86::Is32Bit);
        unsigned rv = MatchInstructionImpl(Operands, Inst, ErrorInfo, MissingFeatures, matchingInlineAsm, VariantID);
        if (Code16GCC)
    ...
    class InfixCalculator {
        ...
        bool
        isUnaryOperator(InfixCalculatorTok Op) const {
      ...
      public:
        ...
        void
        pushOperand(InfixCalculatorTok Op, int64_t Val = 0) {
        ...
        void
        pushOperator(InfixCalculatorTok Op) {
                ...
                if (Op.first == IC_IMM || Op.first == IC_REGISTER) {
                    ...
                    switch (Op.first) {
                        ...
                        case IC_MINUS:
                            ...
                            break;
                        case IC_MULTIPLY:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Multiply operation with an immediate and a register!");
                            Val = Op1.second * Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_DIVIDE:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Divide operation with an immediate and a register!");
                            assert(Op2.second != 0 && "Division by zero!");
                            Val = Op1.second / Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_MOD:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Modulo operation with an immediate and a register!");
                            Val = Op1.second % Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_OR:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Or operation with an immediate and a register!");
                            Val = Op1.second | Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_XOR:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Xor operation with an immediate and a register!");
                            Val = Op1.second ^ Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_AND:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "And operation with an immediate and a register!");
                            Val = Op1.second & Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_LSHIFT:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Left shift operation with an immediate and a register!");
                            Val = Op1.second << Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_RSHIFT:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Right shift operation with an immediate and a register!");
                            Val = Op1.second >> Op2.second;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_EQ:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Equals operation with an immediate and a register!");
                            Val = (Op1.second == Op2.second) ? -1 : 0;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_NE:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Not-equals operation with an immediate and a register!");
                            Val = (Op1.second != Op2.second) ? -1 : 0;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_LT:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Less-than operation with an immediate and a register!");
                            Val = (Op1.second < Op2.second) ? -1 : 0;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_LE:
                            ...
                            break;
                        case IC_GT:
                            assert(Op1.first == IC_IMM && Op2.first == IC_IMM && "Greater-than operation with an immediate and a register!");
                            Val = (Op1.second > Op2.second) ? -1 : 0;
                            OperandStack.push_back(std::make_pair(IC_IMM, Val));
                            break;
                        case IC_GE:
    ...
    class IntelExprStateMachine {
        ...
        bool
        setSymRef(const MCExpr* Val, StringRef ID, StringRef& ErrMsg) {
            if (Sym) {
      ...
      public:
        ...
        getIdentifierInfo() const {
            return Info;
        }

        bool
        regsUseUpError(StringRef& ErrMsg) {
        ...
        onMinus(StringRef& ErrMsg) {
            ...
            switch (State) {
                ...
                case IES_OFFSET:
                    ...
                    if (CurrState == IES_REGISTER || CurrState == IES_RPAREN || CurrState == IES_INTEGER || CurrState == IES_RBRAC || CurrState == IES_OFFSET)
        ...
        bool
        onRegister(unsigned Reg, StringRef& ErrMsg) {
            IntelExprState CurrState = State;
            switch (State) {
                default:
                    State = IES_ERROR;
        ...
        bool
        onIdentifierExpr(const MCExpr* SymRef, StringRef SymRefName, const InlineAsmIdentifierInfo& IDInfo, const AsmTypeInfo& Type, bool ParsingMSInlineAsm, StringRef& ErrMsg) {
        ...
        bool
        onInteger(int64_t TmpInt, StringRef& ErrMsg) {
            IntelExprState CurrState = State;
            switch (State) {
                default:
                    State = IES_ERROR;
        ...
        bool
        onOffset(const MCExpr* Val, SMLoc OffsetLoc, StringRef ID, const InlineAsmIdentifierInfo& IDInfo, bool ParsingMSInlineAsm, StringRef& ErrMsg) {
            PrevState = State;
            switch (State) {
                default:
        ...
        void
        setTypeInfo(AsmTypeInfo Type) {
            CurType = Type;
        }
    };

    bool
    Error(SMLoc L, const Twine& Msg, SMRange Range = std::nullopt, bool MatchingInlineAsm = false) {
        MCAsmParser& Parser = getParser();
    ...
    bool MatchRegisterByName(MCRegister& RegNo, StringRef RegName, SMLoc StartLoc, SMLoc EndLoc);
    ...
    void
             AddDefaultSrcDestOperands(OperandVector& Operands, std::unique_ptr<llvm::MCParsedAsmOperand>&& Src, std::unique_ptr<llvm::MCParsedAsmOperand>&& Dst);
    bool     VerifyAndAdjustOperands(OperandVector& OrigOperands, OperandVector& FinalOperands);
    ...
    bool     ParseIntelOffsetOperator(const MCExpr*& Val, StringRef& ID, InlineAsmIdentifierInfo& Info, SMLoc& End);
    ...
    bool     ParseIntelNamedOperator(StringRef Name, IntelExprStateMachine& SM, bool& ParseError, SMLoc& End);
    bool     ParseMasmNamedOperator(StringRef Name, IntelExprStateMachine& SM, bool& ParseError, SMLoc& End);
    void     RewriteIntelExpression(IntelExprStateMachine& SM, SMLoc Start, SMLoc End);
    bool     ParseIntelExpression(IntelExprStateMachine& SM, SMLoc& End);
    bool     ParseIntelInlineAsmIdentifier(const MCExpr*& Val, StringRef& Identifier, InlineAsmIdentifierInfo& Info, bool IsUnevaluatedOperand, SMLoc& End, bool IsParsingOffsetOperator = false);
    void     tryParseOperandIdx(AsmToken::TokenKind PrevTK, IntelExprStateMachine& SM);

    bool ParseMemOperand(unsigned SegReg, const MCExpr* Disp, SMLoc StartLoc, SMLoc EndLoc, OperandVector& Operands);

    X86::CondCode ParseConditionCode(StringRef CCode);

    bool ParseIntelMemoryOperandSize(unsigned& Size);
    bool CreateMemForMSInlineAsm(unsigned SegReg, const MCExpr* Disp, unsigned BaseReg, unsigned IndexReg, unsigned Scale, SMLoc Start, SMLoc End, unsigned Size, StringRef Identifier, const InlineAsmIdentifierInfo& Info, OperandVector& Operands);
    ...
    void MatchFPUWaitAlias(SMLoc IDLoc, X86Operand& Op, OperandVector& Operands, MCStreamer& Out, bool MatchingInlineAsm);

    bool ErrorMissingFeature(SMLoc IDLoc, const FeatureBitset& MissingFeatures, bool MatchingInlineAsm);

    bool MatchAndEmitATTInstruction(SMLoc IDLoc, unsigned& Opcode, OperandVector& Operands, MCStreamer& Out, uint64_t& ErrorInfo, bool MatchingInlineAsm);

    bool MatchAndEmitIntelInstruction(SMLoc IDLoc, unsigned& Opcode, OperandVector& Operands, MCStreamer& Out, uint64_t& ErrorInfo, bool MatchingInlineAsm);
...
#include "X86GenAsmMatcher.inc"
    ...
    X86AsmParser(const MCSubtargetInfo& sti, MCAsmParser& Parser, const MCInstrInfo& mii, const MCTargetOptions& Options) :
...
static bool
CheckBaseRegAndIndexRegAndScale(unsigned BaseReg, unsigned IndexReg, unsigned Scale, bool Is64BitMode, StringRef& ErrMsg) {
    // If we have both a base register and an index register make sure they are
    // both 64-bit or 32-bit registers.
    // To support VSIB, IndexReg can be 128-bit or 256-bit registers.

    if (BaseReg != 0 && !(BaseReg == X86::RIP || BaseReg == X86::EIP || X86MCRegisterClasses[X86::GR16RegClassID].contains(BaseReg) || X86MCRegisterClasses[X86::GR32RegClassID].contains(BaseReg) || X86MCRegisterClasses[X86::GR64RegClassID].contains(BaseReg))) {
        ErrMsg = "invalid base+index expression";
        return true;
    }

    if (IndexReg != 0 && !(IndexReg == X86::EIZ || IndexReg == X86::RIZ || X86MCRegisterClasses[X86::GR16RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::GR32RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::GR64RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::VR128XRegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::VR256XRegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::VR512RegClassID].contains(IndexReg))) {
        ErrMsg = "invalid base+index expression";
        return true;
    }

    if (((BaseReg == X86::RIP || BaseReg == X86::EIP) && IndexReg != 0) || IndexReg == X86::EIP || IndexReg == X86::RIP || IndexReg == X86::ESP || IndexReg == X86::RSP) {
        ErrMsg = "invalid base+index expression";
        return true;
    }
    ...
    if (X86MCRegisterClasses[X86::GR16RegClassID].contains(BaseReg) && (Is64BitMode || (BaseReg != X86::BX && BaseReg != X86::BP && BaseReg != X86::SI && BaseReg != X86::DI))) {
        ErrMsg = "invalid 16-bit base register";
        return true;
    }

    if (BaseReg == 0 && X86MCRegisterClasses[X86::GR16RegClassID].contains(IndexReg)) {
        ErrMsg = "16-bit memory operand may not include only index register";
        return true;
    }

    if (BaseReg != 0 && IndexReg != 0) {
        if (X86MCRegisterClasses[X86::GR64RegClassID].contains(BaseReg) && (X86MCRegisterClasses[X86::GR16RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::GR32RegClassID].contains(IndexReg) || IndexReg == X86::EIZ)) {
            ErrMsg = "base register is 64-bit, but index register is not";
            return true;
        }
        if (X86MCRegisterClasses[X86::GR32RegClassID].contains(BaseReg) && (X86MCRegisterClasses[X86::GR16RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::GR64RegClassID].contains(IndexReg) || IndexReg == X86::RIZ)) {
            ErrMsg = "base register is 32-bit, but index register is not";
            return true;
        }
        if (X86MCRegisterClasses[X86::GR16RegClassID].contains(BaseReg)) {
            if (X86MCRegisterClasses[X86::GR32RegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::GR64RegClassID].contains(IndexReg)) {
                ErrMsg = "base register is 16-bit, but index register is not";
                return true;
            }
            if ((BaseReg != X86::BX && BaseReg != X86::BP) || (IndexReg != X86::SI && IndexReg != X86::DI)) {
    ...
    if (!Is64BitMode && BaseReg != 0 && (BaseReg == X86::RIP || BaseReg == X86::EIP)) {
...
bool
X86AsmParser::MatchRegisterByName(MCRegister& RegNo, StringRef RegName, SMLoc StartLoc, SMLoc EndLoc) {
    ...
    if (isParsingMSInlineAsm() && isParsingIntelSyntax() && (RegNo == X86::EFLAGS || RegNo == X86::MXCSR))
        RegNo = 0;
    ...
    if (!is64BitMode()) {
        ...
        // checked.
        if (RegNo == X86::RIZ || RegNo == X86::RIP || X86MCRegisterClasses[X86::GR64RegClassID].contains(RegNo) || X86II::isX86_64NonExtLowByteReg(RegNo) || X86II::isX86_64ExtendedReg(RegNo)) {
            return Error(StartLoc, "register %" + RegName + " is only available in 64-bit mode", SMRange(StartLoc, EndLoc));
        }
    }
...
bool
X86AsmParser::ParseRegister(MCRegister& RegNo, SMLoc& StartLoc, SMLoc& EndLoc, bool RestoreOnFailure) {
    MCAsmParser& Parser = getParser();
...
bool
X86AsmParser::parseRegister(MCRegister& RegNo, SMLoc& StartLoc, SMLoc& EndLoc) {
    return ParseRegister(RegNo, StartLoc, EndLoc, /*RestoreOnFailure=*/false);
}

OperandMatchResultTy
X86AsmParser::tryParseRegister(MCRegister& RegNo, SMLoc& StartLoc, SMLoc& EndLoc) {
    bool Result =
...
unsigned
X86AsmParser::GetSIDIForRegClass(unsigned RegClassID, unsigned Reg, bool IsSIReg) {
    switch (RegClassID) {
...
void
X86AsmParser::AddDefaultSrcDestOperands(
...
bool
X86AsmParser::VerifyAndAdjustOperands(OperandVector& OrigOperands, OperandVector& FinalOperands) {
    if (OrigOperands.size() > 1) {
        // Check if sizes match, OrigOperands also contains the instruction name
        assert(OrigOperands.size() == FinalOperands.size() + 1 && "Operand size mismatch");
        ...
        for (unsigned int i = 0; i < FinalOperands.size(); ++i) {
            ...
            if (FinalOp.isReg() && (!OrigOp.isReg() || FinalOp.getReg() != OrigOp.getReg()))
            ...
            if (FinalOp.isMem()) {
                ...
                if (RegClassID != -1 && !X86MCRegisterClasses[RegClassID].contains(OrigReg)) {
                    return Error(OrigOp.getStartLoc(), "mismatching source and destination index registers");
                }
                ...
                if (FinalReg != OrigReg) {
                    ...
                    Warnings.push_back(std::make_pair(
                        OrigOp.getStartLoc(),
                        "memory operand is only for determining the size, " + RegName + " will be used for the location"
                    ));
                }
...
bool
X86AsmParser::parseOperand(OperandVector& Operands, StringRef Name) {
    if (isParsingIntelSyntax())
...
bool
X86AsmParser::CreateMemForMSInlineAsm(
    unsigned                       SegReg,
    const MCExpr*                  Disp,
    unsigned                       BaseReg,
    unsigned                       IndexReg,
    ...
    if (Info.isKind(InlineAsmIdentifierInfo::IK_Label)) {
        ...
        Operands.push_back(X86Operand::CreateMem(getPointerWidth(), Disp, Start, End, Size, Identifier, Info.Label.Decl));
        return false;
    }
    ...
    if (IsGlobalLV && (BaseReg || IndexReg)) {
        Operands.push_back(X86Operand::CreateMem(getPointerWidth(), Disp, Start, End, Size, Identifier, Decl, 0, BaseReg && IndexReg));
        return false;
    }
...
bool
X86AsmParser::ParseIntelNamedOperator(StringRef Name, IntelExprStateMachine& SM, bool& ParseError, SMLoc& End) {
    // A named operator should be either lower or upper case, but not a mix...
    // except in MASM, which uses full case-insensitivity.
    if (Name.compare(Name.lower()) && Name.compare(Name.upper()) && !getParser().isParsingMasm())
        return false;
...
bool
X86AsmParser::ParseMasmNamedOperator(StringRef Name, IntelExprStateMachine& SM, bool& ParseError, SMLoc& End) {
...
void
X86AsmParser::tryParseOperandIdx(AsmToken::TokenKind PrevTK, IntelExprStateMachine& SM) {
        ...
        switch (TK) {
            ...
            case AsmToken::Identifier: {
                ...
                if (Parser.isParsingMasm()) {
                    ...
                    if (NextTok.is(AsmToken::Identifier) && NextTok.getIdentifier().equals_insensitive("ptr")) {
                        AsmTypeInfo Info;
                ...
                if (Tok.is(AsmToken::Identifier)) {
                    ...
                    if (Parser.isParsingMasm()) {
                        ...
                        if (!Field.empty() && !MatchRegisterByName(Reg, ID, IdentLoc, IDEndLoc)) {
                            if (SM.onRegister(Reg, ErrMsg))
                                return Error(IdentLoc, ErrMsg);
                ...
                if (Parser.isParsingMasm() && ParseMasmNamedOperator(Identifier, SM, ParseError, End)) {
                    if (ParseError)
                        return true;
                    break;
                }
                ...
                if (isParsingMSInlineAsm() || Parser.isParsingMasm()) {
                    ...
                    if (Identifier.count('.') && (PrevTK == AsmToken::RBrac || PrevTK == AsmToken::RParen)) {
                        if (ParseIntelDotOperator(SM, End))
                            return true;
                        break;
                    }
                ...
                if (isParsingMSInlineAsm()) {
                    ...
                    if (ParseIntelInlineAsmIdentifier(Val, Identifier, Info, false, End))
                        return true;
                    else if (SM.onIdentifierExpr(Val, Identifier, Info, FieldInfo.Type, true, ErrMsg))
                        return Error(IdentLoc, ErrMsg);
                    break;
                }
                if (Parser.isParsingMasm()) {
                    ...
                    if (!getParser().lookUpType(Identifier, FieldInfo.Type)) {
                        ...
                        while (EndDot || (getTok().is(AsmToken::Identifier) && getTok().getString().startswith("."))) {
                            getParser().parseIdentifier(Identifier);
                            if (!EndDot)
                                Identifier.consume_front(".");
                            EndDot = Identifier.consume_back(".");
                            if (getParser().lookUpField(FieldInfo.Type.Name, Identifier, FieldInfo)) {
                                SMLoc IDEnd =
                                    SMLoc::getFromPointer(Identifier.data() + Identifier.size());
                                return Error(IdentLoc, "Unable to lookup field reference!", SMRange(IdentLoc, IDEnd));
                            }
                            if (!EndDot)
                ...
                } else if (SM.onIdentifierExpr(Val, Identifier, Info, FieldInfo.Type, false, ErrMsg)) {
                    return Error(IdentLoc, ErrMsg);
                }
                break;
            }
            case AsmToken::Integer: {
                ...
                if (getLexer().getKind() == AsmToken::Identifier) {
                    ...
                    if (IDVal == "f" || IDVal == "b") {
                        ...
                        InlineAsmIdentifierInfo Info;
                        AsmTypeInfo             Type;
                        if (SM.onIdentifierExpr(Val, Identifier, Info, Type, isParsingMSInlineAsm(), ErrMsg))
...
void
X86AsmParser::RewriteIntelExpression(IntelExprStateMachine& SM, SMLoc Start, SMLoc End) {
    SMLoc    Loc = Start;
    ...
    // Emit it
    IntelExpr Expr(BaseRegStr, IndexRegStr, SM.getScale(), OffsetNameStr, SM.getImm(), SM.isMemExpr());
    InstInfo->AsmRewrites->emplace_back(Loc, ExprLen, Expr);
}

// Inline assembly may use variable names with namespace alias qualifiers.
bool
X86AsmParser::ParseIntelInlineAsmIdentifier(
    const MCExpr*&           Val,
    StringRef&               Identifier,
    InlineAsmIdentifierInfo& Info,
    ...
    // failed parsing.
    assert((End.getPointer() == EndPtr || Info.isKind(InlineAsmIdentifierInfo::IK_Invalid)) && "frontend claimed part of a token?");

    // If the identifier lookup was unsuccessful, assume that we are dealing with
    // a label.
    if (Info.isKind(InlineAsmIdentifierInfo::IK_Invalid)) {
        StringRef InternalName =
            SemaCallback->LookupInlineAsmLabel(Identifier, getSourceManager(), Loc, false);
...
bool
X86AsmParser::ParseRoundingModeOp(SMLoc Start, OperandVector& Operands) {
    MCAsmParser&    Parser = getParser();
    const AsmToken& Tok = Parser.getTok();
...
/// Parse the '.' operator.
bool
X86AsmParser::ParseIntelDotOperator(IntelExprStateMachine& SM, SMLoc& End) {
    ...
    if (Tok.is(AsmToken::Real)) {
    ...
    } else if ((isParsingMSInlineAsm() || getParser().isParsingMasm()) && Tok.is(AsmToken::Identifier)) {
        ...
        if (getParser().lookUpField(SM.getType(), DotDispStr, Info) && getParser().lookUpField(SM.getSymName(), DotDispStr, Info) && getParser().lookUpField(DotDispStr, Info) && (!SemaCallback || SemaCallback->LookupInlineAsmField(Base, Member, Info.Offset)))
...
bool
X86AsmParser::ParseIntelOffsetOperator(const MCExpr*& Val, StringRef& ID, InlineAsmIdentifierInfo& Info, SMLoc& End) {
    // Eat offset, mark start of identifier.
    SMLoc Start = Lex().getLoc();
    ID = getTok().getString();
    if (!isParsingMSInlineAsm()) {
        if ((getTok().isNot(AsmToken::Identifier) && getTok().isNot(AsmToken::String)) || getParser().parsePrimaryExpr(Val, End, nullptr))
...
unsigned
X86AsmParser::IdentifyIntelInlineAsmOperator(StringRef Name) {
...
unsigned
X86AsmParser::ParseIntelInlineAsmOperator(unsigned OpKind) {
    MCAsmParser&    Parser = getParser();
    const AsmToken& Tok = Parser.getTok();
...
// Report back its kind, or IOK_INVALID if does not evaluated as a known one
unsigned
X86AsmParser::IdentifyMasmOperator(StringRef Name) {
...
bool
X86AsmParser::ParseMasmOperator(unsigned OpKind, int64_t& Val) {
    MCAsmParser& Parser = getParser();
    ...
    if (OpKind == MOK_SIZEOF || OpKind == MOK_TYPE) {
        ...
        AsmTypeInfo     Type;
        if (IDTok.is(AsmToken::Identifier) && !Parser.lookUpType(IDTok.getIdentifier(), Type)) {
            Val = Type.Size;
...
bool
X86AsmParser::ParseIntelMemoryOperandSize(unsigned& Size) {
...
bool
X86AsmParser::parseIntelOperand(OperandVector& Operands, StringRef Name) {
    MCAsmParser&    Parser = getParser();
    const AsmToken& Tok = Parser.getTok();
    SMLoc           Start, End;
    ...
    if (!SM.isMemExpr() && !RegNo) {
        if (isParsingMSInlineAsm() && SM.isOffsetOperator()) {
            ...
            if (Info.isKind(InlineAsmIdentifierInfo::IK_Var)) {
                ...
                // for later handling.
                Operands.push_back(X86Operand::CreateImm(Disp, Start, End, SM.getSymName(), Info.Var.Decl, Info.Var.IsGlobalLV));
                return false;
            }
        }
    ...
    if (Scale == 0 && BaseReg != X86::ESP && BaseReg != X86::RSP && (IndexReg == X86::ESP || IndexReg == X86::RSP))
    ...
    if (Scale == 0 && !(X86MCRegisterClasses[X86::VR128XRegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::VR256XRegClassID].contains(IndexReg) || X86MCRegisterClasses[X86::VR512RegClassID].contains(IndexReg)) && (X86MCRegisterClasses[X86::VR128XRegClassID].contains(BaseReg) || X86MCRegisterClasses[X86::VR256XRegClassID].contains(BaseReg) || X86MCRegisterClasses[X86::VR512RegClassID].contains(BaseReg)))
        std::swap(BaseReg, IndexReg);

    if (Scale != 0 && X86MCRegisterClasses[X86::GR16RegClassID].contains(IndexReg))
    ...
    if ((BaseReg == X86::SI || BaseReg == X86::DI) && (IndexReg == X86::BX || IndexReg == X86::BP))
        std::swap(BaseReg, IndexReg);

    if ((BaseReg || IndexReg) && CheckBaseRegAndIndexRegAndScale(BaseReg, IndexReg, Scale, is64BitMode(), ErrMsg))
    ...
    if (Parser.isParsingMasm()) {
        ...
        if (IsUnconditionalBranch) {
            if (PtrInOperand) {
                ...
                if (is64BitMode())
                    DefaultBaseReg = X86::RIP;
            } else if (!BaseReg && !IndexReg && Disp && Disp->getKind() == MCExpr::SymbolRef) {
                if (is64BitMode()) {
                    if (SM.getSize() == 8) {
                        MaybeDirectBranchDest = false;
                        DefaultBaseReg = X86::RIP;
...
bool
X86AsmParser::parseATTOperand(OperandVector& Operands) {
    MCAsmParser& Parser = getParser();
    switch (getLexer().getKind()) {
        case AsmToken::Dollar: {
            ...
            if (check(getLexer().is(AsmToken::Percent), L, "expected immediate expression") || getParser().parseExpression(Val, End) || check(isa<X86MCExpr>(Val), L, "expected immediate expression"))
                return true;
        ...
        default: {
            ...
            if (getLexer().isNot(AsmToken::LParen)) {
                ...
                if (auto* RE = dyn_cast<X86MCExpr>(Expr)) {
                    ...
                    if (Reg == X86::RIP)
                        return Error(Loc, "%rip can only be used as a base register", SMRange(Loc, EndLoc));
...
bool
X86AsmParser::ParseZ(std::unique_ptr<X86Operand>& Z, const SMLoc& StartLoc) {
    MCAsmParser& Parser = getParser();
...
// true on failure, false otherwise
bool
X86AsmParser::HandleAVX512Operand(OperandVector& Operands) {
    MCAsmParser& Parser = getParser();
    if (getLexer().is(AsmToken::LCurly)) {
        // Eat "{" and mark the current place.
        const SMLoc consumedToken = consumeToken();
        ...
        if (getLexer().is(AsmToken::Integer)) {
            ...
            Parser.Lex();  // Eat "}"
            Operands.push_back(X86Operand::CreateToken(BroadcastPrimitive, consumedToken));
            ...
            if (!Z || getLexer().is(AsmToken::LCurly)) {
                ...
                MCRegister RegNo;
                SMLoc      RegLoc;
                if (!parseRegister(RegNo, RegLoc, StartLoc) && X86MCRegisterClasses[X86::VK1RegClassID].contains(RegNo)) {
                    if (RegNo == X86::K0)
                    ...
                    return Error(getLexer().getLoc(), "Expected an op-mask register at this point");
                ...
                if (getLexer().is(AsmToken::LCurly) && !Z) {
                    ...
                    if (ParseZ(Z, consumeToken()) || !Z)
                        return Error(getLexer().getLoc(), "Expected a {z} mark at this point");
                }
...
bool
X86AsmParser::ParseMemOperand(unsigned SegReg, const MCExpr* Disp, SMLoc StartLoc, SMLoc EndLoc, OperandVector& Operands) {
    MCAsmParser& Parser = getParser();
    SMLoc        Loc;
    ...
    auto isAtMemOperand = [this]() {
        ...
        switch (Buf[0].getKind()) {
            ...
            case AsmToken::At:
            case AsmToken::Dollar:
                if ((TokCount > 1) && (Buf[1].is(AsmToken::Identifier) || Buf[1].is(AsmToken::String)) && (Buf[0].getLoc().getPointer() + 1 == Buf[1].getLoc().getPointer()))
                    Id = StringRef(Buf[0].getLoc().getPointer(), Buf[1].getIdentifier().size() + 1);
                break;
            case AsmToken::Identifier:
            case AsmToken::String:
    ...
    if (!parseOptionalToken(AsmToken::LParen)) {
        if (SegReg == 0)
            ...
            Operands.push_back(X86Operand::CreateMem(getPointerWidth(), SegReg, Disp, 0, 0, 1, StartLoc, EndLoc));
        return false;
    }
    ...
    if (getLexer().isNot(AsmToken::Comma) && getLexer().isNot(AsmToken::RParen)) {
        if (Parser.parseExpression(E, EndLoc) || check(!isa<X86MCExpr>(E), BaseLoc, "expected register here"))
            return true;

        // Check the register.
        BaseReg = cast<X86MCExpr>(E)->getRegNo();
        if (BaseReg == X86::EIZ || BaseReg == X86::RIZ)
            return Error(BaseLoc, "eiz and riz can only be used as index registers", SMRange(BaseLoc, EndLoc));
    }
    ...
    if (parseOptionalToken(AsmToken::Comma)) {
        ...
        if (getLexer().isNot(AsmToken::RParen)) {
            ...
            if (!isa<X86MCExpr>(E)) {
                ...
                if (BaseReg == X86::RIP)
                    return Error(Loc, "%rip as base register can not have an index register");
                if (IndexReg == X86::RIP)
                ...
                if (parseOptionalToken(AsmToken::Comma)) {
                    ...
                    if (getLexer().isNot(AsmToken::RParen)) {
                        int64_t ScaleVal;
                        if (Parser.parseTokenLoc(Loc) || Parser.parseAbsoluteExpression(ScaleVal))
                            return Error(Loc, "expected scale expression");
                        Scale = (unsigned)ScaleVal;
                        // Validate the scale amount.
                        if (X86MCRegisterClasses[X86::GR16RegClassID].contains(BaseReg) && Scale != 1)
    ...
    if (BaseReg == X86::DX && IndexReg == 0 && Scale == 1 && SegReg == 0 && isa<MCConstantExpr>(Disp) && cast<MCConstantExpr>(Disp)->getValue() == 0) {
        Operands.push_back(X86Operand::CreateDXReg(BaseLoc, BaseLoc));
        return false;
    }

    if (CheckBaseRegAndIndexRegAndScale(BaseReg, IndexReg, Scale, is64BitMode(), ErrMsg))
        return Error(BaseLoc, ErrMsg);

    if (SegReg || BaseReg || IndexReg)
        Operands.push_back(X86Operand::CreateMem(getPointerWidth(), SegReg, Disp, BaseReg, IndexReg, Scale, StartLoc, EndLoc));
    else
        Operands.push_back(
            X86Operand::CreateMem(getPointerWidth(), Disp, StartLoc, EndLoc)
        );
...
bool
X86AsmParser::parsePrimaryExpr(const MCExpr*& Res, SMLoc& EndLoc) {
    MCAsmParser& Parser = getParser();
    // See if this is a register first.
    if (getTok().is(AsmToken::Percent) || (isParsingIntelSyntax() && getTok().is(AsmToken::Identifier) && MatchRegisterName(Parser.getTok().getString()))) {
...
bool
X86AsmParser::ParseInstruction(ParseInstructionInfo& Info, StringRef Name, SMLoc NameLoc, OperandVector& Operands) {
    MCAsmParser& Parser = getParser();
    InstInfo = &Info;
    ...
    if (isParsingIntelSyntax() && (PatchedName == "jmp" || PatchedName == "jc" || PatchedName == "jnc" || PatchedName == "jcxz" || PatchedName == "jecxz" || (PatchedName.startswith("j") && ParseConditionCode(PatchedName.substr(1)) != X86::COND_INVALID))) {
        ...
        if (Parser.isParsingMasm() ? NextTok.equals_insensitive("short")
            ...
            InstInfo->AsmRewrites->emplace_back(AOK_Skip, NameEndLoc, NextTok.size() + 1);
        }
    }

    // FIXME: Hack to recognize setneb as setne.
    if (PatchedName.startswith("set") && PatchedName.endswith("b") && PatchedName != "setb" && PatchedName != "setnb")
        PatchedName = PatchedName.substr(0, Name.size() - 1);

    unsigned ComparisonPredicate = ~0U;

    // FIXME: Hack to recognize cmp<comparison code>{sh,ss,sd,ph,ps,pd}.
    if ((PatchedName.startswith("cmp") || PatchedName.startswith("vcmp")) && (PatchedName.endswith("ss") || PatchedName.endswith("sd") || PatchedName.endswith("sh") || PatchedName.endswith("ph") || PatchedName.endswith("ps") || PatchedName.endswith("pd"))) {
        ...
        unsigned CC = StringSwitch<unsigned>(
                          ...
                          .Case("true_us", 0x1F)
                          .Default(~0U);
        if (CC != ~0U && (IsVCMP || CC < 8) && (IsVCMP || PatchedName.back() != 'h')) {
    ...
    if (PatchedName.startswith("vpcmp") && (PatchedName.back() == 'b' || PatchedName.back() == 'w' || PatchedName.back() == 'd' || PatchedName.back() == 'q')) {
    ...
    if (PatchedName.startswith("vpcom") && (PatchedName.back() == 'b' || PatchedName.back() == 'w' || PatchedName.back() == 'd' || PatchedName.back() == 'q')) {
        unsigned SuffixSize = PatchedName.drop_back().back() == 'u' ? 2 : 1;
        unsigned CC = StringSwitch<unsigned>(
                          PatchedName.slice(5, PatchedName.size() - SuffixSize)
        )
    ...
    if (ComparisonPredicate != ~0U && !isParsingIntelSyntax()) {
        const MCExpr* ImmOp = MCConstantExpr::create(ComparisonPredicate, getParser().getContext());
    ...
    if (getLexer().isNot(AsmToken::EndOfStatement) && !IsPrefix) {
        ...
        CurlyAsEndOfStatement =
            isParsingIntelSyntax() && isParsingMSInlineAsm() && (getLexer().is(AsmToken::LCurly) || getLexer().is(AsmToken::RCurly));
    ...
    if (getLexer().is(AsmToken::EndOfStatement) || (IsPrefix && getLexer().is(AsmToken::Slash)))
        ...
        Info.AsmRewrites->emplace_back(AOK_EndOfStatement, getLexer().getTok().getLoc(), 0);
    ...
    if ((Name == "mov" || Name == "movw" || Name == "movl") && (Operands.size() == 3)) {
        ...
        if (Op1.isReg() && Op2.isReg() && X86MCRegisterClasses[X86::SEGMENT_REGRegClassID].contains(Op2.getReg()) && (X86MCRegisterClasses[X86::GR16RegClassID].contains(Op1.getReg()) || X86MCRegisterClasses[X86::GR32RegClassID].contains(Op1.getReg()))) {
    ...
    if ((Name == "outb" || Name == "outsb" || Name == "outw" || Name == "outsw" || Name == "outl" || Name == "outsl" || Name == "out" || Name == "outs") && Operands.size() == 3) {
        X86Operand& Op = (X86Operand&)*Operands.back();
        if (Op.isDXReg())
            Operands.back() = X86Operand::CreateReg(X86::DX, Op.getStartLoc(), Op.getEndLoc());
    }
    // Same hack for "in[s]?[bwl]? (%dx), %al" -> "inb %dx, %al".
    if ((Name == "inb" || Name == "insb" || Name == "inw" || Name == "insw" || Name == "inl" || Name == "insl" || Name == "in" || Name == "ins") && Operands.size() == 3) {
        X86Operand& Op = (X86Operand&)*Operands[1];
        if (Op.isDXReg())
            Operands[1] = X86Operand::CreateReg(X86::DX, Op.getStartLoc(), Op.getEndLoc());
    }
    ...
    if (Name.startswith("ins") && (Operands.size() == 1 || Operands.size() == 3) && (Name == "insb" || Name == "insw" || Name == "insl" || Name == "insd" || Name == "ins")) {
        AddDefaultSrcDestOperands(TmpOperands, X86Operand::CreateReg(X86::DX, NameLoc, NameLoc), DefaultMemDIOperand(NameLoc));
        HadVerifyError = VerifyAndAdjustOperands(Operands, TmpOperands);
    }

    // Append default arguments to "outs[bwld]"
    if (Name.startswith("outs") && (Operands.size() == 1 || Operands.size() == 3) && (Name == "outsb" || Name == "outsw" || Name == "outsl" || Name == "outsd" || Name == "outs")) {
        AddDefaultSrcDestOperands(TmpOperands, DefaultMemSIOperand(NameLoc), X86Operand::CreateReg(X86::DX, NameLoc, NameLoc));
        HadVerifyError = VerifyAndAdjustOperands(Operands, TmpOperands);
    }
    ...
    if (Name.startswith("lods") && (Operands.size() == 1 || Operands.size() == 2) && (Name == "lods" || Name == "lodsb" || Name == "lodsw" || Name == "lodsl" || Name == "lodsd" || Name == "lodsq")) {
    ...
    // could be achieved with InstAlias in the tables.
    if (Name.startswith("stos") && (Operands.size() == 1 || Operands.size() == 2) && (Name == "stos" || Name == "stosb" || Name == "stosw" || Name == "stosl" || Name == "stosd" || Name == "stosq")) {
    ...
    // values of $DIREG according to the mode. It would be nice if this
    // could be achieved with InstAlias in the tables.
    if (Name.startswith("scas") && (Operands.size() == 1 || Operands.size() == 2) && (Name == "scas" || Name == "scasb" || Name == "scasw" || Name == "scasl" || Name == "scasd" || Name == "scasq")) {
        TmpOperands.push_back(DefaultMemDIOperand(NameLoc));
        HadVerifyError = VerifyAndAdjustOperands(Operands, TmpOperands);
    }

    // Add default SI and DI operands to "cmps[bwlq]".
    if (Name.startswith("cmps") && (Operands.size() == 1 || Operands.size() == 3) && (Name == "cmps" || Name == "cmpsb" || Name == "cmpsw" || Name == "cmpsl" || Name == "cmpsd" || Name == "cmpsq")) {
        AddDefaultSrcDestOperands(TmpOperands, DefaultMemDIOperand(NameLoc), DefaultMemSIOperand(NameLoc));
        HadVerifyError = VerifyAndAdjustOperands(Operands, TmpOperands);
    }

    // Add default SI and DI operands to "movs[bwlq]".
    if (((Name.startswith("movs") && (Name == "movs" || Name == "movsb" || Name == "movsw" || Name == "movsl" || Name == "movsd" || Name == "movsq")) || (Name.startswith("smov") && (Name == "smov" || Name == "smovb" || Name == "smovw" || Name == "smovl" || Name == "smovd" || Name == "smovq"))) && (Operands.size() == 1 || Operands.size() == 3)) {
        if (Name == "movsd" && Operands.size() == 1 && !isParsingIntelSyntax())
            Operands.back() = X86Operand::CreateToken("movsl", NameLoc);
        AddDefaultSrcDestOperands(TmpOperands, DefaultMemSIOperand(NameLoc), DefaultMemDIOperand(NameLoc));
        HadVerifyError = VerifyAndAdjustOperands(Operands, TmpOperands);
    }
...
bool
X86AsmParser::processInstruction(MCInst& Inst, const OperandVector& Ops) {
    ...
    switch (Inst.getOpcode()) {
        ...
        case X86::VMOVUPSYrr: {
            ...
            if (ForcedVEXEncoding == VEXEncoding_VEX3 || MRI->getEncodingValue(Inst.getOperand(0).getReg()) >= 8 || MRI->getEncodingValue(Inst.getOperand(1).getReg()) < 8)
                return false;
        ...
        case X86::VMOVSSrr: {
            // We can get a smaller encoding by using VEX.R instead of VEX.B if one of
            // the registers is extended, but other isn't.
            if (ForcedVEXEncoding == VEXEncoding_VEX3 || MRI->getEncodingValue(Inst.getOperand(0).getReg()) >= 8 || MRI->getEncodingValue(Inst.getOperand(2).getReg()) < 8)
                return false;
        ...
        case X86::SHL64mi: {
            // Optimize s{hr,ar,hl} $1, <op> to "shift <op>". Similar for rotate.
            // FIXME: It would be great if we could just do this with an InstAlias.
            if (!Inst.getOperand(X86::AddrNumOperands).isImm() || Inst.getOperand(X86::AddrNumOperands).getImm() != 1)
                return false;
...
bool
X86AsmParser::validateInstruction(MCInst& Inst, const OperandVector& Ops) {
    using namespace X86;
    const MCRegisterInfo* MRI = getContext().getRegisterInfo();
    unsigned              Opcode = Inst.getOpcode();
    uint64_t              TSFlags = MII.get(Opcode).TSFlags;
    if (isVFCMADDCPH(Opcode) || isVFCMADDCSH(Opcode) || isVFMADDCPH(Opcode) || isVFMADDCSH(Opcode)) {
    ...
    } else if (isVFCMULCPH(Opcode) || isVFCMULCSH(Opcode) || isVFMULCPH(Opcode) || isVFMULCSH(Opcode)) {
        unsigned Dest = Inst.getOperand(0).getReg();
        ...
        for (unsigned i = TSFlags & X86II::EVEX_K ? 2 : 1;
            ...
            if (Inst.getOperand(i).isReg() && Dest == Inst.getOperand(i).getReg())
                return Warning(Ops[0]->getStartLoc(),
                               "Destination register should be "
                               "distinct from source registers");
    } else if (isV4FMADDPS(Opcode) || isV4FMADDSS(Opcode) || isV4FNMADDPS(Opcode) || isV4FNMADDSS(Opcode) || isVP4DPWSSDS(Opcode) || isVP4DPWSSD(Opcode)) {
        unsigned Src2 = Inst.getOperand(Inst.getNumOperands() - X86::AddrNumOperands - 1).getReg();
        ...
        if (Src2Enc % 4 != 0) {
            ...
            return Warning(Ops[0]->getStartLoc(), "source register '" + RegName + "' implicitly denotes '" + RegName.take_front(3) + Twine(GroupStart) + "' to '" + RegName.take_front(3) + Twine(GroupEnd) + "' source group");
        }
    } else if (isVGATHERDPD(Opcode) || isVGATHERDPS(Opcode) || isVGATHERQPD(Opcode) || isVGATHERQPS(Opcode) || isVPGATHERDD(Opcode) || isVPGATHERDQ(Opcode) || isVPGATHERQD(Opcode) || isVPGATHERQQ(Opcode)) {
...
void
X86AsmParser::emitWarningForSpecialLVIInstruction(SMLoc Loc) {
    Warning(Loc,
...
void
X86AsmParser::applyLVICFIMitigation(MCInst& Inst, MCStreamer& Out) {
...
/// Returns `true` if a mitigation was applied or warning was emitted.
void
X86AsmParser::applyLVILoadHardeningMitigation(MCInst& Inst, MCStreamer& Out) {
...
void
X86AsmParser::emitInstruction(MCInst& Inst, OperandVector& Operands, MCStreamer& Out) {
    if (LVIInlineAsmHardening && getSTI().getFeatureBits()[X86::FeatureLVIControlFlowIntegrity])
        applyLVICFIMitigation(Inst, Out);

    Out.emitInstruction(Inst, getSTI());

    if (LVIInlineAsmHardening && getSTI().getFeatureBits()[X86::FeatureLVILoadHardening])
        applyLVILoadHardeningMitigation(Inst, Out);
}

bool
X86AsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned& Opcode, OperandVector& Operands, MCStreamer& Out, uint64_t& ErrorInfo, bool MatchingInlineAsm) {
    if (isParsingIntelSyntax())
        return MatchAndEmitIntelInstruction(IDLoc, Opcode, Operands, Out, ErrorInfo, MatchingInlineAsm);
    return MatchAndEmitATTInstruction(IDLoc, Opcode, Operands, Out, ErrorInfo, MatchingInlineAsm);
}

void
X86AsmParser::MatchFPUWaitAlias(SMLoc IDLoc, X86Operand& Op, OperandVector& Operands, MCStreamer& Out, bool MatchingInlineAsm) {
...
bool
X86AsmParser::ErrorMissingFeature(SMLoc IDLoc, const FeatureBitset& MissingFeatures, bool MatchingInlineAsm) {
...
static unsigned
getPrefixes(OperandVector& Operands) {
    unsigned    Result = 0;
...
unsigned
X86AsmParser::checkTargetMatchPredicate(MCInst& Inst) {
    unsigned           Opc = Inst.getOpcode();
    const MCInstrDesc& MCID = MII.get(Opc);

    if (ForcedVEXEncoding == VEXEncoding_EVEX && (MCID.TSFlags & X86II::EncodingMask) != X86II::EVEX)
        return Match_Unsupported;

    if ((ForcedVEXEncoding == VEXEncoding_VEX || ForcedVEXEncoding == VEXEncoding_VEX2 || ForcedVEXEncoding == VEXEncoding_VEX3) && (MCID.TSFlags & X86II::EncodingMask) != X86II::VEX)
        return Match_Unsupported;

    // These instructions are only available with {vex}, {vex2} or {vex3} prefix
    if (MCID.TSFlags & X86II::ExplicitVEXPrefix && (ForcedVEXEncoding != VEXEncoding_VEX && ForcedVEXEncoding != VEXEncoding_VEX2 && ForcedVEXEncoding != VEXEncoding_VEX3))
        return Match_Unsupported;
...
bool
X86AsmParser::MatchAndEmitATTInstruction(SMLoc IDLoc, unsigned& Opcode, OperandVector& Operands, MCStreamer& Out, uint64_t& ErrorInfo, bool MatchingInlineAsm) {
    ...
    unsigned      OriginalError = MatchInstruction(Operands, Inst, ErrorInfo, MissingFeatures, MatchingInlineAsm, isParsingIntelSyntax());
    if (ForcedDataPrefix == X86::Is32Bit) {
        SwitchMode(X86::Is16Bit);
        ForcedDataPrefix = 0;
    }
    switch (OriginalError) {
        ...
        case Match_InvalidImmUnsignedi4: {
            ...
            if (ErrorLoc == SMLoc())
                ErrorLoc = IDLoc;
            return Error(ErrorLoc, "immediate must be an integer in range [0, 15]", EmptyRange, MatchingInlineAsm);
        }
    ...
    if (Op.getToken().empty()) {
        Error(IDLoc, "instruction must have size higher than 0", EmptyRange, MatchingInlineAsm);
        return true;
    }
    ...
    for (unsigned I = 0, E = std::size(Match); I != E; ++I) {
        ...
        if (MemOp || !HasVectorReg) {
            Match[I] =
                MatchInstruction(Operands, Inst, ErrorInfoIgnore, MissingFeatures, MatchingInlineAsm, isParsingIntelSyntax());
    ...
    if (llvm::count(Match, Match_MnemonicFail) == 4) {
        if (OriginalError == Match_MnemonicFail)
            return Error(IDLoc, "invalid instruction mnemonic '" + Base + "'", Op.getLocRange(), MatchingInlineAsm);

        if (OriginalError == Match_Unsupported)
            return Error(IDLoc, "unsupported instruction", EmptyRange, MatchingInlineAsm);

        assert(OriginalError == Match_InvalidOperand && "Unexpected error");
        // Recover location info for the operand if we know which was the problem.
        if (ErrorInfo != ~0ULL) {
            if (ErrorInfo >= Operands.size())
                return Error(IDLoc, "too few operands for instruction", EmptyRange, MatchingInlineAsm);

            X86Operand& Operand = (X86Operand&)*Operands[ErrorInfo];
            if (Operand.getStartLoc().isValid()) {
                SMRange OperandRange = Operand.getLocRange();
                return Error(Operand.getStartLoc(), "invalid operand for instruction", OperandRange, MatchingInlineAsm);
            }
        }

        return Error(IDLoc, "invalid operand for instruction", EmptyRange, MatchingInlineAsm);
    }
    ...
    if (llvm::count(Match, Match_MissingFeature) == 1) {
        ...
        return ErrorMissingFeature(IDLoc, ErrorInfoMissingFeatures, MatchingInlineAsm);
    }
    ...
    Error(IDLoc, "unknown use of instruction mnemonic without a size suffix", EmptyRange, MatchingInlineAsm);
    return true;
}

bool
X86AsmParser::MatchAndEmitIntelInstruction(SMLoc IDLoc, unsigned& Opcode, OperandVector& Operands, MCStreamer& Out, uint64_t& ErrorInfo, bool MatchingInlineAsm) {
    assert(!Operands.empty() && "Unexpect empty operand list!");
    assert((*Operands[0]).isToken() && "Leading operand should always be a mnemonic!");
    ...
    if (Mnemonic == "push" && Operands.size() == 2) {
        ...
        if (X86Op->isImm()) {
            ...
            if (CE && (isIntN(Size, CE->getValue()) || isUIntN(Size, CE->getValue()))) {
                SmallString<16> Tmp;
                Tmp += Base;
                Tmp += (is64BitMode())
                    ? "q"
                ...
                Match.push_back(MatchInstruction(Operands, Inst, ErrorInfo, MissingFeatures, MatchingInlineAsm, false /*isParsingIntelSyntax()*/));
                Op.setTokenValue(Base);
            }
        }
    }
    ...
    if (UnsizedMemOp && UnsizedMemOp->isMemUnsized()) {
        ...
        for (unsigned Size : MopSizes) {
            ...
            unsigned M = MatchInstruction(Operands, Inst, ErrorInfoIgnore, MissingFeatures, MatchingInlineAsm, isParsingIntelSyntax());
    ...
    if (Match.back() == Match_MnemonicFail) {
        return Error(IDLoc, "invalid instruction mnemonic '" + Mnemonic + "'", Op.getLocRange(), MatchingInlineAsm);
    }
    ...
    if (UnsizedMemOp && NumSuccessfulMatches > 1 && UnsizedMemOp->getMemFrontendSize()) {
    ...
    if (NumSuccessfulMatches == 1) {
        ...
        assert(UnsizedMemOp && "multiple matches only possible with unsized memory operands");
        return Error(UnsizedMemOp->getStartLoc(), "ambiguous operand size for instruction '" + Mnemonic + "\'", UnsizedMemOp->getLocRange());
    }
    ...
    // If all of these were an outright failure, report it in a useless way.
    return Error(IDLoc, "unknown instruction mnemonic", EmptyRange, MatchingInlineAsm);
}

bool
X86AsmParser::OmitRegisterFromClobberLists(unsigned RegNo) {
...
X86AsmParser::ParseDirective(AsmToken DirectiveID) {
    ...
    else if (IDVal == ".seh_pushreg" || (Parser.isParsingMasm() && IDVal.equals_insensitive(".pushreg")))
        return parseDirectiveSEHPushReg(DirectiveID.getLoc());
    else if (IDVal == ".seh_setframe" || (Parser.isParsingMasm() && IDVal.equals_insensitive(".setframe")))
        return parseDirectiveSEHSetFrame(DirectiveID.getLoc());
    else if (IDVal == ".seh_savereg" || (Parser.isParsingMasm() && IDVal.equals_insensitive(".savereg")))
        return parseDirectiveSEHSaveReg(DirectiveID.getLoc());
    else if (IDVal == ".seh_savexmm" || (Parser.isParsingMasm() && IDVal.equals_insensitive(".savexmm128")))
        return parseDirectiveSEHSaveXMM(DirectiveID.getLoc());
    else if (IDVal == ".seh_pushframe" || (Parser.isParsingMasm() && IDVal.equals_insensitive(".pushframe")))
...
bool
X86AsmParser::parseDirectiveArch() {
    // Ignore .arch for now.
...
///  ::= .nops size[, control]
bool
X86AsmParser::parseDirectiveNops(SMLoc L) {
    int64_t                NumBytes = 0, Control = 0;
    SMLoc                  NumBytesLoc, ControlLoc;
    const MCSubtargetInfo& STI = getSTI();
    NumBytesLoc = getTok().getLoc();
    if (getParser().checkForValidSection() || getParser().parseAbsoluteExpression(NumBytes))
        return true;
...
///  ::= .even
bool
X86AsmParser::parseDirectiveEven(SMLoc L) {
    if (parseEOL())
        return false;
...
// .cv_fpo_proc foo
bool
X86AsmParser::parseDirectiveFPOProc(SMLoc L) {
    MCAsmParser& Parser = getParser();
    StringRef    ProcName;
    int64_t      ParamsSize;
...
// .cv_fpo_setframe ebp
bool
X86AsmParser::parseDirectiveFPOSetFrame(SMLoc L) {
    MCRegister Reg;
    SMLoc      DummyLoc;
...
// .cv_fpo_pushreg ebx
bool
X86AsmParser::parseDirectiveFPOPushReg(SMLoc L) {
    MCRegister Reg;
    SMLoc      DummyLoc;
    if (parseRegister(Reg, DummyLoc, DummyLoc) || parseEOL())
        return true;
...
bool
X86AsmParser::parseDirectiveFPOStackAlloc(SMLoc L) {
    MCAsmParser& Parser = getParser();
    int64_t      Offset;
...
// .cv_fpo_stackalign 8
bool
X86AsmParser::parseDirectiveFPOStackAlign(SMLoc L) {
    MCAsmParser& Parser = getParser();
    int64_t      Offset;
    if (Parser.parseIntToken(Offset, "expected offset") || parseEOL())
        return true;
...
// .cv_fpo_endprologue
bool
X86AsmParser::parseDirectiveFPOEndPrologue(SMLoc L) {
    MCAsmParser& Parser = getParser();
    if (Parser.parseEOL())
        return true;
...
// .cv_fpo_endproc
bool
X86AsmParser::parseDirectiveFPOEndProc(SMLoc L) {
    MCAsmParser& Parser = getParser();
    if (Parser.parseEOL())
        return true;
...
bool
X86AsmParser::parseSEHRegisterNumber(unsigned RegClassID, MCRegister& RegNo) {
    ...
    if (getLexer().getTok().isNot(AsmToken::Integer)) {
        ...
        if (!X86MCRegisterClasses[RegClassID].contains(RegNo)) {
            return Error(startLoc, "register is not supported for use with this directive");
        }
    } else {
        ...
        if (RegNo == 0) {
            return Error(startLoc, "incorrect register number for use with this directive");
        }
    }
...
bool
X86AsmParser::parseDirectiveSEHPushReg(SMLoc Loc) {
    MCRegister Reg;
...
bool
X86AsmParser::parseDirectiveSEHSetFrame(SMLoc Loc) {
    MCRegister Reg;
    int64_t    Off;
    if (parseSEHRegisterNumber(X86::GR64RegClassID, Reg))
        return true;
...
bool
X86AsmParser::parseDirectiveSEHSaveReg(SMLoc Loc) {
    MCRegister Reg;
    int64_t    Off;
    if (parseSEHRegisterNumber(X86::GR64RegClassID, Reg))
        return true;
...
bool
X86AsmParser::parseDirectiveSEHSaveXMM(SMLoc Loc) {
    MCRegister Reg;
    int64_t    Off;
...
bool
X86AsmParser::parseDirectiveSEHPushFrame(SMLoc Loc) {
    bool      Code = false;
    StringRef CodeID;
...
static MCTargetAsmParser*
X86AsmParserCtorFn(const MCSubtargetInfo& STI, MCAsmParser& P, const MCInstrInfo& MII, const MCTargetOptions& Options) {
    return new X86AsmParser(STI, P, MII, Options);
}

extern "C" LLVM_EXTERNAL_VISIBILITY void
LLVMInitializeX86AsmParser() {
    LLVMTargetRegistryTheTarget->MCAsmParserCtorFn = X86AsmParserCtorFn;
}
