...
#include "clang_include_clang_CodeGen_BackendUtil.h"
...
#include "llvm_include_llvm_CodeGen_RegAllocRegistry.h"
#include "llvm_include_llvm_CodeGen_SchedulerRegistry.h"
...
#include "llvm_include_llvm_IR_ModuleSummaryIndex.h"
...
#include "llvm_include_llvm_Passes_PassPlugin.h"
...
#include "llvm_include_llvm_Transforms_IPO_ThinLTOBitcodeWriter.h"
...
#include "llvm_include_llvm_Transforms_Instrumentation_AddressSanitizerOptions.h"
...
#include "llvm_include_llvm_Support_Extension.def"
...
class EmitAssemblyHelper {
    ...
    bool AddEmitPasses(legacy::PassManager& CodeGenPasses, BackendAction Action, raw_pwrite_stream& OS, raw_pwrite_stream* DwoOS);
    ...
    void
         RunOptimizationPipeline(BackendAction Action, std::unique_ptr<raw_pwrite_stream>& OS, std::unique_ptr<llvm::ToolOutputFile>& ThinLinkOS);
    void RunCodegenPipeline(BackendAction Action, std::unique_ptr<raw_pwrite_stream>& OS, std::unique_ptr<llvm::ToolOutputFile>& DwoOS);
    ...
    shouldEmitRegularLTOSummary() const {
        return CodeGenOpts.PrepareForLTO && !CodeGenOpts.DisableLLVMPasses && TargetTriple.getVendor() != llvm::Triple::Apple;
    }

  public:
    EmitAssemblyHelper(DiagnosticsEngine& _Diags, const HeaderSearchOptions& HeaderSearchOpts, const CodeGenOptions& CGOpts, const clang::TargetOptions& TOpts, const LangOptions& LOpts, Module* M) :
        Diags(_Diags),
...
getCGOptLevel(const CodeGenOptions& CodeGenOpts) {
...
actionRequiresCodeGen(BackendAction Action) {
    return Action != Backend_EmitNothing && Action != Backend_EmitBC && Action != Backend_EmitLL;
}

static bool
initTargetOptions(DiagnosticsEngine& Diags, llvm::TargetOptions& Options, const CodeGenOptions& CodeGenOpts, const clang::TargetOptions& TargetOpts, const LangOptions& LangOpts, const HeaderSearchOptions& HSOpts) {
    ...
    // Set float ABI type.
    assert((CodeGenOpts.FloatABI == "soft" || CodeGenOpts.FloatABI == "softfp" || CodeGenOpts.FloatABI == "hard" || CodeGenOpts.FloatABI.empty()) && "Invalid Floating Point ABI!");
    ...
    Options.UnsafeFPMath = LangOpts.AllowFPReassoc && LangOpts.AllowRecip && LangOpts.NoSignedZero && LangOpts.ApproxFunc && (LangOpts.getDefaultFPContractMode() == LangOptions::FPModeKind::FPM_Fast || LangOpts.getDefaultFPContractMode() == LangOptions::FPModeKind::FPM_FastHonorPragmas);
    ...
    for (const auto& Entry : HSOpts.UserEntries)
        if (!Entry.IsFramework && (Entry.Group == frontend::IncludeDirGroup::Quoted || Entry.Group == frontend::IncludeDirGroup::Angled || Entry.Group == frontend::IncludeDirGroup::System))
...
bool
EmitAssemblyHelper::AddEmitPasses(legacy::PassManager& CodeGenPasses, BackendAction Action, raw_pwrite_stream& OS, raw_pwrite_stream* DwoOS) {
    // Add LibraryInfo.
...
addKCFIPass(const Triple& TargetTriple, const LangOptions& LangOpts, PassBuilder& PB) {
    ...
    if (TargetTriple.getArch() == llvm::Triple::x86_64 || TargetTriple.isAArch64(64))
        return;

    // Ensure we lower KCFI operand bundles with -O0.
    PB.registerOptimizerLastEPCallback(
        [&](ModulePassManager& MPM, OptimizationLevel Level) {
            if (Level == OptimizationLevel::O0 && LangOpts.Sanitize.has(SanitizerKind::KCFI))
    ...
    auto SanitizersCallback = [&](ModulePassManager& MPM,
        ...
        auto MSanPass = [&](SanitizerMask Mask, bool CompileKernel) {
            if (LangOpts.Sanitize.has(Mask)) {
                ...
                MemorySanitizerOptions options(TrackOrigins, Recover, CompileKernel, CodeGenOpts.SanitizeMemoryParamRetval);
        ...
        auto ASanPass = [&](SanitizerMask Mask, bool CompileKernel) {
            if (LangOpts.Sanitize.has(Mask)) {
                ...
                MPM.addPass(AddressSanitizerPass(Opts, UseGlobalGC, UseOdrIndicator, DestructorKind));
            }
        };
...
EmitAssemblyHelper::RunOptimizationPipeline(
    ...
    if (CodeGenOpts.hasProfileIRInstr())
        // -fprofile-generate.
        PGOOpt = PGOOptions(CodeGenOpts.InstrProfileOutput.empty() ? getDefaultProfileGenName() : CodeGenOpts.InstrProfileOutput, "", "", PGOOptions::IRInstr, PGOOptions::NoCSAction, CodeGenOpts.DebugInfoForProfiling);
    else if (CodeGenOpts.hasProfileIRUse()) {
        // -fprofile-use.
        auto CSAction = CodeGenOpts.hasProfileCSIRUse() ? PGOOptions::CSIRUse
                                                        : PGOOptions::NoCSAction;
        PGOOpt = PGOOptions(CodeGenOpts.ProfileInstrumentUsePath, "", CodeGenOpts.ProfileRemappingFile, PGOOptions::IRUse, CSAction, CodeGenOpts.DebugInfoForProfiling);
    } else if (!CodeGenOpts.SampleProfileFile.empty())
        // -fprofile-sample-use
        PGOOpt = PGOOptions(
            CodeGenOpts.SampleProfileFile,
            "",
        ...
        PGOOpt =
            PGOOptions("", "", "", PGOOptions::NoAction, PGOOptions::NoCSAction, CodeGenOpts.DebugInfoForProfiling, true);
    else if (CodeGenOpts.DebugInfoForProfiling)
        // -fdebug-info-for-profiling
        PGOOpt = PGOOptions("", "", "", PGOOptions::NoAction, PGOOptions::NoCSAction, true);
    ...
    if (CodeGenOpts.hasProfileCSIRInstr()) {
        assert(!CodeGenOpts.hasProfileCSIRUse() &&
               ...
               "the same time");
        if (PGOOpt) {
            assert(PGOOpt->Action != PGOOptions::IRInstr && PGOOpt->Action != PGOOptions::SampleUse &&
            ...
            PGOOpt = PGOOptions("", CodeGenOpts.InstrProfileOutput.empty() ? getDefaultProfileGenName() : CodeGenOpts.InstrProfileOutput, "", PGOOptions::NoAction, PGOOptions::CSIRInstr, CodeGenOpts.DebugInfoForProfiling);
    }
    if (TM)
...
#include "llvm_include_llvm_Support_Extension.def"
    ...
    if (!CodeGenOpts.DisableLLVMPasses) {
        ...
        if (CodeGenOpts.InstrumentFunctions || CodeGenOpts.InstrumentFunctionEntryBare || CodeGenOpts.InstrumentFunctionsAfterInlining || CodeGenOpts.InstrumentForProfiling) {
            PB.registerPipelineStartEPCallback(
                [](ModulePassManager& MPM, OptimizationLevel Level) {
    ...
    if (Action == Backend_EmitBC || Action == Backend_EmitLL) {
        if (CodeGenOpts.PrepareForThinLTO && !CodeGenOpts.DisableLLVMPasses) {
            if (!TheModule->getModuleFlag("EnableSplitLTOUnit"))
                TheModule->addModuleFlag(Module::Error, "EnableSplitLTOUnit", CodeGenOpts.EnableSplitLTOUnit);
            if (Action == Backend_EmitBC) {
                ...
                MPM.addPass(ThinLTOBitcodeWriterPass(*OS, ThinLinkOS ? &ThinLinkOS->os() : nullptr));
            } else {
            ...
            if (EmitLTOSummary) {
                ...
                if (!TheModule->getModuleFlag("EnableSplitLTOUnit"))
                    TheModule->addModuleFlag(Module::Error, "EnableSplitLTOUnit", uint32_t(1));
            }
            if (Action == Backend_EmitBC)
...
EmitAssemblyHelper::RunCodegenPipeline(
    ...
    switch (Action) {
        ...
        case Backend_EmitObj:
            ...
            if (!AddEmitPasses(CodeGenPasses, Action, *OS, DwoOS ? &DwoOS->os() : nullptr))
...
void
EmitAssemblyHelper::EmitAssembly(BackendAction Action, std::unique_ptr<raw_pwrite_stream> OS) {
...
runThinLTOBackend(
    ...
    auto AddStream = [&](size_t Task, const Twine& ModuleName) {
        return std::make_unique<CachedFileStream>(std::move(OS), CGOpts.ObjectFilenameForDebug);
    };
    lto::Config Conf;
    ...
    if (Error E =
            thinBackend(Conf, -1, AddStream, *M, *CombinedIndex, ImportList, ModuleToDefinedGVSummaries[M->getModuleIdentifier()],
                        /* ModuleMap */ nullptr,
                        CGOpts.CmdArgs)) {
        handleAllErrors(std::move(E), [&](ErrorInfoBase& EIB) {
...
void
clang::EmitBackendOutput(DiagnosticsEngine& Diags, const HeaderSearchOptions& HeaderOpts, const CodeGenOptions& CGOpts, const clang::TargetOptions& TOpts, const LangOptions& LOpts, StringRef TDesc, Module* M, BackendAction Action, std::unique_ptr<raw_pwrite_stream> OS) {
    ...
    if (!CGOpts.ThinLTOIndexFile.empty()) {
        ...
        if (Error E = llvm::getModuleSummaryIndexForFile(
            ...
            logAllUnhandledErrors(std::move(E), errs(), "Error loading index file '" + CGOpts.ThinLTOIndexFile + "': ");
            return;
        }
        ...
        if (CombinedIndex) {
            if (!CombinedIndex->skipModuleByDistributedBackend()) {
                runThinLTOBackend(Diags, CombinedIndex.get(), M, HeaderOpts, CGOpts, TOpts, LOpts, std::move(OS), CGOpts.SampleProfileFile, CGOpts.ProfileRemappingFile, Action);
                return;
            }
...
void
clang::EmbedBitcode(llvm::Module* M, const CodeGenOptions& CGOpts, llvm::MemoryBufferRef Buf) {
...
void
clang::EmbedObject(llvm::Module* M, const CodeGenOptions& CGOpts, DiagnosticsEngine& Diags) {
    ...
    for (StringRef OffloadObject : CGOpts.OffloadObjects) {
        ...
        if (std::error_code EC = ObjectOrErr.getError()) {
            auto DiagID = Diags.getCustomDiagID(DiagnosticsEngine::Error, "could not open '%0' for embedding");
            Diags.Report(DiagID) << OffloadObject;
            return;
        }

        llvm::embedBufferInModule(*M, **ObjectOrErr, ".llvm.offloading", Align(object::OffloadBinary::getAlignment()));
    }
}
