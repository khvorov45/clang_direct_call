...
#include "llvm_include_llvm_Object_ELFObjectFile.h"
...
#include "llvm_include_llvm_Object_ELFTypes.h"
#include "llvm_include_llvm_Object_Error.h"
#include "llvm_include_llvm_Support_ARMAttributeParser.h"
#include "llvm_include_llvm_Support_ARMBuildAttributes.h"
#include "llvm_include_llvm_Support_ErrorHandling.h"
#include "llvm_include_llvm_Support_MathExtras.h"
#include "llvm_include_llvm_Support_RISCVAttributeParser.h"
#include "llvm_include_llvm_Support_RISCVAttributes.h"
#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <memory>
...
ELFObjectFileBase::getPltAddresses() const {
  ...
  const auto *T = LLVMTargetRegistryTheTarget;
  if (!T)
    return {};
  ...
  std::unique_ptr<const MCInstrInfo> MII(T->MCInstrInfoCtorFn());
  if (T->MCInstrAnalysisCtorFn == 0) {
    return {};
  }
  std::unique_ptr<const MCInstrAnalysis> MIA(T->MCInstrAnalysisCtorFn(MII.get()));
