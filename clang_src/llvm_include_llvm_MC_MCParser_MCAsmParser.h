...
#include "llvm_include_llvm_ADT_STLFunctionalExtras.h"
...
#include "llvm_include_llvm_MC_MCAsmMacro.h"
...
struct InlineAsmIdentifierInfo {
    ...
    bool
    isKind(IdKind kind) const {
        return Kind == kind;
    }
    // Initializers
    void
    setEnum(int64_t enumVal) {
    ...
    void
    setVar(void* decl, bool isGlobalLV, unsigned size, unsigned type) {
        assert(isKind(IK_Invalid) && "should be initialized only once");
        Kind = IK_Var;
        Var.Decl = decl;
...
class MCAsmParser {
  ...
  public:
    ...
    virtual void
    setAssemblerDialect(unsigned i) {}

    bool
    getShowParsedOperands() const {
        return ShowParsedOperands;
    }
    void
    setShowParsedOperands(bool Value) {
    ...
    virtual bool
    discardLTOSymbol(StringRef) const {
        return false;
    }
    ...
    virtual bool
    defineMacro(StringRef Name, StringRef Value) {
        return true;
    }

    virtual bool
    lookUpField(StringRef Name, AsmFieldInfo& Info) const {
        return true;
    }
    virtual bool
    lookUpField(StringRef Base, StringRef Member, AsmFieldInfo& Info) const {
        return true;
    }

    virtual bool
    lookUpType(StringRef Name, AsmTypeInfo& Info) const {
        return true;
    }
    ...
    bool
    parseComma() {
        return parseToken(AsmToken::Comma, "expected comma");
    }
    bool
    parseRParen() {
