...
#include "llvm_lib_Target_X86_MCTargetDesc_X86MCAsmInfo.h"
...
#include "llvm_include_llvm_MC_MCInstrAnalysis.h"
...
#include "llvm_include_llvm_MC_MachineLocation.h"
...
std::string
X86_MC::ParseX86Triple(const Triple& TT) {
    std::string FS;
...
unsigned
X86_MC::getDwarfRegFlavour(const Triple& TT, bool isEH) {
...
bool
X86_MC::hasLockPrefix(const MCInst& MI) {
    return MI.getFlags() & X86::IP_HAS_LOCK;
}

static bool
isMemOperand(const MCInst& MI, unsigned Op, unsigned RegClassID) {
    ...
    return (Base.isReg() && Base.getReg() != 0 && RC.contains(Base.getReg())) || (Index.isReg() && Index.getReg() != 0 && RC.contains(Index.getReg()));
}

bool
X86_MC::is16BitMemOperand(const MCInst& MI, unsigned Op, const MCSubtargetInfo& STI) {
    const MCOperand& Base = MI.getOperand(Op + X86::AddrBaseReg);
    const MCOperand& Index = MI.getOperand(Op + X86::AddrIndexReg);

    if (STI.hasFeature(X86::Is16Bit) && Base.isReg() && Base.getReg() == 0 && Index.isReg() && Index.getReg() == 0)
        return true;
    return isMemOperand(MI, Op, X86::GR16RegClassID);
}

bool
X86_MC::is32BitMemOperand(const MCInst& MI, unsigned Op) {
    const MCOperand& Base = MI.getOperand(Op + X86::AddrBaseReg);
    const MCOperand& Index = MI.getOperand(Op + X86::AddrIndexReg);
...
#ifndef NDEBUG
bool
X86_MC::is64BitMemOperand(const MCInst& MI, unsigned Op) {
    return isMemOperand(MI, Op, X86::GR64RegClassID);
}
#endif

bool
X86_MC::needsAddressSizeOverride(const MCInst& MI, const MCSubtargetInfo& STI, int MemoryOperand, uint64_t TSFlags) {
    ...
    if ((Is16BitMode && AdSize == X86II::AdSize32) || (Is32BitMode && AdSize == X86II::AdSize16) || (Is64BitMode && AdSize == X86II::AdSize32))
        return true;
    ...
    switch (Form) {
        ...
        case X86II::RawFrmDstSrc: {
            ...
            assert(((siReg == X86::SI && MI.getOperand(0).getReg() == X86::DI) || (siReg == X86::ESI && MI.getOperand(0).getReg() == X86::EDI) || (siReg == X86::RSI && MI.getOperand(0).getReg() == X86::RDI)) && "SI and DI register sizes do not match");
            return (!Is32BitMode && siReg == X86::ESI) || (Is32BitMode && siReg == X86::SI);
        }
        ...
        case X86II::RawFrmDst: {
            unsigned siReg = MI.getOperand(0).getReg();
            return (!Is32BitMode && siReg == X86::EDI) || (Is32BitMode && siReg == X86::DI);
        }
    }
...
void
X86_MC::initLLVMToSEHAndCVRegMapping(MCRegisterInfo* MRI) {
...
createX86MCRegisterInfo(const Triple& TT) {
    unsigned RA = (TT.getArch() == Triple::x86_64)
        ? X86::RIP  // Should have dwarf #16.
        : X86::EIP;  // Should have dwarf #8.

    MCRegisterInfo* X = new MCRegisterInfo();
    InitX86MCRegisterInfo(X, RA, X86_MC::getDwarfRegFlavour(TT, false), X86_MC::getDwarfRegFlavour(TT, true), RA);
    X86_MC::initLLVMToSEHAndCVRegMapping(X);
    return X;
}

static MCAsmInfo*
createX86MCAsmInfo(const MCRegisterInfo& MRI, const Triple& TheTriple, const MCTargetOptions& Options) {
    ...
    } else if (TheTriple.isWindowsMSVCEnvironment() || TheTriple.isWindowsCoreCLREnvironment()) {
        if (Options.getAssemblyLanguage().equals_insensitive("masm"))
            MAI = new X86MCAsmInfoMicrosoftMASM(TheTriple);
        else
            MAI = new X86MCAsmInfoMicrosoft(TheTriple);
    } else if (TheTriple.isOSCygMing() || TheTriple.isWindowsItaniumEnvironment()) {
    ...
    MCCFIInstruction Inst = MCCFIInstruction::cfiDefCfa(
        nullptr,
        MRI.getDwarfRegNum(StackPtr, true),
        -stackGrowth
    );
    ...
    MCCFIInstruction Inst2 = MCCFIInstruction::createOffset(
        nullptr,
        MRI.getDwarfRegNum(InstPtr, true),
        stackGrowth
    );
...
createX86MCInstPrinter(const Triple& T, unsigned SyntaxVariant, const MCAsmInfo& MAI, const MCInstrInfo& MII, const MCRegisterInfo& MRI) {
    if (SyntaxVariant == 0)
...
createX86MCRelocationInfo(const Triple& TheTriple, MCContext& Ctx) {
...
#include "X86GenSubtargetInfo.inc"
        ...
        findPltEntries(uint64_t PltSectionVA, ArrayRef<uint8_t> PltContents, uint64_t GotSectionVA, const Triple& TargetTriple) const override;

        bool evaluateBranch(const MCInst& Inst, uint64_t Addr, uint64_t Size, uint64_t& Target) const override;
        std::optional<uint64_t>
        evaluateMemoryOperandAddress(const MCInst& Inst, const MCSubtargetInfo* STI, uint64_t Addr, uint64_t Size) const override;
        std::optional<uint64_t>
        getMemoryOperandRelocationOffset(const MCInst& Inst, uint64_t Size) const override;
    };
...
#include "X86GenSubtargetInfo.inc"
    ...
    bool
    X86MCInstrAnalysis::clearsSuperRegisters(const MCRegisterInfo& MRI, const MCInst& Inst, APInt& Mask) const {
        const MCInstrDesc& Desc = Info->get(Inst.getOpcode());
        unsigned           NumDefs = Desc.getNumDefs();
        unsigned           NumImplicitDefs = Desc.getNumImplicitDefs();
        assert(Mask.getBitWidth() == NumDefs + NumImplicitDefs && "Unexpected number of bits in the mask!");
    ...
    findX86PltEntries(uint64_t PltSectionVA, ArrayRef<uint8_t> PltContents, uint64_t GotPltSectionVA) {
    ...
    std::vector<std::pair<uint64_t, uint64_t>>
    X86MCInstrAnalysis::findPltEntries(
    ...
    bool
    X86MCInstrAnalysis::evaluateBranch(const MCInst& Inst, uint64_t Addr, uint64_t Size, uint64_t& Target) const {
        if (Inst.getNumOperands() == 0 || Info->get(Inst.getOpcode()).OpInfo[0].OperandType != MCOI::OPERAND_PCREL)
            return false;
    ...
    std::optional<uint64_t>
    X86MCInstrAnalysis::evaluateMemoryOperandAddress(
        const MCInst&          Inst,
        ...
        if (SegReg.getReg() != 0 || IndexReg.getReg() != 0 || ScaleAmt.getImm() != 1 || !Disp.isImm())
            return std::nullopt;
    ...
    std::optional<uint64_t>
    X86MCInstrAnalysis::getMemoryOperandRelocationOffset(const MCInst& Inst, uint64_t Size) const {
        ...
        if (BaseReg.getReg() != X86::RIP || SegReg.getReg() != 0 || IndexReg.getReg() != 0 || ScaleAmt.getImm() != 1 || !Disp.isImm())
            return std::nullopt;
...
LLVMInitializeX86TargetMC() {
    LLVMTargetRegistryTheTarget->MCAsmInfoCtorFn = createX86MCAsmInfo;
    LLVMTargetRegistryTheTarget->MCInstrInfoCtorFn = createX86MCInstrInfo;
    LLVMTargetRegistryTheTarget->MCRegInfoCtorFn = createX86MCRegisterInfo;
    LLVMTargetRegistryTheTarget->MCSubtargetInfoCtorFn = X86_MC::createX86MCSubtargetInfo;
    LLVMTargetRegistryTheTarget->MCInstrAnalysisCtorFn = createX86MCInstrAnalysis;
    LLVMTargetRegistryTheTarget->MCCodeEmitterCtorFn = createX86MCCodeEmitter;
    LLVMTargetRegistryTheTarget->ObjectTargetStreamerCtorFn = createX86ObjectTargetStreamer;
    LLVMTargetRegistryTheTarget->AsmTargetStreamerCtorFn = createX86AsmTargetStreamer;
    LLVMTargetRegistryTheTarget->NullTargetStreamerCtorFn = createX86NullTargetStreamer;
    LLVMTargetRegistryTheTarget->COFFStreamerCtorFn = createX86WinCOFFStreamer;
    LLVMTargetRegistryTheTarget->MCInstPrinterCtorFn = createX86MCInstPrinter;
    LLVMTargetRegistryTheTarget->MCRelocationInfoCtorFn = createX86MCRelocationInfo;
    LLVMTargetRegistryTheTarget->MCAsmBackendCtorFn = createX86_64AsmBackend;
}

MCRegister
llvm::getX86SubSuperRegisterOrZero(MCRegister Reg, unsigned Size, bool High) {
    switch (Size) {
...
MCRegister
llvm::getX86SubSuperRegister(MCRegister Reg, unsigned Size, bool High) {
